...
The CI implementation is [ci_operator_perf.py](../../.github/scripts/ci_operator_perf.py).
Its `ci` command retrieves history and publishes results; `suite` holds the
grouped device allocations; `case` captures one model's golden and timing result
in a fresh process; `finalize` preserves checkpoints after queue cleanup.
`operator_perf_cases.json` defines all twenty workloads and their devices.
...
| MTP MoE | EP8 | 16 experts/rank, 8 tokens/rank, layer 0, default random routes |
| MTP LM-head | TP1, DP1 | 8 active logit rows, projection |
| DSpark CSA, HCA, SWA | TP1 | B=16, S=8, T=128, start position 8192 or 131072 |
| DSpark MoE | EP8 | 16 experts/rank, 128 tokens/rank, layer 0, default uniform routes |
...
EP8 expert parallelism. Both MoE entries use 16 experts/rank (128 total) and
default random routing with the suite's fixed seed. For MTP, the CI capture
consumes the manifest's `--experts-per-rank 16` before invoking the model. It sets
the process-local FLASH expert count before kernel imports and forwards the
existing `--ep 8` option. The model entry and its standalone defaults (EP2,
32 experts/rank) are unchanged; this expert override is a CI option, not a model
CLI option. DSpark already uses 16 experts/rank. The suite does not require perfectly balanced
expert loads or run an EPLB algorithm. These workloads differ from earlier
balanced-route measurements, which cannot supply comparable MoE baselines.
...
The suite makes two serial queue submissions, each holding its allocation
across both models:

```text
# All 14 Attention and LM-head measurements, MTP then DSpark
task-submit --device 4
# Both MoE EP8 measurements, MTP then DSpark
task-submit --device 0,2,4,6,8,10,12,14
```

Each group launches fresh model processes sequentially without nested queue
submissions or releasing its cards between cases. The single-device group
releases device 4 before the EP8 group requests its eight devices. A single-device
case does not reserve seven unused devices. Each case still checks its exact
allocation. Requests must not use `--device-num` or `auto`.

This reduces queue admissions from sixteen to two and prevents requeueing when
switching from MTP to DSpark within a group. The first admission of each group
can still time out in the shared pool. Guaranteed daily availability requires
reserved capacity or a coordinated test window; batching does not provide that.
...
```bash
python -S .github/scripts/ci_operator_perf.py suite --dry-run
```

The plan shows each model's configured arguments and its CI capture command.
Use the capture or suite entry for MTP MoE so the expert override is applied.

Outside any task allocation, source CI's `activate.sh` and run the suite with a
profile that includes the expected `hostname`:

```bash
python .github/scripts/ci_operator_perf.py suite \
...
benchmarking). Each allocation's `submission_wall_seconds` includes queue wait,
all its cases, worker startup and recovery; it is not charged to each case.
Shared wall times must not be summed twice. Golden is computed live;
...
case under the same allocation. A device fault, failed cleanup, unknown recovery state
...
measurement retains `measurement.json`, raw results, model logs and any
recovery observations. `group-1/` and `group-8/` retain their own `queue.log` and
`group-result.json`. The report lists each allocation's status and submission wall
time. Queue return codes and errors are stored independently of
...
failures retain passing rows, fingerprints and metrics. Finalization collects
per-case worker checkpoints again after queue cleanup, including a measurement
completed after the parent's last progress poll. Interrupted rows remain
...
an isolated `operator-checkout` and a distinct build namespace. Per-group queue
...
comparisons unavailable. Device reservations are per group; the host remains
...
active allocation and devices, the active case when one has started, and the
...
| `OPERATOR_PERF_GROUP_TIMEOUT` | 3600 | Allocated group wall time, including cases and recovery |
| `OPERATOR_PERF_QUEUE_TIMEOUT` | 7200 | Per-group client completion wait, including queueing and execution |
| `OPERATOR_PERF_SUITE_TIMEOUT` | 10800 | Entire suite, including queue waits |

Before each model launch, the group worker requires enough remaining group time
for the full case limit plus 90 seconds of overhead, cleanup and recovery. It
leaves later cases explicitly unrun when that budget is exhausted, preserving
completed measurements. The queue watchdog bounds each group by its group limit
plus 30 seconds for final checkpointing and exit.

`task-submit --timeout` includes both queueing and execution; it is not an
acquisition-only timeout. The completion wait must exceed the group limit plus
30 seconds and is capped by the remaining suite budget, reserving another
30 seconds for client exit. The suite refuses to submit when a full group and
exit allowance cannot fit. If the client disconnects before worker completion
is known, stop further submissions and let the workflow clean up this job's
queued tasks. A failed progress monitor first signals its own queue client so
the client's cancellation handler can release the task.
...
GitHub metadata and artifact GETs retry connection interruptions, timeouts and
HTTP 408/500/502/503/504 up to four attempts, waiting 2, 4 and 8 seconds between
attempts. Retries include response-body reads and refresh artifact download URLs.
Permission, certificate and invalid-data errors fail immediately. Exhausted
current-run metadata requests fail setup; exhausted history requests leave
comparisons unavailable while allowing today's measurements to proceed.
...
16 experts/rank on the intended CI stack; the default TP2/EP2
correctness sweep does not cover these specializations. Then complete the 16 unique measurements
