...
from config import DECODE_TOKENS, FLASH as M, PREFILL_TOKENS
...
TP_SIZE = _parse_int_argv("--tp", TP_DEFAULT)
if TP_SIZE not in TP_CHOICES:
    raise ValueError(f"--tp must be one of {TP_CHOICES}, got {TP_SIZE}")
...
if GROUP_T_MAX % TP_SIZE != 0:
    raise ValueError(f"maximum token rows {GROUP_T_MAX} must be divisible by TP {TP_SIZE}")
if D % COMM_D_TILE != 0:
