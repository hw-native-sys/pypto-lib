...
def _make_q_proj_qr(cache_bypass: bool):
    if cache_bypass:
...
q_proj_qr = _make_q_proj_qr(False)
q_proj_qr_bypass = _make_q_proj_qr(True)


def _make_q_proj_q_matmul(cache_bypass: bool):
    if cache_bypass:
        @pl.jit.inline(auto_scope=False)
        def q_proj_q_matmul(
        ...
        def q_proj_q_matmul(
            ...
            return q_proj_i32, qproj_tid
    return q_proj_q_matmul


q_proj_q_matmul = _make_q_proj_q_matmul(False)
q_proj_q_matmul_bypass = _make_q_proj_q_matmul(True)
...
def _make_q_proj_q(cache_bypass: bool):
    matmul_fn = q_proj_q_matmul_bypass if cache_bypass else q_proj_q_matmul
    @pl.jit.inline(auto_scope=False)
    def q_proj_q(
        x: pl.Tensor[[T_DYN, D], pl.BF16],
        wq_b: pl.Tensor[[Q_LORA, H * HEAD_DIM], pl.INT8, pl.NZ],
        ...
        for tile_base in pl.range(0, t_dim, PREFILL_DENSE_TILE):
            ...
            with pl.scope():
                ...
                q_proj_i32, _qproj_tid = matmul_fn(
...
q_proj_q = _make_q_proj_q(False)
q_proj_q_bypass = _make_q_proj_q(True)


def _make_q_proj_rope(cache_bypass: bool):
    qr_fn = q_proj_qr_bypass if cache_bypass else q_proj_qr
    q_fn = q_proj_q_bypass if cache_bypass else q_proj_q
    @pl.jit.inline(auto_scope=False)
    def q_proj_rope(
        x: pl.Tensor[[T_DYN, D], pl.BF16],
        wq_a: pl.Tensor[[D, Q_LORA], pl.BF16, pl.NZ],
        ...
        for tile_base in pl.range(0, t_dim, PREFILL_DENSE_TILE):
            ...
            with pl.scope():
                ...
                qr_fn(x_tile, wq_a, gamma_cq, qr_tile, qr_scale_tile, qr_i8_matmul, qr_scale_pad_store)
...
q_proj_rope = _make_q_proj_rope(False)
q_proj_rope_bypass = _make_q_proj_rope(True)


def _make_kv_proj_rope(cache_bypass: bool):
    if cache_bypass:
        @pl.jit.inline(auto_scope=False)
        def kv_proj_rope(
...
kv_proj_rope = _make_kv_proj_rope(False)
kv_proj_rope_bypass = _make_kv_proj_rope(True)


def _make_qkv_proj_rope(cache_bypass: bool):
    rope_fn = q_proj_rope_bypass if cache_bypass else q_proj_rope
    kv_fn = kv_proj_rope_bypass if cache_bypass else kv_proj_rope
    @pl.jit.inline(auto_scope=False)
    def qkv_proj_rope(
        x: pl.Tensor[[T_DYN, D], pl.BF16],
        wq_a: pl.Tensor[[D, Q_LORA], pl.BF16, pl.NZ],
...
qkv_proj_rope = _make_qkv_proj_rope(False)
qkv_proj_rope_bypass = _make_qkv_proj_rope(True)
@pl.jit
