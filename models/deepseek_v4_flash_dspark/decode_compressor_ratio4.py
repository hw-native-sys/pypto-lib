...
def compressor_ratio4_project(
    ...
    with pl.spmd(
        KV_SCORE_WORKERS, name_hint="kv_score_proj", allow_early_resolve=True, deps=[late_dep],
    ) as _kv_score_tid:
...
def compressor_ratio4_cache_write_only(
    ...
    cmp_kv_cache_rows = cmp_block_num * BLOCK_SIZE
    kv_flat = kv
    cmp_kv_cache_flat = pl.reshape(cmp_kv_cache, [cmp_kv_cache_rows, HEAD_DIM])

    normed_kv = pl.create_tensor([BS_PAD, HEAD_DIM], dtype=pl.FP32)
    norm_w_2d = pl.reshape(norm_w, [1, HEAD_DIM])
    with pl.spmd(
        rms_blocks, name_hint="rmsnorm_rope_cache_write", allow_early_resolve=True, deps=[pool_tid, late_write_dep]
    ) as cache_write_tid:
