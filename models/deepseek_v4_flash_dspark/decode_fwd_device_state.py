...
from dspark_device_state import (
    LOCAL_BATCH as DSPARK_STATE_LOCAL_BATCH,
    STATE_CAPACITY as DSPARK_STATE_CAPACITY,
...
def decode_fwd(
    ...
    state_tokens: pl.InOut[
        pl.Tensor[[DSPARK_STATE_CAPACITY, DSPARK_STATE_TOKEN_WIDTH], pl.INT64]
    ],
    state_meta: pl.InOut[
        pl.Tensor[[DSPARK_STATE_CAPACITY, DSPARK_STATE_META_WIDTH], pl.INT32]
    ],
    ...
    accepted_token_ids: pl.Out[
        pl.Tensor[[DSPARK_STATE_LOCAL_BATCH, SAMPLED_IDS_PAD], pl.INT32]
    ],
...
@pl.jit.host
def l3_decode_fwd_device_state(
    ...
    state_tokens: pl.InOut[
        pl.Tensor[
            [N_RANKS, DSPARK_STATE_CAPACITY, DSPARK_STATE_TOKEN_WIDTH],
            pl.INT64,
        ]
    ],
    state_meta: pl.InOut[
        pl.Tensor[
            [N_RANKS, DSPARK_STATE_CAPACITY, DSPARK_STATE_META_WIDTH],
            pl.INT32,
        ]
    ],
    ...
    accepted_token_ids: pl.Out[
        pl.Tensor[[N_RANKS, DSPARK_STATE_LOCAL_BATCH, SAMPLED_IDS_PAD], pl.INT32]
    ],
    accepted_counts: pl.Out[
        pl.Tensor[[N_RANKS, DSPARK_STATE_LOCAL_BATCH], pl.INT32]
    ],
    ...
    drafter_last_sampled: pl.Out[
        pl.Tensor[[N_RANKS, DSPARK_STATE_LOCAL_BATCH], pl.INT64]
    ],
    ...
    for rank in pl.range(pld.world_size()):
        ...
        decode_fwd(
            ...
            sampled_ids[rank],
            state_slot_ids[rank], state_generations[rank],
            state_tokens[rank], state_meta[rank],
            sampled_row_offsets[rank], hidden_row_offsets[rank],
            accepted_token_ids[rank], accepted_counts[rank],
            drafter_target_hidden[rank],
            drafter_context_positions[rank], drafter_context_valid[rank],
            drafter_last_sampled[rank], drafter_anchor_positions[rank],
...
def build_tensor_specs(
    ...
    *,
    ...
    state_shapes = {
        "state_slot_ids": [N_RANKS, DSPARK_STATE_LOCAL_BATCH],
        "state_generations": [N_RANKS, DSPARK_STATE_LOCAL_BATCH],
        "state_tokens": [
            N_RANKS,
            DSPARK_STATE_CAPACITY,
        ...
        "sampled_row_offsets": [N_RANKS, DSPARK_STATE_LOCAL_BATCH],
        "hidden_row_offsets": [N_RANKS, DSPARK_STATE_LOCAL_BATCH],
        "accepted_token_ids": [
            N_RANKS,
            DSPARK_STATE_LOCAL_BATCH,
            SAMPLED_IDS_PAD,
        ],
        "accepted_counts": [N_RANKS, DSPARK_STATE_LOCAL_BATCH],
        "drafter_target_hidden": [N_RANKS, local_t, MAIN_HIDDEN_DIM],
        "drafter_context_positions": [N_RANKS, local_t],
        "drafter_context_valid": [N_RANKS, local_t],
        "drafter_last_sampled": [N_RANKS, DSPARK_STATE_LOCAL_BATCH],
        "drafter_anchor_positions": [N_RANKS, DSPARK_STATE_LOCAL_BATCH],
        "drafter_row_offsets": [N_RANKS, DSPARK_STATE_LOCAL_BATCH],
    }
    state_dtypes = {
        "state_slot_ids": torch.int32,
        "state_generations": torch.int32,
        "state_tokens": torch.int64,
        "state_meta": torch.int32,
        "sampled_row_offsets": torch.int32,
        "hidden_row_offsets": torch.int32,
        "accepted_token_ids": torch.int32,
        "accepted_counts": torch.int32,
        "drafter_target_hidden": torch.bfloat16,
        "drafter_context_positions": torch.int32,
        "drafter_context_valid": torch.int32,
        "drafter_last_sampled": torch.int64,
        "drafter_anchor_positions": torch.int32,
        "drafter_row_offsets": torch.int32,
    }
    ...
    if local_requests > DSPARK_STATE_LOCAL_BATCH:
        raise ValueError(
            f"decode fixture needs {local_requests} local state rows, "
            f"capacity is {DSPARK_STATE_LOCAL_BATCH}"
        )
    ...
    parameter_names = list(inspect.signature(l3_decode_fwd_device_state._func).parameters)
