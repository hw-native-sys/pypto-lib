...
_LENGTH_MATRIX = [
    ...
    (M.max_position_embeddings - 1, 128, 8191, 64, 262143, 128),
...
def _selftest_direct_boundaries() -> None:
    ...
    for pos in [0, 3, 4, 127, 128, 129, 12287, 16383, M.max_position_embeddings - 2, M.max_position_embeddings - 1]:
        r = _full_capacity_request(M.max_position_embeddings)
...
def _selftest_over_capacity_rejected() -> None:
    ...
    try:
        RequestGeometry(
            committed_tokens=M.max_position_embeddings + 1,
            raw_allocated_rows=SWA_WINDOW_ROWS,
            raw_valid_begin=M.max_position_embeddings - SWA_WINDOW_ROWS,
            raw_valid_end=M.max_position_embeddings,
            hca_allocated_rows=MAX_HCA_ROWS,
            hca_valid_begin=0,
            hca_valid_end=MAX_HCA_ROWS,
            csa_allocated_candidates=MAX_CSA_CANDIDATES,
    ...
    # position >= M.max_position_embeddings must fail.
    r = _full_capacity_request(M.max_position_embeddings)
    for bad_pos in (M.max_position_embeddings, M.max_position_embeddings + 1):
        try:
...
def _selftest_inactive_lane() -> None:
    ...
    r = RequestGeometry(
        committed_tokens=M.max_position_embeddings,
        raw_allocated_rows=SWA_WINDOW_ROWS,
        raw_valid_begin=M.max_position_embeddings - SWA_WINDOW_ROWS,
        raw_valid_end=M.max_position_embeddings,
        hca_allocated_rows=MAX_HCA_ROWS,
    ...
    q = QueryGeometry(request=r, position=M.max_position_embeddings - 1)
    res = derive_query_geometry(q)
...
def _selftest_swa_ring_and_sources() -> None:
    ...
    raw_pages = (5, 9, 2, 7)
    raw_epochs = (9, 9, 9, 9)
    for length in (1, 127, 128, 129, 12288, 16384, M.max_position_embeddings):
...
def _selftest_hca_phase_c() -> None:
    ...
    assert hca_boundary_event(M.max_position_embeddings - 1) == MAX_HCA_ROWS - 1
    ...
    full = RequestGeometry(
        committed_tokens=M.max_position_embeddings,
        raw_allocated_rows=128,
        raw_valid_begin=M.max_position_embeddings - 128,
        raw_valid_end=M.max_position_embeddings,
        hca_allocated_rows=MAX_HCA_ROWS,
        hca_valid_begin=0,
        hca_valid_end=MAX_HCA_ROWS,
    ...
    full_work = build_hca_packed_work(
        [QueryGeometry(full, M.max_position_embeddings - 1)],
        [0],
        {},
    )
...
def _selftest_csa_phase_d() -> None:
    ...
    assert csa_next_state_valid_range(M.max_position_embeddings - 1) == (
        M.max_position_embeddings - 4,
        M.max_position_embeddings,
    )
    ...
    full = _full_capacity_request(M.max_position_embeddings)
    full_work = build_csa_packed_work(
        [QueryGeometry(full, M.max_position_embeddings - 1)], [0], {}
    )
...
if __name__ == "__main__":
    ...
    print(f"M.max_position_embeddings      = {M.max_position_embeddings}")
