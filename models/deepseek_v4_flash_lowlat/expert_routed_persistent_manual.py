...
# -----------------------------------------------------------------------------------------------------------
"""The single-kernel persistent routed expert, with the AIV split written BY HAND.

Same chain as ``expert_routed_persistent.py`` but in ONE ``pl.spmd`` task instead
of three phases. What makes that buildable is `pl.split_aiv`: the auto split
emits lane 0 doing the work and lane 1 *replaying* the body with every tile at a
static ``valid_shape=0``, and an INT8 cast's codegen bridges that 0 into a
``(0, 0)`` view pto-isa cannot compile. A data-parallel region has no replay
lane -- both lanes do real work on real row extents -- so the INT8 requant that
blocks the auto form compiles here.

Structure follows ``deepseek_v4_flash_mtp/expert_routed_tp.py``: cube ops sit
outside every region, each vector phase is its own ``UP_DOWN`` region, and the
two crossings are named -- ``pl.aiv_shard`` for cube->vector, ``pl.aic_gather``
for vector->cube.

    gate/up matmul          cube, outside
    region 1 (UP_DOWN)      dequant + SwiGLU + per-row A8 requant -> h_i8
    down matmul             cube, outside, on the gathered h
    region 2 (UP_DOWN)      per-row/per-channel dequant -> recv_y BF16
...
def expert_routed_persistent_manual(
...
@pl.jit
def expert_routed_persistent_manual_test(
    recv_x: pl.Tensor[[N_SLOTS, RECV_MAX, D], pl.INT8],
    recv_scale_dq: pl.Tensor[[N_SLOTS, RECV_MAX], pl.FP32],
    recv_weights: pl.Tensor[[N_SLOTS, RECV_MAX], pl.FP32],
    recv_expert_count: pl.Tensor[[N_SLOTS, IDX_PAD], pl.INT32],
    ...
    expert_routed_persistent_manual(
...
if __name__ == "__main__":
    ...
    result = run_jit(
        ...
        fn=expert_routed_persistent_manual_test,
