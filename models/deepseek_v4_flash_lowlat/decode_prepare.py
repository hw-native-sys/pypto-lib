...
HCA_STATE_MAX_BLOCKS = (M.max_position_embeddings + C128_COMPRESSOR_BLOCK_SIZE - 1) // C128_COMPRESSOR_BLOCK_SIZE
CSA_STATE_MAX_BLOCKS = (M.max_position_embeddings + C4A_COMPRESSOR_BLOCK_SIZE - 1) // C4A_COMPRESSOR_BLOCK_SIZE
CSA_INNER_STATE_MAX_BLOCKS = CSA_STATE_MAX_BLOCKS
...
def pack_x_hc_rows(
    ...
    for block in pl.spmd(SPMD_BLOCKS, name_hint="pack_x_hc_rows", allow_early_resolve=True):
        for work_idx in pl.range(block, T * (D // X_HC_HIDDEN_TILE), SPMD_BLOCKS):
            token_idx = work_idx // (D // X_HC_HIDDEN_TILE)
            hidden_offset = (work_idx % (D // X_HC_HIDDEN_TILE)) * X_HC_HIDDEN_TILE
            hidden_row = hidden_states[token_idx : token_idx + 1, hidden_offset : hidden_offset + X_HC_HIDDEN_TILE]
            hidden_chunk = pl.cast(hidden_row, target_type=pl.FP32)
            for hc_idx in pl.range(HC_MULT):
                x_hc_row = token_idx * HC_MULT + hc_idx
                x_hc_flat[x_hc_row : x_hc_row + 1, hidden_offset : hidden_offset + X_HC_HIDDEN_TILE] = hidden_chunk
    return x_hc
