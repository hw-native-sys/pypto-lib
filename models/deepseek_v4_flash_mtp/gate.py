...
# can fan tokens across ranks. decode_moe.py shrinks config.FLASH.n_routed_experts to
...
def make_gate(token_tile_rows=GATE_M_TILE, hidden_tile_cols=GATE_D_TILE):
    ...
    def _gate(
        ...
        active_gate_tiles = (active_tokens + TOKEN_TILE_ROWS - 1) // TOKEN_TILE_ROWS
        active_gate_tokens = active_gate_tiles * TOKEN_TILE_ROWS
        ...
        with pl.at(level=pl.Level.CORE_GROUP, name_hint="gate_pre_route"):
            ...
            if N_EXPERTS < SCORE_PAD:
                ...
                for pad_block in pl.range(padded_rows // TOKEN_TILE_ROWS):
                    pad_t0 = pad_block * TOKEN_TILE_ROWS
                    biased_scores_buf[pad_t0 : pad_t0 + TOKEN_TILE_ROWS, N_EXPERTS:SCORE_PAD] = pl.full(
                        [TOKEN_TILE_ROWS, SCORE_PAD - N_EXPERTS],
                        dtype=pl.FP32,
                        value=FP32_NEG_INF,
                    )

        # Gate matmul + post: x_norm @ gate_w.T → sqrt(softplus(logits)) (+bias).
        # Fan the matmul over expert columns so each block computes a [TOKEN_TILE_ROWS,
        ...
        for gb_idx in pl.spmd(active_gate_tiles * GATE_N_BLOCKS, name_hint="gate", allow_early_resolve=True):
            ...
            t1 = tg * TOKEN_TILE_ROWS
            n0 = nb * GATE_N_TILE
            gp_bias_row = pl.reshape(gate_bias[n0 : n0 + GATE_N_TILE], [1, GATE_N_TILE])
            gate_logits_tile = pl.create_tensor([TOKEN_TILE_ROWS, GATE_N_TILE], dtype=pl.FP32)
            for kb in pl.pipeline(0, D // HIDDEN_TILE_COLS, stage=2):
                gd_kd = kb * HIDDEN_TILE_COLS
                gd_x = xg_buf[t1 : t1 + TOKEN_TILE_ROWS, gd_kd : gd_kd + HIDDEN_TILE_COLS]
                gd_w = gate_w[n0 : n0 + GATE_N_TILE, gd_kd : gd_kd + HIDDEN_TILE_COLS]
            ...
            gate_logits_tile = pl.row_expand_mul(gate_logits_tile, inv_rms_buf[t1 : t1 + TOKEN_TILE_ROWS, 0:1])
            ...
            route_scores_buf[t1 : t1 + TOKEN_TILE_ROWS, n0 : n0 + GATE_N_TILE] = gp_score
            if layer_id >= N_HASH_LAYERS:
                gp_bias = pl.col_expand_mul(pl.full([TOKEN_TILE_ROWS, GATE_N_TILE], dtype=pl.FP32, value=1.0), gp_bias_row)
                gp_biased = pl.add(gp_score, gp_bias)
                biased_scores_buf[t1 : t1 + TOKEN_TILE_ROWS, n0 : n0 + GATE_N_TILE] = gp_biased
