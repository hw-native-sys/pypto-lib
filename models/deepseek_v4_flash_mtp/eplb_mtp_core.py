...
def build_tensor_specs(
    ...
    if spec_adapter is not None:
        base_specs = spec_adapter(base_specs)
...
def build_numeric_compare_fn(*, mapped_kv_cache: bool):
    """Build the numeric MTP contract without diluting active KV-cache rows."""
    from golden import mapped_pool_ratio_reldiff, ratio_reldiff

    kv_cache_compare = ratio_reldiff(diff_thd=0.01, pct_thd=0.05)
    if not mapped_kv_cache:
        return {
            "hidden_out": ratio_reldiff(diff_thd=0.02, pct_thd=0.10),
            "next_pre_hc_hidden": ratio_reldiff(diff_thd=0.02, pct_thd=0.05),
            "kv_cache": kv_cache_compare,
            "logits": ratio_reldiff(diff_thd=0.02, pct_thd=0.10),
        }

    from golden import rowwise_ratio_reldiff

    active_rows = N_RANKS * T
    return {
        "hidden_out": rowwise_ratio_reldiff(
            "hidden_out",
            row_shape=(N_RANKS, T),
            diff_thd=0.02,
            pct_thd=0.10,
            expected_active_rows=active_rows,
        ),
        "next_pre_hc_hidden": rowwise_ratio_reldiff(
            "next_pre_hc_hidden",
            row_shape=(N_RANKS, T),
            diff_thd=0.02,
            pct_thd=0.05,
        ...
        "kv_cache": mapped_pool_ratio_reldiff(
            ...
            diff_thd=0.01,
            pct_thd=0.05,
            expected_mapped_rows=active_rows,
            compare_mapped_rows_individually=True,
        ),
        "logits": rowwise_ratio_reldiff(
            "logits",
            row_shape=(N_RANKS, MAX_LOGIT_ROWS),
...
def main():
    ...
    description = "DeepSeek-V4 strict EPLB MTP-core benchmark."
    if stats_placement_enabled():
        description = "DeepSeek-V4 MTP stats-shaped route replay with offline expert-placement controls."
    ...
    if stats_placement_enabled() and args.finite_only:
        parser.error("--finite-only is not supported by the stats-placement entrypoint")
    ...
    compare_fn = build_numeric_compare_fn(
        mapped_kv_cache=stats_placement_enabled()
    )
    ...
    if stats_placement_enabled():
        validation_mode = "finite_only" if args.finite_only else "numeric_golden"
        reference = (
            "golden_finite_smoke"
            if args.finite_only
            else "golden_eplb_mtp_core"
        )
        print(
            f"[VALIDATION] mode={validation_mode} reference={reference}",
            flush=True,
        )
    ...
    if args.save_device_output is not None or args.compare_device_output is not None:
        ...
        captures = (
            ...
            DenseOutputCapture(
                logical_key="mtp.next_pre_hc_hidden",
                source_name="next_pre_hc_hidden",
                row_axes=2,
                expected_rows=N_RANKS * T,
                thresholds=semantic_thresholds,
            ),
            DenseOutputCapture(
                logical_key="mtp.logits",
                source_name="logits",
                row_axes=2,
                expected_rows=N_RANKS * T,
                thresholds=semantic_thresholds,
            ...
            MappedPoolCapture(
                logical_key="mtp.kv_cache",
                source_name="kv_cache",
                block_size=BLOCK_SIZE,
                layer_ids=(43,),
                mapping_names=("swa_slot_mapping",),
                expected_rows=N_RANKS * T,
    ...
    result = run(
        ...
        share_readonly_golden_inputs=(
            stats_placement_enabled()
            and not args.finite_only
            and args.golden_data is None
        ),
        compile_cfg=dict(
