    ...
    with pl.scope():
        for local_e in pl.parallel(N_LOCAL_EXPERTS):
            ...
            for tt in pl.parallel(e_tiles):
                ...
                # pl.at, not pl.spmd: the assert above pins this grid to a single
                # block, and only pl.at takes no_dep_args. Without it the fused
                # pushes make every exp_w2_act task a whole-buffer writer of
                # routed_y_buf and combine_arrived, and the tensormap serializes
                # the whole grid on a WAW chain it cannot disprove (deps.json
                # records these edges as "overlap": "covered"). Each push owns a
                # unique r_route row and the arrival counter is an AtomicAdd, so
                # neither edge is real.
                with pl.at(
                    level=pl.Level.CORE_GROUP,
                    name_hint="exp_w2_act",
                    allow_early_resolve=True,
                    no_dep_args=[routed_y_buf, combine_arrived],
                ):
                    act_d_base = 0
