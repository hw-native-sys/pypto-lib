...
from eplb_fixture import (
    COMPRESSED_CACHE_ACTIVE_WRITE_FLAG,
    EXPERT_PLACEMENT_CHOICES,
    EXPERT_PLACEMENT_FLAG,
    EPLB_COMPRESSED_WRITE_START_POS,
    EPLB_EP_SIZE,
    EPLB_START_POS,
    EPLB_TOKENS,
    EPLB_TP_SIZE,
    ...
    benchmark_experts_per_rank,
    configure_eplb_argv,
    make_eplb_input_ids_spec,
    make_eplb_tid2eid_spec,
    stats_placement_enabled,
    validate_eplb_topology,
)

configure_eplb_argv()
BENCHMARK_EXPERTS_PER_RANK = benchmark_experts_per_rank()
...
def _make_layer_stacked_spec(
    name,
    base_specs,
    layer_count=FWD_NUM_LAYERS,
    layer_spec_factory=None,
):
    import torch
    from golden import TensorSpec

    spec = base_specs[name]
    if layer_spec_factory is not None:
        override = layer_spec_factory(name, spec, layer_count)
...
def make_forward_metadata_tensors(
    ...
    def init_compressed_slot_mapping_single(
        compress_ratio,
        storage_block_size,
        max_blocks,
        physical_blocks,
    ...
    init_by_name = {
        ...
        "hca_cmp_slot_mapping": lambda: ranked(lambda: init_compressed_slot_mapping_single(HCA_COMPRESS_RATIO, HCA_CMP_STORAGE_BLOCK_SIZE, table_columns("cmp_block_table"), cmp_block_num)),
        "csa_cmp_slot_mapping": lambda: ranked(lambda: init_compressed_slot_mapping_single(CSA_COMPRESS_RATIO, CSA_CMP_STORAGE_BLOCK_SIZE, table_columns("cmp_block_table"), cmp_block_num)),
        "csa_idx_slot_mapping": lambda: ranked(lambda: init_compressed_slot_mapping_single(CSA_COMPRESS_RATIO, CSA_CMP_STORAGE_BLOCK_SIZE, table_columns("idx_block_table"), idx_block_num)),
...
def build_tensor_specs(
    ...
    for name in ordered_names:
        if name in metadata_specs:
            ...
            specs.append(
                _make_layer_stacked_spec(
                    name,
                    base_specs,
                    CSA_NUM_LAYERS,
                    layer_spec_factory=layer_spec_factory,
                )
            )
...
def main():
    description = "DeepSeek-V4 Flash host-prepared EPLB decode-to-logits benchmark."
    if stats_placement_enabled():
        description = "DeepSeek-V4 Flash stats-shaped route replay with offline expert-placement controls."
    ...
    parser.add_argument(
        "--experts-per-rank", type=int,
        default=BENCHMARK_EXPERTS_PER_RANK, choices=[BENCHMARK_EXPERTS_PER_RANK],
    )
    ...
    parser.add_argument(
        ...
        default="stats",
        help=(
            "physical expert layout for the EP8x32 matched workload (default: stats)"
            if stats_placement_enabled()
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "static placement manifest (default: checked-in decode profile)"
            if stats_placement_enabled()
            else argparse.SUPPRESS
        ),
    )
    ...
    parser.add_argument(
        ...
        default=False,
        help=(
            "validate active HCA/CSA compressed-cache writes at position 8191"
            if stats_placement_enabled()
            else argparse.SUPPRESS
        ),
    )
    ...
    parser.add_argument(
        ...
        default=False,
        help=(
            "save the generated stats-placement inputs and Golden outputs"
            if stats_placement_enabled()
            else argparse.SUPPRESS
        ),
    )
    parser.add_argument(
        ...
        default=None,
        help=(
            "reuse a prior stats-placement data/{in,out} snapshot"
            if stats_placement_enabled()
            else argparse.SUPPRESS
        ),
    )
    ...
    add_device_output_arguments(parser, visible=stats_placement_enabled())
    args = parser.parse_args()
    if args.compressed_cache_active_write and not stats_placement_enabled():
        parser.error(
            f"{COMPRESSED_CACHE_ACTIVE_WRITE_FLAG} requires {STATS_PLACEMENT_FLAG}"
        )
    effective_start_pos = (
        EPLB_COMPRESSED_WRITE_START_POS
        if args.compressed_cache_active_write
        else args.start_pos
    )
    validate_eplb_topology(
        ep_size=args.ep,
    ...
    if stats_placement_enabled():
        ...
        from stats_placement_fixture import DEFAULT_MANIFEST_PATH, stats_layer_spec_factory

        placement_manifest = args.placement_manifest or DEFAULT_MANIFEST_PATH
        layer_spec_factory = partial(
            stats_layer_spec_factory,
            placement=args.expert_placement,
            manifest_path=placement_manifest,
        )
    ...
    golden_fn = None
    compare_fn = {}
    share_readonly_golden_inputs = False
    if stats_placement_enabled():
        from eplb_decode_logits_golden import (
            build_eplb_decode_logits_compare_fn,
            golden_eplb_decode_logits,
        )

        golden_fn = golden_eplb_decode_logits
        compare_fn = build_eplb_decode_logits_compare_fn(
            compressed_cache_active_write=args.compressed_cache_active_write,
        )
        share_readonly_golden_inputs = args.golden_data is None
        print(
            "[VALIDATION] mode=numeric_golden "
            "reference=golden_eplb_decode_logits",
            flush=True,
        )
        print(
            "[VALIDATION SCENARIO] "
            f"compressed_cache_active_write={int(args.compressed_cache_active_write)} "
            f"start_pos={effective_start_pos}",
            flush=True,
        )

    device_output_capture = None
    if args.save_device_output is not None or args.compare_device_output is not None:
        if not stats_placement_enabled() or placement_manifest is None:
            parser.error("device-output A/B is only supported by the stats-placement entrypoint")
        ...
        semantic_thresholds = RowwiseThresholds(
            min_cosine=0.99,
            max_rel_l2=0.10,
            max_abs=10.0,
        )
        cache_thresholds = RowwiseThresholds(
            min_cosine=0.99,
            max_rel_l2=0.10,
            max_abs=1.0,
        )
        kv_mapping_names = tuple(
            "swa_slot_mapping"
            if ratio == 0
            else "ori_slot_mapping"
            for ratio in MODEL_CONFIG.compress_ratios[:FWD_NUM_LAYERS]
        )
        captures = (
            DenseOutputCapture(
                logical_key="decode.pre_hc_hidden_out",
                source_name="pre_hc_hidden_out",
                row_axes=2,
                expected_rows=N_RANKS * T,
                thresholds=semantic_thresholds,
            ),
            DenseOutputCapture(
                logical_key="decode.hidden_out",
            ...
            MappedPoolCapture(
                logical_key="decode.kv_cache",
                source_name="kv_cache",
                block_size=BLOCK_SIZE,
                layer_ids=tuple(range(FWD_NUM_LAYERS)),
                mapping_names=kv_mapping_names,
                expected_rows=N_RANKS * T * FWD_NUM_LAYERS,
