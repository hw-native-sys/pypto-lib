...
from prefill_sparse_attn import (
    ...
    prefill_sparse_attn_precomputed_rope,
    prefill_sparse_rope_tables,
)
...
def prefill_attention_csa(
    ...
    # Build the sparse-attention RoPE tables before the large CSA task graph.
    # Keeping this AIV phase away from the later merge/rotate residency window
    # avoids the timing-sensitive UB fault documented in prefill_sparse_attn.
    sparse_rope_cos_il = pl.create_tensor([T, ROPE_HEAD_DIM], dtype=pl.FP32)
    sparse_rope_sin_signed = pl.create_tensor([T, ROPE_HEAD_DIM], dtype=pl.FP32)
    prefill_sparse_rope_tables(
        rope_cos_t,
        rope_sin_t,
        sparse_rope_cos_il,
        sparse_rope_sin_signed,
    )
    ...
    prefill_sparse_attn_precomputed_rope(
        ...
        sparse_rope_cos_il, sparse_rope_sin_signed,
...
if __name__ == "__main__":
    ...
    x_out_diff_thd, x_out_max_diff = (8e-3, 2) if args.start_pos else (5e-3, 2)
