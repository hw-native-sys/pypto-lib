...
from decode_attention_csa import (
    ...
    IDX_MX_N_TILE as CSA_IDX_MX_N_TILE,
    IDX_WQB_SCALE_ROWS as CSA_IDX_WQB_SCALE_ROWS,
...
def decode_fwd(
    ...
    csa_idx_wq_b_scale: pl.Tensor[[CSA_NUM_LAYERS * CSA_IDX_WQB_SCALE_ROWS, CSA_IDX_MX_N_TILE], pl.FP8E8M0],
    ...
    wq_a_l0 = pl.slice(wq_a, [D, Q_LORA], [0, 0])
    wq_a_scale_l0 = pl.slice(wq_a_scale, [WQA_SCALE_ROWS, QKV_MX_N_TILE], [0, 0])
    wq_b_l0 = pl.slice(wq_b, [Q_LORA, H * HEAD_DIM], [0, 0])
    wq_b_scale_l0 = pl.slice(wq_b_scale, [WQB_SCALE_ROWS, QKV_MX_N_TILE], [0, 0])
    wkv_l0 = pl.slice(wkv, [D, HEAD_DIM], [0, 0])
    wkv_scale_l0 = pl.slice(wkv_scale, [WKV_SCALE_ROWS, QKV_MX_N_TILE], [0, 0])
    ...
    wo_a_l0 = pl.slice(wo_a, [O_GROUPS, O_GROUP_IN, O_LORA], [0, 0, 0])
    wo_a_scale_l0 = pl.slice(wo_a_scale, [WO_A_SCALE_ROWS, O_MX_N_TILE], [0, 0])
    wo_b_l0 = pl.slice(wo_b, [O_GROUPS * O_LORA, D], [0, 0])
    wo_b_scale_l0 = pl.slice(wo_b_scale, [WO_B_SCALE_ROWS, O_MX_N_TILE], [0, 0])
    ...
    routed_w1_l0 = pl.slice(routed_w1, [N_LOCAL * D, MOE_INTER], [0, 0])
    routed_w1_scale_l0 = pl.slice(routed_w1_scale, [ROUTED_W13_SCALE_ROWS, ROUTED_N_TILE], [0, 0])
    routed_w3_l0 = pl.slice(routed_w3, [N_LOCAL * D, MOE_INTER], [0, 0])
    routed_w3_scale_l0 = pl.slice(routed_w3_scale, [ROUTED_W13_SCALE_ROWS, ROUTED_N_TILE], [0, 0])
    routed_w2_l0 = pl.slice(routed_w2, [N_LOCAL * MOE_INTER, D], [0, 0])
    routed_w2_scale_l0 = pl.slice(routed_w2_scale, [ROUTED_W2_SCALE_ROWS, ROUTED_N_TILE], [0, 0])
    shared_w1_l0 = pl.slice(shared_w1, [D, MOE_INTER], [0, 0])
    shared_w1_scale_l0 = pl.slice(shared_w1_scale, [SHARED_W13_SCALE_ROWS, SHARED_N_TILE], [0, 0])
    shared_w3_l0 = pl.slice(shared_w3, [D, MOE_INTER], [0, 0])
    shared_w3_scale_l0 = pl.slice(shared_w3_scale, [SHARED_W13_SCALE_ROWS, SHARED_N_TILE], [0, 0])
    shared_w2_l0 = pl.slice(shared_w2, [MOE_INTER, D], [0, 0])
    shared_w2_scale_l0 = pl.slice(shared_w2_scale, [SHARED_W2_SCALE_ROWS, SHARED_N_TILE], [0, 0])
    ...
    wq_a_l1 = pl.slice(wq_a, [D, Q_LORA], [D, 0])
    wq_a_scale_l1 = pl.slice(wq_a_scale, [WQA_SCALE_ROWS, QKV_MX_N_TILE], [WQA_SCALE_ROWS, 0])
    wq_b_l1 = pl.slice(wq_b, [Q_LORA, H * HEAD_DIM], [Q_LORA, 0])
    wq_b_scale_l1 = pl.slice(wq_b_scale, [WQB_SCALE_ROWS, QKV_MX_N_TILE], [WQB_SCALE_ROWS, 0])
    wkv_l1 = pl.slice(wkv, [D, HEAD_DIM], [D, 0])
    wkv_scale_l1 = pl.slice(wkv_scale, [WKV_SCALE_ROWS, QKV_MX_N_TILE], [WKV_SCALE_ROWS, 0])
    ...
    wo_a_l1 = pl.slice(wo_a, [O_GROUPS, O_GROUP_IN, O_LORA], [O_GROUPS, 0, 0])
    wo_a_scale_l1 = pl.slice(wo_a_scale, [WO_A_SCALE_ROWS, O_MX_N_TILE], [WO_A_SCALE_ROWS, 0])
    wo_b_l1 = pl.slice(wo_b, [O_GROUPS * O_LORA, D], [O_GROUPS * O_LORA, 0])
    wo_b_scale_l1 = pl.slice(wo_b_scale, [WO_B_SCALE_ROWS, O_MX_N_TILE], [WO_B_SCALE_ROWS, 0])
    ...
    routed_w1_l1 = pl.slice(routed_w1, [N_LOCAL * D, MOE_INTER], [N_LOCAL * D, 0])
    routed_w1_scale_l1 = pl.slice(routed_w1_scale, [ROUTED_W13_SCALE_ROWS, ROUTED_N_TILE], [ROUTED_W13_SCALE_ROWS, 0])
    routed_w3_l1 = pl.slice(routed_w3, [N_LOCAL * D, MOE_INTER], [N_LOCAL * D, 0])
    routed_w3_scale_l1 = pl.slice(routed_w3_scale, [ROUTED_W13_SCALE_ROWS, ROUTED_N_TILE], [ROUTED_W13_SCALE_ROWS, 0])
    routed_w2_l1 = pl.slice(routed_w2, [N_LOCAL * MOE_INTER, D], [N_LOCAL * MOE_INTER, 0])
    routed_w2_scale_l1 = pl.slice(routed_w2_scale, [ROUTED_W2_SCALE_ROWS, ROUTED_N_TILE], [ROUTED_W2_SCALE_ROWS, 0])
    shared_w1_l1 = pl.slice(shared_w1, [D, MOE_INTER], [D, 0])
    shared_w1_scale_l1 = pl.slice(shared_w1_scale, [SHARED_W13_SCALE_ROWS, SHARED_N_TILE], [SHARED_W13_SCALE_ROWS, 0])
    shared_w3_l1 = pl.slice(shared_w3, [D, MOE_INTER], [D, 0])
    shared_w3_scale_l1 = pl.slice(shared_w3_scale, [SHARED_W13_SCALE_ROWS, SHARED_N_TILE], [SHARED_W13_SCALE_ROWS, 0])
    shared_w2_l1 = pl.slice(shared_w2, [MOE_INTER, D], [MOE_INTER, 0])
    shared_w2_scale_l1 = pl.slice(shared_w2_scale, [SHARED_W2_SCALE_ROWS, SHARED_N_TILE], [SHARED_W2_SCALE_ROWS, 0])
    ...
    with pl.scope():
        if LEAD_COMPRESS_RATIO == SWA_COMPRESS_RATIO:
            attention_swa(
                ...
                wkv_l0, wkv_scale_l0, gamma_cq_l0, gamma_ckv_l0, freqs_cos, freqs_sin,
                kv_cache_l0,
    ...
    with pl.scope():
        if LEAD_COMPRESS_RATIO == SWA_COMPRESS_RATIO:
            attention_swa(
                ...
                wkv_l1, wkv_scale_l1, gamma_cq_l1, gamma_ckv_l1, freqs_cos, freqs_sin,
                kv_cache_l1,
                swa_slot_mapping, swa_indices, swa_lens, position_ids,
    ...
    for loop_i in pl.range(PAIR_LOOP_COUNT):
        ...
        wq_a_scale_csa = pl.slice(wq_a_scale, [WQA_SCALE_ROWS, QKV_MX_N_TILE], [csa_layer * WQA_SCALE_ROWS, 0])
        wq_b_csa = pl.slice(wq_b, [Q_LORA, H * HEAD_DIM], [csa_layer * Q_LORA, 0])
        wq_b_scale_csa = pl.slice(wq_b_scale, [WQB_SCALE_ROWS, QKV_MX_N_TILE], [csa_layer * WQB_SCALE_ROWS, 0])
        wkv_csa = pl.slice(wkv, [D, HEAD_DIM], [csa_layer * D, 0])
        wkv_scale_csa = pl.slice(wkv_scale, [WKV_SCALE_ROWS, QKV_MX_N_TILE], [csa_layer * WKV_SCALE_ROWS, 0])
        ...
        wo_a_csa = pl.slice(wo_a, [O_GROUPS, O_GROUP_IN, O_LORA], [csa_layer * O_GROUPS, 0, 0])
        wo_a_scale_csa = pl.slice(wo_a_scale, [WO_A_SCALE_ROWS, O_MX_N_TILE], [csa_layer * WO_A_SCALE_ROWS, 0])
        wo_b_csa = pl.slice(wo_b, [O_GROUPS * O_LORA, D], [csa_layer * O_GROUPS * O_LORA, 0])
        wo_b_scale_csa = pl.slice(wo_b_scale, [WO_B_SCALE_ROWS, O_MX_N_TILE], [csa_layer * WO_B_SCALE_ROWS, 0])
        ...
        csa_idx_wq_b_scale_csa = pl.slice(csa_idx_wq_b_scale, [CSA_IDX_WQB_SCALE_ROWS, CSA_IDX_MX_N_TILE], [loop_i * CSA_IDX_WQB_SCALE_ROWS, 0])
        ...
        routed_w1_csa = pl.slice(routed_w1, [N_LOCAL * D, MOE_INTER], [csa_layer * N_LOCAL * D, 0])
        routed_w1_scale_csa = pl.slice(routed_w1_scale, [ROUTED_W13_SCALE_ROWS, ROUTED_N_TILE], [csa_layer * ROUTED_W13_SCALE_ROWS, 0])
        routed_w3_csa = pl.slice(routed_w3, [N_LOCAL * D, MOE_INTER], [csa_layer * N_LOCAL * D, 0])
        routed_w3_scale_csa = pl.slice(routed_w3_scale, [ROUTED_W13_SCALE_ROWS, ROUTED_N_TILE], [csa_layer * ROUTED_W13_SCALE_ROWS, 0])
        routed_w2_csa = pl.slice(routed_w2, [N_LOCAL * MOE_INTER, D], [csa_layer * N_LOCAL * MOE_INTER, 0])
        routed_w2_scale_csa = pl.slice(routed_w2_scale, [ROUTED_W2_SCALE_ROWS, ROUTED_N_TILE], [csa_layer * ROUTED_W2_SCALE_ROWS, 0])
        shared_w1_csa = pl.slice(shared_w1, [D, MOE_INTER], [csa_layer * D, 0])
        shared_w1_scale_csa = pl.slice(shared_w1_scale, [SHARED_W13_SCALE_ROWS, SHARED_N_TILE], [csa_layer * SHARED_W13_SCALE_ROWS, 0])
        shared_w3_csa = pl.slice(shared_w3, [D, MOE_INTER], [csa_layer * D, 0])
        shared_w3_scale_csa = pl.slice(shared_w3_scale, [SHARED_W13_SCALE_ROWS, SHARED_N_TILE], [csa_layer * SHARED_W13_SCALE_ROWS, 0])
        shared_w2_csa = pl.slice(shared_w2, [MOE_INTER, D], [csa_layer * MOE_INTER, 0])
        shared_w2_scale_csa = pl.slice(shared_w2_scale, [SHARED_W2_SCALE_ROWS, SHARED_N_TILE], [csa_layer * SHARED_W2_SCALE_ROWS, 0])
        with pl.scope():
            attention_csa(
                hidden,
                hc_attn_fn_csa, hc_attn_scale_csa, hc_attn_base_csa,
                attn_norm_w_csa, wq_a_csa, wq_a_scale_csa, wq_b_csa, wq_b_scale_csa,
                wkv_csa, wkv_scale_csa, gamma_cq_csa, gamma_ckv_csa, freqs_cos, freqs_sin,
        ...
        wq_a_scale_hca = pl.slice(wq_a_scale, [WQA_SCALE_ROWS, QKV_MX_N_TILE], [hca_layer * WQA_SCALE_ROWS, 0])
        wq_b_hca = pl.slice(wq_b, [Q_LORA, H * HEAD_DIM], [hca_layer * Q_LORA, 0])
        wq_b_scale_hca = pl.slice(wq_b_scale, [WQB_SCALE_ROWS, QKV_MX_N_TILE], [hca_layer * WQB_SCALE_ROWS, 0])
        wkv_hca = pl.slice(wkv, [D, HEAD_DIM], [hca_layer * D, 0])
        wkv_scale_hca = pl.slice(wkv_scale, [WKV_SCALE_ROWS, QKV_MX_N_TILE], [hca_layer * WKV_SCALE_ROWS, 0])
        ...
        wo_a_hca = pl.slice(wo_a, [O_GROUPS, O_GROUP_IN, O_LORA], [hca_layer * O_GROUPS, 0, 0])
        wo_a_scale_hca = pl.slice(wo_a_scale, [WO_A_SCALE_ROWS, O_MX_N_TILE], [hca_layer * WO_A_SCALE_ROWS, 0])
        wo_b_hca = pl.slice(wo_b, [O_GROUPS * O_LORA, D], [hca_layer * O_GROUPS * O_LORA, 0])
        wo_b_scale_hca = pl.slice(wo_b_scale, [WO_B_SCALE_ROWS, O_MX_N_TILE], [hca_layer * WO_B_SCALE_ROWS, 0])
        ...
        routed_w1_hca = pl.slice(routed_w1, [N_LOCAL * D, MOE_INTER], [hca_layer * N_LOCAL * D, 0])
        routed_w1_scale_hca = pl.slice(routed_w1_scale, [ROUTED_W13_SCALE_ROWS, ROUTED_N_TILE], [hca_layer * ROUTED_W13_SCALE_ROWS, 0])
        routed_w3_hca = pl.slice(routed_w3, [N_LOCAL * D, MOE_INTER], [hca_layer * N_LOCAL * D, 0])
        routed_w3_scale_hca = pl.slice(routed_w3_scale, [ROUTED_W13_SCALE_ROWS, ROUTED_N_TILE], [hca_layer * ROUTED_W13_SCALE_ROWS, 0])
        routed_w2_hca = pl.slice(routed_w2, [N_LOCAL * MOE_INTER, D], [hca_layer * N_LOCAL * MOE_INTER, 0])
        routed_w2_scale_hca = pl.slice(routed_w2_scale, [ROUTED_W2_SCALE_ROWS, ROUTED_N_TILE], [hca_layer * ROUTED_W2_SCALE_ROWS, 0])
        shared_w1_hca = pl.slice(shared_w1, [D, MOE_INTER], [hca_layer * D, 0])
        shared_w1_scale_hca = pl.slice(shared_w1_scale, [SHARED_W13_SCALE_ROWS, SHARED_N_TILE], [hca_layer * SHARED_W13_SCALE_ROWS, 0])
        shared_w3_hca = pl.slice(shared_w3, [D, MOE_INTER], [hca_layer * D, 0])
        shared_w3_scale_hca = pl.slice(shared_w3_scale, [SHARED_W13_SCALE_ROWS, SHARED_N_TILE], [hca_layer * SHARED_W13_SCALE_ROWS, 0])
        shared_w2_hca = pl.slice(shared_w2, [MOE_INTER, D], [hca_layer * MOE_INTER, 0])
        shared_w2_scale_hca = pl.slice(shared_w2_scale, [SHARED_W2_SCALE_ROWS, SHARED_N_TILE], [hca_layer * SHARED_W2_SCALE_ROWS, 0])
        with pl.scope():
            attention_hca(
                hidden_mid,
                hc_attn_fn_hca, hc_attn_scale_hca, hc_attn_base_hca,
                attn_norm_w_hca, wq_a_hca, wq_a_scale_hca, wq_b_hca, wq_b_scale_hca,
                wkv_hca, wkv_scale_hca, gamma_cq_hca, gamma_ckv_hca, freqs_cos, freqs_sin,
    ...
    wq_a_scale_last = pl.slice(wq_a_scale, [WQA_SCALE_ROWS, QKV_MX_N_TILE], [csa_layer_last * WQA_SCALE_ROWS, 0])
    wq_b_last = pl.slice(wq_b, [Q_LORA, H * HEAD_DIM], [csa_layer_last * Q_LORA, 0])
    wq_b_scale_last = pl.slice(wq_b_scale, [WQB_SCALE_ROWS, QKV_MX_N_TILE], [csa_layer_last * WQB_SCALE_ROWS, 0])
    wkv_last = pl.slice(wkv, [D, HEAD_DIM], [csa_layer_last * D, 0])
    wkv_scale_last = pl.slice(wkv_scale, [WKV_SCALE_ROWS, QKV_MX_N_TILE], [csa_layer_last * WKV_SCALE_ROWS, 0])
    ...
    wo_a_last = pl.slice(wo_a, [O_GROUPS, O_GROUP_IN, O_LORA], [csa_layer_last * O_GROUPS, 0, 0])
    wo_a_scale_last = pl.slice(wo_a_scale, [WO_A_SCALE_ROWS, O_MX_N_TILE], [csa_layer_last * WO_A_SCALE_ROWS, 0])
    wo_b_last = pl.slice(wo_b, [O_GROUPS * O_LORA, D], [csa_layer_last * O_GROUPS * O_LORA, 0])
    wo_b_scale_last = pl.slice(wo_b_scale, [WO_B_SCALE_ROWS, O_MX_N_TILE], [csa_layer_last * WO_B_SCALE_ROWS, 0])
    ...
    csa_idx_wq_b_scale_last = pl.slice(csa_idx_wq_b_scale, [CSA_IDX_WQB_SCALE_ROWS, CSA_IDX_MX_N_TILE], [(CSA_NUM_LAYERS - 1) * CSA_IDX_WQB_SCALE_ROWS, 0])
    ...
    routed_w1_last = pl.slice(routed_w1, [N_LOCAL * D, MOE_INTER], [csa_layer_last * N_LOCAL * D, 0])
    routed_w1_scale_last = pl.slice(routed_w1_scale, [ROUTED_W13_SCALE_ROWS, ROUTED_N_TILE], [csa_layer_last * ROUTED_W13_SCALE_ROWS, 0])
    routed_w3_last = pl.slice(routed_w3, [N_LOCAL * D, MOE_INTER], [csa_layer_last * N_LOCAL * D, 0])
    routed_w3_scale_last = pl.slice(routed_w3_scale, [ROUTED_W13_SCALE_ROWS, ROUTED_N_TILE], [csa_layer_last * ROUTED_W13_SCALE_ROWS, 0])
    routed_w2_last = pl.slice(routed_w2, [N_LOCAL * MOE_INTER, D], [csa_layer_last * N_LOCAL * MOE_INTER, 0])
    routed_w2_scale_last = pl.slice(routed_w2_scale, [ROUTED_W2_SCALE_ROWS, ROUTED_N_TILE], [csa_layer_last * ROUTED_W2_SCALE_ROWS, 0])
    shared_w1_last = pl.slice(shared_w1, [D, MOE_INTER], [csa_layer_last * D, 0])
    shared_w1_scale_last = pl.slice(shared_w1_scale, [SHARED_W13_SCALE_ROWS, SHARED_N_TILE], [csa_layer_last * SHARED_W13_SCALE_ROWS, 0])
    shared_w3_last = pl.slice(shared_w3, [D, MOE_INTER], [csa_layer_last * D, 0])
    shared_w3_scale_last = pl.slice(shared_w3_scale, [SHARED_W13_SCALE_ROWS, SHARED_N_TILE], [csa_layer_last * SHARED_W13_SCALE_ROWS, 0])
    shared_w2_last = pl.slice(shared_w2, [MOE_INTER, D], [csa_layer_last * MOE_INTER, 0])
    shared_w2_scale_last = pl.slice(shared_w2_scale, [SHARED_W2_SCALE_ROWS, SHARED_N_TILE], [csa_layer_last * SHARED_W2_SCALE_ROWS, 0])
    with pl.scope():
        attention_csa(
            hidden,
            hc_attn_fn_last, hc_attn_scale_last, hc_attn_base_last,
            attn_norm_w_last, wq_a_last, wq_a_scale_last, wq_b_last, wq_b_scale_last,
            wkv_last, wkv_scale_last, gamma_cq_last, gamma_ckv_last, freqs_cos, freqs_sin,
...
def l3_decode_fwd(
    ...
    csa_idx_wq_b_scale: pl.Tensor[[N_RANKS, CSA_NUM_LAYERS * CSA_IDX_WQB_SCALE_ROWS, CSA_IDX_MX_N_TILE], pl.FP8E8M0],
    ...
    def init_value():
        if name == "tid2eid":
            ...
            for layer in range(layer_count):
                ...
                layer_eids = (linear % N_RANKS) * N_LOCAL + linear // N_RANKS
        ...
        # This is a full-forward smoke fixture, so sharing one initialized
        # weight across layers preserves the packed ABI while avoiding the
        # 2x host-memory peak of materializing 61 tensors before torch.cat.
        one_layer = base_init()
        reps = [1, layer_count] + [1] * (one_layer.dim() - 2)
        return one_layer.repeat(*reps)
