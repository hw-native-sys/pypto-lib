...
@pl.jit
def decode_c2a_reuse_rank_sharded(
    x_hc: pl.Tensor[[OUTPUT_T_DYN, C.HC_MULT, C.D], pl.FP32],
    incoming_pre_mix: pl.Tensor[[OUTPUT_T_DYN, C.HC_MULT], pl.FP32],
    hc_attn_fn: pl.Tensor[[C.MIX_HC, C.HC_DIM], pl.FP32],
    hc_attn_scale: pl.Tensor[[3], pl.FP32],
    ...
    """Dispatch one production C2A Reuse composition to a device-selected JIT entry."""
    return decode_c2a_reuse_sharded(
        x_hc,
        incoming_pre_mix,
        hc_attn_fn,
        hc_attn_scale,
...
def build_specs_sharded(args, initial_state=None):
...
def make_golden_sharded(epochs):
    def golden_half(tensors):
        for _ in range(epochs):
            ...
            reuse.golden_c2a_reuse(leaf, sharded=True)
            common.golden_attention_post(tensors, post, residual)
...
def comparisons_sharded(initial_state=None):
    ...
    compare.update(
        {
            "attention_output": common.make_compare_sharded_rows(reuse.compare_output),
