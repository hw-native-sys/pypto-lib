...
The attention operator stays in ``decode_c1a_attn_full.py``; this entry consumes the staggered
...
from models.deepseek_v4_1_flash.decode_c1a_attn_full import (
    ...
    decode_c1a_attn_full,
    exact_bytes,
    golden_decode_c1a_attn_full,
...
def decode_c1a_full_test(
    ...
    return decode_c1a_full(
        ...
        output_window, output_arrived, output, hc_attn_pre_mix_next, group_base, tp_rank, num_tokens,
        attention_epoch,
    )
        ...
        for epoch in pl.range(1, EPOCHS + 1):
            for rank in pl.unroll(TP_SIZE):
                ...
                decode_c1a_full_test(
                    ...
                    index_wq_b_scale[rank], index_weights_proj[rank], topk_indices[rank], candidate_mask[rank],
                    output_window, output_arrived, output[rank], hc_attn_pre_mix_next[rank], 0, rank, TOKENS,
                    epoch, device=rank,
                )
...
def build_hc_validation_values(mode, tokens, pages, seed=17, case="random"):
    ...
    values["hc_attn_pre_mix"] = torch.sigmoid(torch.randn(TP_SIZE, tokens, HC_MULT, generator=generator))
    ...
    values["hc_attn_pre_mix_next"] = torch.zeros(TP_SIZE, tokens, HC_MULT, dtype=torch.float32)
    return values
...
def hc_pre_mix_compare(actual, expected, **kwargs):
    ...
    print(
        f"[PRECISION] hc_attn_pre_mix_next rel_l2={error.item():.6g} "
...
def hc_topk_indices_compare(mode):
    ...
    def compare(actual, expected, **kwargs):
        ...
        if inputs is not None and "x" not in inputs:
            hidden = [
                golden_mhc_pre(inputs["x_hc"][rank], inputs["hc_attn_pre_mix"][rank])
...
def golden_c1a_hc_case(tensors, attn_golden, epochs=1):
    ...
    for _ in range(epochs):
        ...
        for rank in range(TP_SIZE):
            ...
            pre_mix_next, post_mix, residual_mix = golden_mhc_mixes(
                x_hc, tensors["hc_attn_fn"][rank], tensors["hc_attn_scale"][rank], tensors["hc_attn_base"][rank]
            )
            tensors["hc_attn_pre_mix_next"][rank].copy_(pre_mix_next)
            hidden = golden_mhc_pre(x_hc, tensors["hc_attn_pre_mix"][rank])
...
def golden_decode_c1a_full_case(tensors, epochs=1):
    ...
    golden_c1a_hc_case(tensors, golden_decode_c1a_attn_full, epochs)
...
def run_c1a_hc(mode, kernel_factory, golden_case):
    ...
    comparisons = {
        ...
        "hc_attn_pre_mix_next": hc_pre_mix_compare,
