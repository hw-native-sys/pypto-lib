...
def decode_layer_tq(
    ...
    # Per-batch: TQ inline KV quantization (via turboquant_kv_prefill) + Q RoPE.
    for b in pl.parallel(user_batch):
        ...
        # Allocate GM buffers for turboquant_kv_prefill.
        # Rows 1-15 are uninitialized; turboquant_kv_prefill processes each row
        ...
        # Q RoPE + pad into all_q_padded (inside pl.at for col_expand_mul etc.)
            ...
            for sb in pl.range(ctx_blocks):
                ...
                with pl.at(level=pl.Level.CORE_GROUP, name_hint="qk_dequant"):
                    for k_sub in pl.range(BLOCK_SIZE // CMP_CHUNK):
                        ...
                        k_dec = pl.gather(tq_codebook, dim=-1, index=k_idx_i32)
                        temp_fp32_cache = pl.assemble(
                            temp_fp32_cache, k_dec, [k_sub * CMP_CHUNK, 0],
                        )
                ...
                with pl.at(level=pl.Level.CORE_GROUP, name_hint="qk_renorm"):
                    ...
                    for k_sub in pl.range(BLOCK_SIZE // CMP_CHUNK):
                        ...
                        k_sub_sq = pl.reshape(
                            pl.row_sum(pl.mul(k_sub_tile, k_sub_tile)), [CMP_CHUNK, 1],
                        )
                        k_sub_tile = pl.row_expand_mul(
                            k_sub_tile, pl.rsqrt(pl.add(k_sub_sq, EPS)),
                        )
                        k_scales_sub = pl.slice(k_scales_full, [CMP_CHUNK, 1], [sub_row, 0])
                        k_sub_tile = pl.row_expand_mul(k_sub_tile, k_scales_sub)
                        temp_fp32_cache = pl.assemble(
                            temp_fp32_cache, k_sub_tile, [sub_row, 0],
                        )

                # 2.2d2: Unrotate K from compressed space back to original space.
                with pl.at(level=pl.Level.CORE_GROUP, name_hint="qk_unrotate"):
                    for k_sub in pl.range(BLOCK_SIZE // CMP_CHUNK):
                        sub_row = k_sub * CMP_CHUNK
                        k_sub_tile = pl.slice(temp_fp32_cache, [CMP_CHUNK, HEAD_DIM], [sub_row, 0])
                        k_sub_bf16 = pl.cast(k_sub_tile, target_type=pl.BF16)
                        k_sub_unrot = pl.matmul(k_sub_bf16, rot_slice, b_trans=True, out_dtype=pl.FP32)
                        temp_fp32_cache = pl.assemble(temp_fp32_cache, k_sub_unrot, [sub_row, 0])
                # Copy dequant K from FP32 GM → BF16 GM (separate scope, mirrors prefill fix).
                with pl.at(level=pl.Level.CORE_GROUP, name_hint="k_to_bf16"):
                    for ks in pl.range(BLOCK_SIZE // CMP_CHUNK):
                        k_sub_bf16 = pl.cast(
                            pl.slice(temp_fp32_cache, [CMP_CHUNK, HEAD_DIM], [ks * CMP_CHUNK, 0]),
                            target_type=pl.BF16,
                        )
                        k_bf16_buf = pl.assemble(
                            k_bf16_buf, k_sub_bf16, [ks * CMP_CHUNK, 0],
                        )
            ...
            for sb in pl.range(ctx_blocks):
                ...
                with pl.at(level=pl.Level.CORE_GROUP, name_hint="sv_dequant"):
                    for v_sub in pl.range(BLOCK_SIZE // CMP_CHUNK):
                        ...
                        v_dec = pl.gather(tq_codebook, dim=-1, index=v_idx_i32)
                        # Renormalize to unit sphere (correct quantization norm drift).
                        v_dec_sq_sum = pl.reshape(pl.row_sum(pl.mul(v_dec, v_dec)), [CMP_CHUNK, 1])
                        v_dec_unit = pl.row_expand_mul(v_dec, pl.rsqrt(pl.add(v_dec_sq_sum, EPS)))
                        v_scales = pl.slice(quant_v_scales, [CMP_CHUNK, 1], [v_sub_row, 0])
                        v_dec_scaled = pl.row_expand_mul(v_dec_unit, v_scales)
                        v_tile_full = pl.assemble(
                            v_tile_full, pl.cast(v_dec_scaled, target_type=pl.BF16),
                            [v_sub * CMP_CHUNK, 0],
                        )
                # Unrotate V from compressed space back to original space.
                with pl.at(level=pl.Level.CORE_GROUP, name_hint="sv_unrotate"):
                    for v_sub in pl.range(BLOCK_SIZE // CMP_CHUNK):
                        sub_row = v_sub * CMP_CHUNK
                        v_sub_tile = pl.slice(v_tile_full, [CMP_CHUNK, HEAD_DIM], [sub_row, 0])
                        v_sub_unrot = pl.matmul(v_sub_tile, rot_slice, b_trans=True, out_dtype=pl.FP32)
                        v_sub_bf16 = pl.cast(v_sub_unrot, target_type=pl.BF16)
                        v_tile_full = pl.assemble(v_tile_full, v_sub_bf16, [sub_row, 0])
...
def build_tensor_specs(
    ...
    from lloyd_max import solve_lloyd_max as _solve_lm
