        ...
        for p0_idx in pl.range(tok_blocks):
            ...
            for ti in pl.range(valid_tok):
                ...
                for gi in pl.range(TOTAL_Q_GROUPS):
                    ...
                    # K dequant scratch (FP32) + BF16 GM buffer (mirrors FP prefill's k_cache).
                    temp_fp32_cache = pl.create_tensor([SEQ_TILE, HEAD_DIM], dtype=pl.FP32)
                    ...
                    for sb in pl.range(ctx_blocks):
                        ...
                        # Gather: UINT8 indices -> FP32 centroids.
                        ...
                        # Renormalize to unit sphere + rescale by stored norms.
                        ...
                        # Unrotate K back to original space.
                        with pl.at(level=pl.Level.CORE_GROUP, name_hint="qk_unrotate"):
                            for k_sub in pl.range(SEQ_TILE // CMP_CHUNK):
                                sub_row = k_sub * CMP_CHUNK
                                k_sub_tile = pl.slice(temp_fp32_cache, [CMP_CHUNK, HEAD_DIM], [sub_row, 0])
                                k_sub_bf16 = pl.cast(k_sub_tile, target_type=pl.BF16)
                                k_sub_unrot = pl.matmul(k_sub_bf16, rot_matrix, b_trans=True, out_dtype=pl.FP32)
                                temp_fp32_cache = pl.assemble(temp_fp32_cache, k_sub_unrot, [sub_row, 0])

                        # Copy dequant K from FP32 GM → BF16 GM (separate scope, like FP's rope_kv_cache).
                        with pl.at(level=pl.Level.CORE_GROUP, name_hint="k_to_bf16"):
                            for ks in pl.range(SEQ_TILE // CMP_CHUNK):
                                k_sub_bf16 = pl.cast(
                                    pl.slice(temp_fp32_cache, [CMP_CHUNK, HEAD_DIM],
                                             [ks * CMP_CHUNK, 0]),
                                    target_type=pl.BF16,
                                )
                    ...
                    for sb in pl.range(ctx_blocks):
                        ...
                        with pl.at(level=pl.Level.CORE_GROUP, name_hint="sv_dequant"):
                            for v_sub in pl.range(SEQ_TILE // CMP_CHUNK):
                                ...
                                v_dec_sq_sum = pl.reshape(
                                    pl.row_sum(pl.mul(v_dec, v_dec)), [CMP_CHUNK, 1],
                                )
                                v_dec_unit = pl.row_expand_mul(
                                    v_dec, pl.rsqrt(pl.add(v_dec_sq_sum, EPS)),
                                )
                                v_scales_sub = pl.slice(quant_v_scales, [CMP_CHUNK, 1], [v_sub_row, 0])
                                v_dec_scaled = pl.row_expand_mul(v_dec_unit, v_scales_sub)
                        ...
                        with pl.at(level=pl.Level.CORE_GROUP, name_hint="sv_unrotate"):
                            for v_sub in pl.range(SEQ_TILE // CMP_CHUNK):
                                ...
                                v_sub_unrot = pl.matmul(v_sub_tile, rot_matrix, b_trans=True, out_dtype=pl.FP32)
...
def golden_qwen3_14b_prefill_tq(tensors):
    ...
    def tq_quantize(x, rot_matrix_f):
        ...
        Mirrors the kernel's quant path (turboquant_kv.turboquant_kv_prefill
