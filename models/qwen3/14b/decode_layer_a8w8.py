...
QKV_OK = 1  # single split-K path for current PyPTO codegen
...
assert TM == BATCH and N_SUB == 2 and QKV_K_CHUNKS == 10
...
def _decode_layer(  # noqa: PLR0913 — model signature is intrinsic
    ...
    with pl.at(level=pl.Level.CORE_GROUP, name_hint="act_quant"):
        ...
        for act_quant_row in pl.range(BATCH):
            ...
            for act_quant_amax_kb in pl.range(HIDDEN // RMSNORM_K_CHUNK):
                ...
                act_quant_amax_bf16 = pl.cast(
                    pl.slice(normed_states, [1, RMSNORM_K_CHUNK], [act_quant_row, act_quant_amax_k0]),
                    target_type=pl.BF16,
                )
            ...
            for act_quant_write_kb in pl.range(HIDDEN // RMSNORM_K_CHUNK):
                ...
                act_quant_write_bf16 = pl.cast(
                    pl.slice(normed_states, [1, RMSNORM_K_CHUNK], [act_quant_row, act_quant_write_k0]),
                    target_type=pl.BF16,
                )
    ...
    # ── Scope 1: Q projection — A8W8: act-per-row quant + INT8@INT8→INT32 + dequant. ──
    ...
    # Dequant Q: acc_i32 * act_scale * wq_scale
    ...
    for b in pl.spmd(BATCH, name_hint="rope_qkv"):
        ...
        for ki in pl.pipeline(NUM_KV_HEADS, stage=2):
            ...
            k_rot_fp32_lo = pl.cast(rot_lo, target_type=pl.FP32)
            k_rot_fp32_hi = pl.cast(rot_hi, target_type=pl.FP32)
            ...
            for qj in pl.range(Q_PER_KV):
                ...
                q_rot_lo = pl.sub(pl.mul(q_lo, cos_lo), pl.mul(q_hi, sin_lo))
                q_rot_hi = pl.add(pl.mul(q_hi, cos_hi), pl.mul(q_lo, sin_hi))
    ...
    for fa_core in pl.spmd(NUM_CORES, name_hint="fa_fused"):
        ...
        for fa_w in pl.range(fa_core, fa_total_blocks, NUM_CORES):
            ...
            for gp in pl.range(GP_SIZE):
                ...
                v_tile = pl.create_tensor([ATTN_TILE, HEAD_DIM], dtype=pl.BF16)
                ...
                oi_tmp = pl.tensor.set_validshape(oi_tmp, Q_HEAD_BATCH, HEAD_DIM)

                all_oi_tmp = pl.assemble(all_oi_tmp, oi_tmp, [g_base + fa_part * Q_HEAD_PAD, 0])
    ...
    with pl.at(level=pl.Level.CORE_GROUP, name_hint="out_act_quant"):
        for out_quant_row in pl.range(BATCH):
            ...
            for out_quant_amax_kb in pl.range(HIDDEN // K_CHUNK):
                ...
                out_quant_amax_bf16 = pl.cast(
                    pl.slice(attn_out, [1, K_CHUNK], [out_quant_row, out_quant_amax_k0]),
                    target_type=pl.BF16,
                )
            ...
            for out_quant_write_kb in pl.range(HIDDEN // K_CHUNK):
                ...
                out_quant_write_bf16 = pl.cast(
                    pl.slice(attn_out, [1, K_CHUNK], [out_quant_row, out_quant_write_k0]),
                    target_type=pl.BF16,
                )
    ...
    for n_out_proj in pl.parallel(N_SPLITS_OUT):
        ...
        with pl.at(level=pl.Level.CORE_GROUP, name_hint="out_proj"):
            ...
            for k_split_out in pl.range(K_SPLITS_OUT):
                ...
                out_acc_k = pl.matmul(
                    attn_out_i8[:, k_op : k_op + OUT_INNER_TK],
                ...
                for out_lk in pl.range(1, OUT_N_SUB_K):
                    ...
                    out_a_k = attn_out_i8[:, k_op + out_ks_off : k_op + out_ks_off + OUT_INNER_TK]
                    out_w_k = wo[
    ...
    for n_out in pl.parallel(MLP_ON):
        ...
        with pl.at(level=pl.Level.CORE_GROUP, name_hint="gate_proj"):
            ...
            for k_split in pl.range(K_SPLITS_MLP):
                ...
                gate_c_acc = pl.matmul(
                    mlp_norm_in[:, gate_k0 : gate_k0 + MLP_INNER_TK],
                    w_gate[layer_hidden_base + gate_k0 : layer_hidden_base + gate_k0 + MLP_INNER_TK, gate_n0 : gate_n0 + MLP_TN],
                    out_dtype=pl.FP32,
                )
                for lk in pl.range(1, MLP_N_SUB_K):
                    ...
                    gate_w_k = w_gate[
                        layer_hidden_base + gate_k0 + gate_ks_off : layer_hidden_base + gate_k0 + gate_ks_off + MLP_INNER_TK,
        ...
        with pl.at(level=pl.Level.CORE_GROUP, name_hint="up_proj"):
            ...
            for k_split in pl.range(K_SPLITS_MLP):
                ...
                up_c_acc = pl.matmul(
                    mlp_norm_in[:, up_k0 : up_k0 + MLP_INNER_TK],
                    w_up[layer_hidden_base + up_k0 : layer_hidden_base + up_k0 + MLP_INNER_TK, gate_n0 : gate_n0 + MLP_TN],
                    out_dtype=pl.FP32,
                )
                for lk in pl.range(1, MLP_N_SUB_K):
                    ...
                    up_w_k = w_up[
                        layer_hidden_base + up_k0 + up_ks_off : layer_hidden_base + up_k0 + up_ks_off + MLP_INNER_TK,
                        gate_n0 : gate_n0 + MLP_TN,
                    ]
    ...
    if not FUSE_MLP_DOWN_MATERIALIZE:
        ...
        with pl.at(level=pl.Level.CORE_GROUP, name_hint="down_proj"):
            ...
            for k_split in pl.range(K_SPLITS):
                ...
                down_c_acc = pl.matmul(
                    mlp_down_tile[:, down_k0 : down_k0 + DOWN_TK],
                ...
                for lk in pl.range(1, N_SUB_K):
                    ...
                    down_a_k = mlp_down_tile[:, down_k0 + down_ks_off : down_k0 + down_ks_off + DOWN_TK]
                    down_w_k = w_down[
    ...
    for n_out in pl.parallel(DOWN_ON):
        ...
        with pl.at(level=pl.Level.CORE_GROUP, name_hint="down_cast_residual"):
            ...
            out_partial = pl.assemble(out_partial, pl.cast(out_chunk, target_type=pl.BF16), [0, down_cast_n0])
    ...
    if DECODE_DEBUG_STAGE_ID == 1:
            ...
            with pl.at(level=pl.Level.CORE_GROUP, name_hint="debug_q_pre_rope"):
                for debug_ki in pl.range(NUM_KV_HEADS):
                    ...
                    for debug_qj in pl.range(Q_PER_KV):
                        ...
                        debug_q_col = (debug_q_base + debug_qj) * HEAD_DIM
                        debug_q_head = pl.mul(
                            q_proj_norm[debug_b : debug_b + 1, debug_q_col : debug_q_col + HEAD_DIM],
                            debug_q_inv,
                        )
                        out = pl.assemble(
                            out,
        ...
        for debug_b in pl.parallel(BATCH):
            with pl.at(level=pl.Level.CORE_GROUP, name_hint="debug_k_pre_rope"):
                for debug_ki in pl.range(NUM_KV_HEADS):
                    ...
                    debug_k_head = pl.mul(
                        k_proj_norm[debug_b : debug_b + 1, debug_k_col : debug_k_col + HEAD_DIM],
                        debug_k_inv,
                    )
                    out = pl.assemble(
                        out,
        ...
        for oc_b in pl.parallel(0, BATCH, BATCH):
            with pl.at(level=pl.Level.CORE_GROUP, name_hint="out_consolidate"):
