...
_DEBUG_STAGE_NAME = os.environ.get("PREFILL_A8W8_DEBUG_STAGE", "")
DEBUG_STAGE_ID = (
    1 if _DEBUG_STAGE_NAME == "resid1" else
    2 if _DEBUG_STAGE_NAME == "post_norm" else
    3 if _DEBUG_STAGE_NAME == "attn" else
    4 if _DEBUG_STAGE_NAME == "q_norm" else
    5 if _DEBUG_STAGE_NAME == "k_norm" else
    6 if _DEBUG_STAGE_NAME == "v_proj" else
    7 if _DEBUG_STAGE_NAME == "attn_li" else
    8 if _DEBUG_STAGE_NAME == "attn_i8" else
    9 if _DEBUG_STAGE_NAME == "attn_scale" else
    10 if _DEBUG_STAGE_NAME == "mlp_silu" else
    11 if _DEBUG_STAGE_NAME == "down_acc" else
    12 if _DEBUG_STAGE_NAME == "down_partial0" else
    13 if _DEBUG_STAGE_NAME == "down_acc0" else
    14 if _DEBUG_STAGE_NAME == "down_zero_a_w0" else
    15 if _DEBUG_STAGE_NAME == "down_mlp_zero_w" else
    16 if _DEBUG_STAGE_NAME == "down_copy_mlp_w0" else
    0
)
    ...
    for b in pl.parallel(0, user_batch, 1):
        ...
        for p0_idx in pl.range(tok_blocks):
            with pl.scope():
                ...
                # ── A8W8 activation quant: per-row quant of normed_tile -> normed_i8 + act_scales ──
                ...
                with pl.at(level=pl.Level.CORE_GROUP, name_hint="normed_act_quant"):
                    # Use scalar amax for the active rows. This avoids the row_max/reshape path,
                    # which is fragile for the temporary scale tile on current board runtime.
                    for act_scale_ti in pl.range(valid_tok):
                        act_amax_value = INT8_AMAX_EPS
                        for kb in pl.range(HIDDEN_BLOCKS):
                            quant_amax_k0 = kb * K_CHUNK
                            quant_amax_row_bf16 = pl.cast(
                                pl.slice(normed_tile, [1, K_CHUNK], [act_scale_ti, quant_amax_k0]),
                                target_type=pl.BF16,
                            )
                        ...
                        for kb in pl.range(HIDDEN_BLOCKS):
                            ...
                            quant_write_row_bf16 = pl.cast(
                                pl.slice(normed_tile, [1, K_CHUNK], [act_scale_ti, quant_write_k0]),
                                target_type=pl.BF16,
                            )
                ...
                # Stage 1.2: Q projection — keep matmul and dequant in separate tasks
                # to avoid mixed AIC/AIV wrapper calls.
                ...
                for ob_chunk in pl.parallel(0, Q_OUT_BLOCKS, 4):
                    ...
                    with pl.at(level=pl.Level.CORE_GROUP, name_hint="q_proj_dequant"):
                        for ob in pl.range(ob_chunk, ob_chunk + 4):
                            ...
                            # Dequant: INT32 acc * wq_scale * act_scale
                ...
                # Stage 1.3: K/V projection — A8W8. Shared act_scales from above.
                    ...
                    with pl.scope():
                        ...
                        with pl.at(level=pl.Level.CORE_GROUP, name_hint="rope_kv_cache"):
                            for ki in pl.unroll(NUM_KV_HEADS):
                                    ...
                                    # ── K cache INT8 quant: rot_lo/rot_hi → INT8 + per-row FP32 scale ──
                                    rot_abs_lo = pl.maximum(rot_lo, pl.neg(rot_lo))
                                    rot_abs_hi = pl.maximum(rot_hi, pl.neg(rot_hi))
                                    k_amax_scalar = pl.max(
                                        pl.tensor.read(rot_abs_lo, [0, 0]),
                                        INT8_AMAX_EPS,
                                    )
                                    for hd in pl.range(1, HALF_DIM):
                                        k_amax_scalar = pl.max(
                                            k_amax_scalar,
                                            pl.tensor.read(rot_abs_lo, [0, hd]),
                                        )
                                    for hd in pl.range(HALF_DIM):
                                        k_amax_scalar = pl.max(
                                            k_amax_scalar,
                                            pl.tensor.read(rot_abs_hi, [0, hd]),
                                        )
                                    ...
                                    kq_lo_half = pl.cast(kq_lo_i32, target_type=pl.FP16)
                                    kq_hi_half = pl.cast(kq_hi_i32, target_type=pl.FP16)
                                    kq_lo = pl.cast(kq_lo_half, target_type=pl.INT8, mode="trunc")
                                    kq_hi = pl.cast(kq_hi_half, target_type=pl.INT8, mode="trunc")
                                    ...
                                    vq_half = pl.cast(vq_i32, target_type=pl.FP16)
                                    vq = pl.cast(vq_half, target_type=pl.INT8, mode="trunc")
                        ...
                        if DEBUG_STAGE_ID == 7:
                            with pl.at(level=pl.Level.CORE_GROUP, name_hint="debug_attn_diag_init"):
                                attn_diag_zero = pl.cast(
                                    pl.full([1, K_CHUNK], dtype=pl.FP32, value=0.0),
                                    target_type=pl.BF16,
                                )
                                attn_row = pl.assemble(attn_row, attn_diag_zero, [0, 0])
                        ...
                        for gi in pl.unroll(TOTAL_Q_GROUPS):
                            ...
                            # Stage 2.2-2.5: process each sb chunk with chunk-local scratch.
                            ...
                            for sb0 in pl.range(0, ctx_blocks, SB_BATCH):
                                ...
                                chunk_cur_mi = pl.create_tensor([SB_BATCH * Q_HEAD_BATCH_PAD, 1], dtype=pl.FP32)
                                chunk_cur_li = pl.create_tensor([SB_BATCH * Q_HEAD_BATCH_PAD, 1], dtype=pl.FP32)
                                    ...
                                    for qk_load_lane in pl.range(SB_BATCH):
                                        ...
                                        if qk_load_sb < ctx_blocks:
                                            ...
                                            for qk_scale_ti in pl.range(qk_valid_len):
                                                ...
                                                qk_row_fp32 = pl.cast(
                                                    pl.cast(qk_row_i8, target_type=pl.FP16),
                                                    target_type=pl.FP32,
                                                )
                                                qk_tile_bf16 = pl.assemble(
                                                    qk_tile_bf16,
                                                    pl.cast(pl.mul(qk_row_fp32, qk_scale), target_type=pl.BF16),
                                                    [qk_scale_ti, 0],
                                                )
                                    ...
                                    for softmax_sb_lane in pl.range(SB_BATCH):
                                        ...
                                        if softmax_sb < ctx_blocks:
                                            ...
                                            scores_valid = pl.slice(
                                                chunk_raw_scores, [Q_HEAD_BATCH_PAD, SEQ_TILE],
                                            ...
                                            chunk_cur_mi = pl.assemble(chunk_cur_mi, cur_mi, [softmax_sb_lane * Q_HEAD_BATCH_PAD, 0])
                                            chunk_cur_li = pl.assemble(chunk_cur_li, cur_li, [softmax_sb_lane * Q_HEAD_BATCH_PAD, 0])
                                    ...
                                    for sv_load_lane in pl.range(SB_BATCH):
                                        ...
                                        if sv_load_sb < ctx_blocks:
                                            ...
                                            for sv_scale_ti in pl.range(sv_valid_len):
                                                ...
                                                sv_row_fp32 = pl.cast(
                                                    pl.cast(sv_row_i8, target_type=pl.FP16),
                                                    target_type=pl.FP32,
                                                )
                                                sv_tile_bf16 = pl.assemble(
                                                    sv_tile_bf16,
                                                    pl.cast(pl.mul(sv_row_fp32, sv_scale), target_type=pl.BF16),
                                                    [sv_scale_ti, 0],
                                                )
                                ...
                                with pl.at(level=pl.Level.CORE_GROUP, name_hint="online_softmax"):
                                    for online_sb_lane in pl.range(SB_BATCH):
                                        ...
                                        if online_sb < ctx_blocks:
                                            oi_sb = pl.slice(chunk_oi_tmp, [Q_HEAD_BATCH_PAD, HEAD_DIM], [online_sb_lane * Q_HEAD_PAD, 0])
                                            mi_sb = pl.slice(chunk_cur_mi, [Q_HEAD_BATCH_PAD, 1], [online_sb_lane * Q_HEAD_BATCH_PAD, 0])
                                            li_sb = pl.slice(chunk_cur_li, [Q_HEAD_BATCH_PAD, 1], [online_sb_lane * Q_HEAD_BATCH_PAD, 0])
                                            if online_sb == 0:
                                                oi = oi_sb
                                                li = li_sb
                                                mi = mi_sb
                            ...
                            if DEBUG_STAGE_ID == 7:
                                with pl.at(level=pl.Level.CORE_GROUP, name_hint="debug_attention_li_writeback"):
                                    diag_li = pl.cast(
                                        pl.reshape(
                                            pl.slice(li, [Q_HEAD_PAD, 1], [0, 0]),
                                            [1, Q_HEAD_PAD],
                                        ),
                                        target_type=pl.BF16,
                                    )
                                    attn_row = pl.assemble(attn_row, diag_li, [0, gi * Q_HEAD_PAD])
                            else:
                ...
                # A8W8: quantize attn_tile per-row -> INT8 + scale, then INT8@INT8->INT32.
                ...
                with pl.at(level=pl.Level.CORE_GROUP, name_hint="out_act_quant"):
                    for attn_scale_ti in pl.range(valid_tok):
                        ...
                        for kb in pl.range(HIDDEN_BLOCKS):
                            ...
                            out_quant_amax_row_bf16 = pl.cast(
                                pl.slice(attn_tile, [1, K_CHUNK], [attn_scale_ti, out_quant_amax_k0]),
                                target_type=pl.BF16,
                            )
                        ...
                        for kb in pl.range(HIDDEN_BLOCKS):
                            ...
                            out_scale_row_bf16 = pl.cast(
                                pl.slice(attn_tile, [1, K_CHUNK], [attn_scale_ti, out_quant_write_k0]),
                                target_type=pl.BF16,
                            )
                ...
                # Stage 3.1: Output projection (A8W8) + first residual.
                ...
                for ob in pl.unroll(Q_OUT_BLOCKS):
                    ...
                    with pl.at(level=pl.Level.CORE_GROUP, name_hint="out_proj_reduce_dequant"):
                        ...
                        # Dequant: INT32 acc * wo_scale * attn_scale
                ...
                if DEBUG_STAGE_ID != 0 and DEBUG_STAGE_ID < 10:
                    if DEBUG_STAGE_ID == 5:
                        for debug_kb in pl.range(KV_HIDDEN // K_CHUNK):
                            debug_k0 = debug_kb * K_CHUNK
                            with pl.at(level=pl.Level.CORE_GROUP, name_hint="debug_k_norm_out"):
                                debug_k_chunk = pl.cast(
                                    pl.slice(k_proj_tile, [TOK_TILE, K_CHUNK], [0, debug_k0]),
                                    target_type=pl.BF16,
                                )
                                out = pl.assemble(out, debug_k_chunk, [token_p0, debug_k0])
                    elif DEBUG_STAGE_ID == 6:
                        for debug_kb in pl.range(KV_HIDDEN // K_CHUNK):
                            debug_k0 = debug_kb * K_CHUNK
                            with pl.at(level=pl.Level.CORE_GROUP, name_hint="debug_v_proj_out"):
                                debug_v_chunk = pl.cast(
                                    pl.slice(v_proj_tile, [TOK_TILE, K_CHUNK], [0, debug_k0]),
                                    target_type=pl.BF16,
                                )
                                out = pl.assemble(out, debug_v_chunk, [token_p0, debug_k0])
                    else:
                        for debug_kb in pl.range(HIDDEN_BLOCKS):
                            debug_k0 = debug_kb * K_CHUNK
                            if DEBUG_STAGE_ID == 1:
                                with pl.at(level=pl.Level.CORE_GROUP, name_hint="debug_resid1_out"):
                                    debug_resid1_chunk = pl.cast(
                                        pl.slice(resid1_tile, [TOK_TILE, K_CHUNK], [0, debug_k0]),
                                        target_type=pl.BF16,
                                    )
                                    out = pl.assemble(out, debug_resid1_chunk, [token_p0, debug_k0])
                            elif DEBUG_STAGE_ID == 3 or DEBUG_STAGE_ID == 7:
                                with pl.at(level=pl.Level.CORE_GROUP, name_hint="debug_attn_out"):
                                    debug_attn_chunk = pl.slice(attn_tile, [TOK_TILE, K_CHUNK], [0, debug_k0])
                                    out = pl.assemble(out, debug_attn_chunk, [token_p0, debug_k0])
                            elif DEBUG_STAGE_ID == 8:
                                with pl.at(level=pl.Level.CORE_GROUP, name_hint="debug_attn_i8_out"):
                                    debug_attn_i8_chunk = pl.cast(
                                        pl.cast(
                                            pl.slice(attn_i8, [TOK_TILE, K_CHUNK], [0, debug_k0]),
                                            target_type=pl.FP16,
                                        ),
                                        target_type=pl.BF16,
                                    )
                                    out = pl.assemble(out, debug_attn_i8_chunk, [token_p0, debug_k0])
                            elif DEBUG_STAGE_ID == 9:
                                with pl.at(level=pl.Level.CORE_GROUP, name_hint="debug_attn_scale_out"):
                                    debug_scale_chunk = pl.create_tensor([TOK_TILE, K_CHUNK], dtype=pl.FP32)
                                    for debug_scale_ti in pl.unroll(TOK_TILE):
                                        debug_scale_value = pl.tensor.read(attn_scales, [debug_scale_ti, 0])
                                        debug_scale_row = pl.mul(
                                            pl.full([1, K_CHUNK], dtype=pl.FP32, value=1.0),
                                            debug_scale_value,
                                        )
                                        debug_scale_chunk = pl.assemble(
                                            debug_scale_chunk,
                                            debug_scale_row,
                                            [debug_scale_ti, 0],
                                        )
                                    out = pl.assemble(
                                        out,
                                        pl.cast(debug_scale_chunk, target_type=pl.BF16),
                                        [token_p0, debug_k0],
                                    )
                            elif DEBUG_STAGE_ID == 4:
                                with pl.at(level=pl.Level.CORE_GROUP, name_hint="debug_q_norm_out"):
                                    debug_q_chunk = pl.cast(
                                        pl.slice(q_proj_tile, [TOK_TILE, K_CHUNK], [0, debug_k0]),
                                        target_type=pl.BF16,
                                    )
                                    out = pl.assemble(out, debug_q_chunk, [token_p0, debug_k0])
                            else:
                                with pl.at(level=pl.Level.CORE_GROUP, name_hint="debug_post_norm_out"):
                                    debug_post_norm_chunk = pl.slice(post_norm_tile, [TOK_TILE, K_CHUNK], [0, debug_k0])
                                    out = pl.assemble(out, debug_post_norm_chunk, [token_p0, debug_k0])
                else:
                    ...
                    if DEBUG_STAGE_ID == 10:
                        ...
                        with pl.at(level=pl.Level.CORE_GROUP, name_hint="debug_down_partial0"):
                            ...
                            debug_down_partial0_tensor = pl.assemble(
                                debug_down_partial0_tensor,
                                debug_down_partial0,
                                [0, 0],
                            )
                        with pl.at(level=pl.Level.CORE_GROUP, name_hint="debug_down_partial0_out"):
                            out = pl.assemble(
                                out,
                                pl.cast(debug_down_partial0_tensor, target_type=pl.BF16),
                                [token_p0, 0],
                            )
                    elif DEBUG_STAGE_ID == 13:
                        debug_down_acc0 = pl.create_tensor([TOK_TILE, K_CHUNK], dtype=pl.FP32)
                        debug_down_partials0 = pl.create_tensor([MLP_OUT_BLOCKS * TOK_TILE, K_CHUNK], dtype=pl.FP32)
                        debug_down_ready0 = pl.create_tensor([MLP_OUT_BLOCKS], dtype=pl.INT32)
                        for ob in pl.range(MLP_OUT_BLOCKS):
                            with pl.at(level=pl.Level.CORE_GROUP, name_hint="debug_down_matmul0"):
                                debug_down_o0 = ob * MLP_OUT_CHUNK
                                debug_mlp_chunk_i = pl.slice(mlp_down_tile, [TOK_TILE, MLP_OUT_CHUNK], [0, debug_down_o0])
                                debug_w_down_i = pl.slice(w_down, [MLP_OUT_CHUNK, K_CHUNK], [layer_inter_base + debug_down_o0, 0])
                                debug_down_partial_i = pl.matmul(
                                    debug_mlp_chunk_i,
                                    debug_w_down_i,
                                    out_dtype=pl.FP32,
                                )
                                debug_down_partials0 = pl.assemble(
                                    debug_down_partials0,
                                    debug_down_partial_i,
                                    [ob * TOK_TILE, 0],
                                )
                                pl.tensor.write(debug_down_ready0, [ob], pl.cast(ob * 0 + 1, target_type=pl.INT32))
                        with pl.at(level=pl.Level.CORE_GROUP, name_hint="debug_down_reduce0"):
                            debug_down_acc0_local = pl.full([TOK_TILE, K_CHUNK], dtype=pl.FP32, value=0.0)
                            for ob in pl.unroll(MLP_OUT_BLOCKS):
                                debug_down_partial0_i = pl.slice(
                                    debug_down_partials0,
                                    [TOK_TILE, K_CHUNK],
                                    [ob * TOK_TILE, 0],
                                )
                                debug_down_ready0_i = pl.cast(
                                    pl.tensor.read(debug_down_ready0, [ob]),
                                    target_type=pl.FP32,
                                )
                                debug_down_partial0_i = pl.mul(debug_down_partial0_i, debug_down_ready0_i)
                                debug_down_acc0_local = pl.add(debug_down_acc0_local, debug_down_partial0_i)
                            debug_down_acc0 = pl.assemble(debug_down_acc0, debug_down_acc0_local, [0, 0])
                        with pl.at(level=pl.Level.CORE_GROUP, name_hint="debug_down_acc0_out"):
                            out = pl.assemble(
                                out,
                                pl.cast(debug_down_acc0, target_type=pl.BF16),
                                [token_p0, 0],
                            )
                    elif DEBUG_STAGE_ID == 14:
                        debug_zero_a_tensor = pl.create_tensor([TOK_TILE, MLP_OUT_CHUNK], dtype=pl.BF16)
                        debug_down_zero_a_out = pl.create_tensor([TOK_TILE, K_CHUNK], dtype=pl.FP32)
                        with pl.at(level=pl.Level.CORE_GROUP, name_hint="debug_make_zero_a"):
                            debug_zero_a_tile = pl.full([TOK_TILE, MLP_OUT_CHUNK], dtype=pl.BF16, value=0.0)
                            debug_zero_a_tensor = pl.assemble(debug_zero_a_tensor, debug_zero_a_tile, [0, 0])
                        with pl.at(level=pl.Level.CORE_GROUP, name_hint="debug_down_zero_a_w0"):
                            debug_zero_a = pl.slice(debug_zero_a_tensor, [TOK_TILE, MLP_OUT_CHUNK], [0, 0])
                            debug_w_down_chunk0 = pl.slice(w_down, [MLP_OUT_CHUNK, K_CHUNK], [layer_inter_base, 0])
                            debug_down_zero_a = pl.matmul(debug_zero_a, debug_w_down_chunk0, out_dtype=pl.FP32)
                            debug_down_zero_a_out = pl.assemble(debug_down_zero_a_out, debug_down_zero_a, [0, 0])
                        with pl.at(level=pl.Level.CORE_GROUP, name_hint="debug_down_zero_a_w0_out"):
                            out = pl.assemble(
                                out,
                                pl.cast(debug_down_zero_a_out, target_type=pl.BF16),
                                [token_p0, 0],
                            )
                    elif DEBUG_STAGE_ID == 15:
                        debug_zero_w_tensor = pl.create_tensor([MLP_OUT_CHUNK, K_CHUNK], dtype=pl.BF16)
                        debug_down_zero_w_out = pl.create_tensor([TOK_TILE, K_CHUNK], dtype=pl.FP32)
                        with pl.at(level=pl.Level.CORE_GROUP, name_hint="debug_make_zero_w"):
                            debug_zero_w_tile = pl.full([MLP_OUT_CHUNK, K_CHUNK], dtype=pl.BF16, value=0.0)
                            debug_zero_w_tensor = pl.assemble(debug_zero_w_tensor, debug_zero_w_tile, [0, 0])
                        with pl.at(level=pl.Level.CORE_GROUP, name_hint="debug_down_mlp_zero_w"):
                            debug_mlp_chunk0 = pl.slice(mlp_down_tile, [TOK_TILE, MLP_OUT_CHUNK], [0, 0])
                            debug_zero_w = pl.slice(debug_zero_w_tensor, [MLP_OUT_CHUNK, K_CHUNK], [0, 0])
                            debug_down_zero_w = pl.matmul(debug_mlp_chunk0, debug_zero_w, out_dtype=pl.FP32)
                            debug_down_zero_w_out = pl.assemble(debug_down_zero_w_out, debug_down_zero_w, [0, 0])
                        with pl.at(level=pl.Level.CORE_GROUP, name_hint="debug_down_mlp_zero_w_out"):
                            out = pl.assemble(
                                out,
                                pl.cast(debug_down_zero_w_out, target_type=pl.BF16),
                                [token_p0, 0],
                            )
                    elif DEBUG_STAGE_ID == 16:
                        debug_mlp_copy_tensor = pl.create_tensor([TOK_TILE, MLP_OUT_CHUNK], dtype=pl.BF16)
                        debug_down_copy_out = pl.create_tensor([TOK_TILE, K_CHUNK], dtype=pl.FP32)
                        with pl.at(level=pl.Level.CORE_GROUP, name_hint="debug_copy_mlp0"):
                            debug_mlp_chunk0 = pl.slice(mlp_down_tile, [TOK_TILE, MLP_OUT_CHUNK], [0, 0])
                            debug_mlp_copy_tensor = pl.assemble(debug_mlp_copy_tensor, debug_mlp_chunk0, [0, 0])
                        with pl.at(level=pl.Level.CORE_GROUP, name_hint="debug_down_copy_mlp_w0"):
                            debug_mlp_copy = pl.slice(debug_mlp_copy_tensor, [TOK_TILE, MLP_OUT_CHUNK], [0, 0])
                            debug_w_down_chunk0 = pl.slice(w_down, [MLP_OUT_CHUNK, K_CHUNK], [layer_inter_base, 0])
                            debug_down_copy = pl.matmul(debug_mlp_copy, debug_w_down_chunk0, out_dtype=pl.FP32)
                            debug_down_copy_out = pl.assemble(debug_down_copy_out, debug_down_copy, [0, 0])
                        with pl.at(level=pl.Level.CORE_GROUP, name_hint="debug_down_copy_mlp_w0_out"):
                            out = pl.assemble(
                                out,
                                pl.cast(debug_down_copy_out, target_type=pl.BF16),
                                [token_p0, 0],
                            )
                    else:
                        # Stage 3.3b: Down projection + final residual -> output.
                        for dob in pl.range(HIDDEN_BLOCKS):
                            ...
                            with pl.at(level=pl.Level.CORE_GROUP, name_hint="down_proj_residual"):
                                if DEBUG_STAGE_ID == 11:
                                    out = pl.assemble(
                                        out,
                                        pl.cast(down_acc, target_type=pl.BF16),
                                        [token_p0, down_proj_d0],
                                    )
                                else:
                                    out_chunk = pl.add(
                                        down_acc,
