    ...
    class Qwen3Decode14B:
        ...
        def qwen3_14b_decode(
            ...
            with pl.at(level=pl.Level.CORE_GROUP, name_hint="qwen3_14b_decode"):
                ...
                s1_sumsq = pl.tile.full([BATCH, VEC_W], value=0.0, dtype=pl.FP32)
                s1_sumsq = pl.tile.set_validshape(s1_sumsq, [BATCH, 1])
                for kb in pl.range(HIDDEN // VEC_BF16):
                    ...
                    for h in pl.range(2):
                        s1_h0 = h * VEC_W
                        s1_x_half = pl.tile.slice(s1_x_bf16, [BATCH, VEC_W], [0, s1_h0])
                        s1_x_half = pl.tile.set_validshape(s1_x_half, [BATCH, VEC_W])
                        s1_x = pl.tile.cast(s1_x_half, dtype=pl.FP32)
                        s1_sq = pl.tile.mul(s1_x, s1_x)
                        s1_part = pl.tile.row_sum(s1_sq)
                        s1_part = pl.tile.set_validshape(s1_part, [BATCH, 1])
                        s1_sumsq_acc = pl.tile.add(s1_sumsq, s1_part)
                        s1_sumsq = pl.tile.set_validshape(s1_sumsq_acc, [BATCH, 1])

                s1_mean_sq = pl.tile.mul(s1_sumsq, HIDDEN_INV)
                s1_mean_sq = pl.tile.set_validshape(s1_mean_sq, [BATCH, 1])
                s1_variance = pl.tile.add(s1_mean_sq, EPS)
                s1_variance = pl.tile.set_validshape(s1_variance, [BATCH, 1])
                s1_rms = pl.tile.sqrt(s1_variance)
                s1_rms = pl.tile.set_validshape(s1_rms, [BATCH, 1])
                s1_inv_rms = pl.tile.recip(s1_rms)
                s1_inv_rms = pl.tile.set_validshape(s1_inv_rms, [BATCH, 1])
                ...
                for kb in pl.range(HIDDEN // VEC_BF16):
                    ...
                    for h in pl.range(2):
                        s2_h0 = h * VEC_W
                        s2_x_half = pl.tile.slice(s2_x_bf16, [BATCH, VEC_W], [0, s2_h0])
                        s2_x_half = pl.tile.set_validshape(s2_x_half, [BATCH, VEC_W])
                        s2_x = pl.tile.cast(s2_x_half, dtype=pl.FP32)
                        s2_gamma_blk = pl.tile.load(input_rms_weight, [1, 1, 1, VEC_W], [kb, 0, 0, s2_h0])
                        s2_gamma = pl.tile.reshape(s2_gamma_blk, [1, VEC_W])
                        s2_gamma = pl.tile.set_validshape(s2_gamma, [1, VEC_W])
                        s2_x_scaled = pl.tile.row_expand_mul(s2_x, s1_inv_rms)
                        s2_normed = pl.tile.col_expand_mul(s2_x_scaled, s2_gamma)
                        s2_normed_bf16 = pl.tile.cast(s2_normed, dtype=pl.BF16)
                        s2_normed_bf16 = pl.tile.set_validshape(s2_normed_bf16, [BATCH, VEC_W])
                ...
                for nb in pl.range(HIDDEN // MM_N):
                    ...
                    q_a0 = pl.tile.reshape(q_a0_blk, [BATCH, MM_K])
                    q_a0 = pl.tile.set_validshape(q_a0, [BATCH, MM_K])
                    ...
                    for kb in pl.range(1, HIDDEN // MM_K):
                        ...
                        q_a = pl.tile.reshape(q_a_blk, [BATCH, MM_K])
                        q_a = pl.tile.set_validshape(q_a, [BATCH, MM_K])
                ...
                for nb in pl.range(KV_HIDDEN // MM_N):
                    ...
                    k_a0 = pl.tile.reshape(k_a0_blk, [BATCH, MM_K])
                    k_a0 = pl.tile.set_validshape(k_a0, [BATCH, MM_K])
                    ...
                    for kb in pl.range(1, HIDDEN // MM_K):
                        ...
                        k_a = pl.tile.reshape(k_a_blk, [BATCH, MM_K])
                        k_a = pl.tile.set_validshape(k_a, [BATCH, MM_K])
                ...
                for nb in pl.range(KV_HIDDEN // MM_N):
                    ...
                    v_a0 = pl.tile.reshape(v_a0_blk, [BATCH, MM_K])
                    v_a0 = pl.tile.set_validshape(v_a0, [BATCH, MM_K])
                    ...
                    for kb in pl.range(1, HIDDEN // MM_K):
                        ...
                        v_a = pl.tile.reshape(v_a_blk, [BATCH, MM_K])
                        v_a = pl.tile.set_validshape(v_a, [BATCH, MM_K])
                ...
                for hq in pl.range(NUM_HEADS):
                    ...
                    qn_sumsq = pl.tile.row_sum(qn_sq_lo)
                    qn_sumsq = pl.tile.set_validshape(qn_sumsq, [BATCH, 1])
                    qn_sq_hi = pl.tile.mul(qn_x_hi, qn_x_hi)
                    qn_part = pl.tile.row_sum(qn_sq_hi)
                    qn_part = pl.tile.set_validshape(qn_part, [BATCH, 1])
                    qn_sumsq_acc = pl.tile.add(qn_sumsq, qn_part)
                    qn_sumsq = pl.tile.set_validshape(qn_sumsq_acc, [BATCH, 1])
                    qn_mean_sq = pl.tile.mul(qn_sumsq, HEAD_DIM_INV)
                    qn_mean_sq = pl.tile.set_validshape(qn_mean_sq, [BATCH, 1])
                    qn_variance = pl.tile.add(qn_mean_sq, EPS)
                    qn_variance = pl.tile.set_validshape(qn_variance, [BATCH, 1])
                    qn_inv_rms = pl.tile.rsqrt(qn_variance)
                    qn_inv_rms = pl.tile.set_validshape(qn_inv_rms, [BATCH, 1])
                    qn_g_lo_blk = pl.tile.load(q_norm_weight, [1, 1, 1, HALF_DIM], [0, 0, 0, 0])
                    qn_g_lo = pl.tile.reshape(qn_g_lo_blk, [1, HALF_DIM])
                    qn_g_lo = pl.tile.set_validshape(qn_g_lo, [1, HALF_DIM])
                    qn_g_hi_blk = pl.tile.load(q_norm_weight, [1, 1, 1, HALF_DIM], [0, 0, 0, HALF_DIM])
                    qn_g_hi = pl.tile.reshape(qn_g_hi_blk, [1, HALF_DIM])
                    qn_g_hi = pl.tile.set_validshape(qn_g_hi, [1, HALF_DIM])
                ...
                for hk in pl.range(NUM_KV_HEADS):
                    ...
                    kn_sumsq = pl.tile.row_sum(kn_sq_lo)
                    kn_sumsq = pl.tile.set_validshape(kn_sumsq, [BATCH, 1])
                    kn_sq_hi = pl.tile.mul(kn_x_hi, kn_x_hi)
                    kn_part = pl.tile.row_sum(kn_sq_hi)
                    kn_part = pl.tile.set_validshape(kn_part, [BATCH, 1])
                    kn_sumsq_acc = pl.tile.add(kn_sumsq, kn_part)
                    kn_sumsq = pl.tile.set_validshape(kn_sumsq_acc, [BATCH, 1])
                    kn_mean_sq = pl.tile.mul(kn_sumsq, HEAD_DIM_INV)
                    kn_mean_sq = pl.tile.set_validshape(kn_mean_sq, [BATCH, 1])
                    kn_variance = pl.tile.add(kn_mean_sq, EPS)
                    kn_variance = pl.tile.set_validshape(kn_variance, [BATCH, 1])
                    kn_inv_rms = pl.tile.rsqrt(kn_variance)
                    kn_inv_rms = pl.tile.set_validshape(kn_inv_rms, [BATCH, 1])
                    kn_g_lo_blk = pl.tile.load(k_norm_weight, [1, 1, 1, HALF_DIM], [0, 0, 0, 0])
                    kn_g_lo = pl.tile.reshape(kn_g_lo_blk, [1, HALF_DIM])
                    kn_g_lo = pl.tile.set_validshape(kn_g_lo, [1, HALF_DIM])
                    kn_g_hi_blk = pl.tile.load(k_norm_weight, [1, 1, 1, HALF_DIM], [0, 0, 0, HALF_DIM])
                    kn_g_hi = pl.tile.reshape(kn_g_hi_blk, [1, HALF_DIM])
                    kn_g_hi = pl.tile.set_validshape(kn_g_hi, [1, HALF_DIM])
                ...
                for b in pl.range(BATCH):
                    ...
                    rp_cos_row = pl.tile.reshape(rp_cos_blk, [1, HEAD_DIM])
                    rp_cos_row = pl.tile.set_validshape(rp_cos_row, [1, HEAD_DIM])
                    rp_sin_blk = pl.tile.load(rope_sin, [1, 1, 1, HEAD_DIM], [rp_pos, 0, 0, 0])
                    rp_sin_row = pl.tile.reshape(rp_sin_blk, [1, HEAD_DIM])
                    rp_sin_row = pl.tile.set_validshape(rp_sin_row, [1, HEAD_DIM])
                    rp_cos_lo = pl.tile.slice(rp_cos_row, [1, HALF_DIM], [0, 0])
                    rp_cos_lo = pl.tile.set_validshape(rp_cos_lo, [1, HALF_DIM])
                    rp_cos_hi = pl.tile.slice(rp_cos_row, [1, HALF_DIM], [0, HALF_DIM])
                    rp_cos_hi = pl.tile.set_validshape(rp_cos_hi, [1, HALF_DIM])
                    rp_sin_lo = pl.tile.slice(rp_sin_row, [1, HALF_DIM], [0, 0])
                    rp_sin_lo = pl.tile.set_validshape(rp_sin_lo, [1, HALF_DIM])
                    rp_sin_hi = pl.tile.slice(rp_sin_row, [1, HALF_DIM], [0, HALF_DIM])
                    rp_sin_hi = pl.tile.set_validshape(rp_sin_hi, [1, HALF_DIM])
                    ...
                    for ki in pl.range(NUM_KV_HEADS):
                        ...
                        rp_k_lo = pl.tile.reshape(rp_k_lo_blk, [1, HALF_DIM])
                        rp_k_lo = pl.tile.set_validshape(rp_k_lo, [1, HALF_DIM])
                        rp_k_hi_blk = pl.tile.load(k_proj_norm, [1, 1, 1, HALF_DIM], [rp_kv_blo + 1, 0, b, 0])
                        rp_k_hi = pl.tile.reshape(rp_k_hi_blk, [1, HALF_DIM])
                        rp_k_hi = pl.tile.set_validshape(rp_k_hi, [1, HALF_DIM])
                        rp_klo_cos = pl.tile.col_expand_mul(rp_k_lo, rp_cos_lo)
                        rp_klo_cos = pl.tile.set_validshape(rp_klo_cos, [1, HALF_DIM])
                        rp_khi_sin = pl.tile.col_expand_mul(rp_k_hi, rp_sin_lo)
                        rp_khi_sin = pl.tile.set_validshape(rp_khi_sin, [1, HALF_DIM])
                        rp_k_rot_lo = pl.tile.sub(rp_klo_cos, rp_khi_sin)
                        rp_k_rot_lo = pl.tile.set_validshape(rp_k_rot_lo, [1, HALF_DIM])
                        rp_khi_cos = pl.tile.col_expand_mul(rp_k_hi, rp_cos_hi)
                        rp_khi_cos = pl.tile.set_validshape(rp_khi_cos, [1, HALF_DIM])
                        rp_klo_sin = pl.tile.col_expand_mul(rp_k_lo, rp_sin_hi)
                        rp_klo_sin = pl.tile.set_validshape(rp_klo_sin, [1, HALF_DIM])
                        rp_k_rot_hi = pl.tile.add(rp_khi_cos, rp_klo_sin)
                        rp_k_rot_hi = pl.tile.set_validshape(rp_k_rot_hi, [1, HALF_DIM])
                        rp_k_rot_lo_bf16 = pl.tile.cast(rp_k_rot_lo, dtype=pl.BF16)
                        rp_k_rot_lo_bf16 = pl.tile.set_validshape(rp_k_rot_lo_bf16, [1, HALF_DIM])
                        rp_k_rot_hi_bf16 = pl.tile.cast(rp_k_rot_hi, dtype=pl.BF16)
                        rp_k_rot_hi_bf16 = pl.tile.set_validshape(rp_k_rot_hi_bf16, [1, HALF_DIM])
                        ...
                        rp_v_lo = pl.tile.reshape(rp_v_lo_blk, [1, HALF_DIM])
                        rp_v_lo = pl.tile.set_validshape(rp_v_lo, [1, HALF_DIM])
                        rp_v_lo_bf16 = pl.tile.cast(rp_v_lo, dtype=pl.BF16)
                        rp_v_lo_bf16 = pl.tile.set_validshape(rp_v_lo_bf16, [1, HALF_DIM])
                        ...
                        rp_v_hi = pl.tile.reshape(rp_v_hi_blk, [1, HALF_DIM])
                        rp_v_hi = pl.tile.set_validshape(rp_v_hi, [1, HALF_DIM])
                        rp_v_hi_bf16 = pl.tile.cast(rp_v_hi, dtype=pl.BF16)
                        rp_v_hi_bf16 = pl.tile.set_validshape(rp_v_hi_bf16, [1, HALF_DIM])
                        ...
                        for qi in pl.range(Q_HEAD_BATCH):
                            ...
                            rp_q_lo = pl.tile.reshape(rp_q_lo_blk, [1, HALF_DIM])
                            rp_q_lo = pl.tile.set_validshape(rp_q_lo, [1, HALF_DIM])
                            rp_q_hi_blk = pl.tile.load(q_proj_norm, [1, 1, 1, HALF_DIM], [rp_q_blo + 1, 0, b, 0])
                            rp_q_hi = pl.tile.reshape(rp_q_hi_blk, [1, HALF_DIM])
                            rp_q_hi = pl.tile.set_validshape(rp_q_hi, [1, HALF_DIM])
                            rp_qlo_cos = pl.tile.col_expand_mul(rp_q_lo, rp_cos_lo)
                            rp_qhi_sin = pl.tile.col_expand_mul(rp_q_hi, rp_sin_lo)
                            rp_q_rot_lo = pl.tile.sub(rp_qlo_cos, rp_qhi_sin)
                            rp_q_rot_lo = pl.tile.set_validshape(rp_q_rot_lo, [1, HALF_DIM])
                            rp_qhi_cos = pl.tile.col_expand_mul(rp_q_hi, rp_cos_hi)
                            rp_qlo_sin = pl.tile.col_expand_mul(rp_q_lo, rp_sin_hi)
                            rp_q_rot_hi = pl.tile.add(rp_qhi_cos, rp_qlo_sin)
                            rp_q_rot_hi = pl.tile.set_validshape(rp_q_rot_hi, [1, HALF_DIM])
                            rp_q_rot_lo_bf16 = pl.tile.cast(rp_q_rot_lo, dtype=pl.BF16)
                            rp_q_rot_lo_bf16 = pl.tile.set_validshape(rp_q_rot_lo_bf16, [1, HALF_DIM])
                            rp_q_rot_hi_bf16 = pl.tile.cast(rp_q_rot_hi, dtype=pl.BF16)
                            rp_q_rot_hi_bf16 = pl.tile.set_validshape(rp_q_rot_hi_bf16, [1, HALF_DIM])
                        ...
                        rp_zpad = pl.tile.full([Q_HEAD_PAD, HEAD_DIM], value=0.0, dtype=pl.BF16)
                        rp_zpad = pl.tile.set_validshape(rp_zpad, [Q_HEAD_PAD - Q_HEAD_BATCH, HEAD_DIM])
                ...
                for b in pl.range(BATCH):
                    ...
                    for gi in pl.range(TOTAL_Q_GROUPS):
                        ...
                        fa_mi = pl.tile.full([Q_HEAD_PAD, VEC_W], value=NEG_INF, dtype=pl.FP32)
                        fa_mi = pl.tile.set_validshape(fa_mi, [Q_HEAD_PAD, 1])
                        fa_li = pl.tile.full([Q_HEAD_PAD, VEC_W], value=0.0, dtype=pl.FP32)
                        fa_li = pl.tile.set_validshape(fa_li, [Q_HEAD_PAD, 1])
                        ...
                        for st in pl.range(fa_n_steps):
                            ...
                            fa_q_sub0 = pl.tile.slice(fa_q_padded, [Q_HEAD_PAD, QK_KD], [0, 0])
                            fa_q_sub0 = pl.tile.set_validshape(fa_q_sub0, [Q_HEAD_PAD, QK_KD])
                            ...
                            for kd in pl.range(1, QK_KSTEPS):
                                fa_kd0 = kd * QK_KD
                                fa_q_sub = pl.tile.slice(fa_q_padded, [Q_HEAD_PAD, QK_KD], [0, fa_kd0])
                                fa_q_sub = pl.tile.set_validshape(fa_q_sub, [Q_HEAD_PAD, QK_KD])
                            ...
                            fa_cur_mi = pl.tile.row_max(fa_scores_pad)
                            fa_cur_mi = pl.tile.set_validshape(fa_cur_mi, [Q_HEAD_PAD, 1])
                            fa_shifted = pl.tile.row_expand_sub(fa_scores_pad, fa_cur_mi)
                            fa_exp_scores = pl.tile.exp(fa_shifted)
                            fa_exp_bf16 = pl.tile.cast(fa_exp_scores, dtype=pl.BF16)
                            fa_exp_bf16 = pl.tile.set_validshape(fa_exp_bf16, [Q_HEAD_PAD, ATT_SEQ])
                            fa_exp_fp32 = pl.tile.cast(fa_exp_bf16, dtype=pl.FP32)
                            fa_cur_li = pl.tile.row_sum(fa_exp_fp32)
                            fa_cur_li = pl.tile.set_validshape(fa_cur_li, [Q_HEAD_PAD, 1])

                            # --- SV matmul: oi halves over the SV_SSTEPS == 2 seq chunks ---
                            fa_exp_sub0 = pl.tile.slice(fa_exp_bf16, [Q_HEAD_PAD, SV_SEQ], [0, 0])
                            fa_exp_sub0 = pl.tile.set_validshape(fa_exp_sub0, [Q_HEAD_PAD, SV_SEQ])
                            ...
                            fa_exp_sub1 = pl.tile.slice(fa_exp_bf16, [Q_HEAD_PAD, SV_SEQ], [0, SV_SEQ])
                            fa_exp_sub1 = pl.tile.set_validshape(fa_exp_sub1, [Q_HEAD_PAD, SV_SEQ])
                            ...
                            fa_mi_new = pl.tile.maximum(fa_mi, fa_cur_mi)
                            fa_mi_new = pl.tile.set_validshape(fa_mi_new, [Q_HEAD_PAD, 1])
                            fa_mdiff = pl.tile.sub(fa_mi, fa_mi_new)
                            fa_mdiff = pl.tile.set_validshape(fa_mdiff, [Q_HEAD_PAD, 1])
                            fa_alpha = pl.tile.exp(fa_mdiff)
                            fa_alpha = pl.tile.set_validshape(fa_alpha, [Q_HEAD_PAD, 1])
                            fa_cdiff = pl.tile.sub(fa_cur_mi, fa_mi_new)
                            fa_cdiff = pl.tile.set_validshape(fa_cdiff, [Q_HEAD_PAD, 1])
                            fa_beta = pl.tile.exp(fa_cdiff)
                            fa_beta = pl.tile.set_validshape(fa_beta, [Q_HEAD_PAD, 1])
                            fa_li_a = pl.tile.mul(fa_alpha, fa_li)
                            fa_li_a = pl.tile.set_validshape(fa_li_a, [Q_HEAD_PAD, 1])
                            fa_li_b = pl.tile.mul(fa_beta, fa_cur_li)
                            fa_li_b = pl.tile.set_validshape(fa_li_b, [Q_HEAD_PAD, 1])
                        ...
                        for qi in pl.range(Q_HEAD_BATCH):
                            ...
                            fa_lo1 = pl.tile.slice(fa_ctx_lo, [1, HALF_DIM], [qi, 0])
                            fa_lo1 = pl.tile.set_validshape(fa_lo1, [1, HALF_DIM])
                            fa_lo1_bf16 = pl.tile.cast(fa_lo1, dtype=pl.BF16)
                            fa_lo1_bf16 = pl.tile.set_validshape(fa_lo1_bf16, [1, HALF_DIM])
                            fa_lo1_blk = pl.tile.reshape(fa_lo1_bf16, [1, 1, 1, HALF_DIM])
                            pl.tile.store(attn_out, fa_lo1_blk, [fa_h_blo, 0, b, 0])
                            fa_hi1 = pl.tile.slice(fa_ctx_hi, [1, HALF_DIM], [qi, 0])
                            fa_hi1 = pl.tile.set_validshape(fa_hi1, [1, HALF_DIM])
                            fa_hi1_bf16 = pl.tile.cast(fa_hi1, dtype=pl.BF16)
                            fa_hi1_bf16 = pl.tile.set_validshape(fa_hi1_bf16, [1, HALF_DIM])
                ...
                for nb in pl.range(HIDDEN // MM_N):
                    ...
                    op_a0 = pl.tile.reshape(op_a0_blk, [BATCH, MM_K])
                    op_a0 = pl.tile.set_validshape(op_a0, [BATCH, MM_K])
                    ...
                    for kb in pl.range(1, HIDDEN // MM_K):
                        ...
                        op_a = pl.tile.reshape(op_a_blk, [BATCH, MM_K])
                        op_a = pl.tile.set_validshape(op_a, [BATCH, MM_K])
                    ...
                    op_resid_bf16 = pl.tile.reshape(op_resid_blk, [BATCH, MM_N])
                    op_resid_bf16 = pl.tile.set_validshape(op_resid_bf16, [BATCH, MM_N])
                ...
                s3_sumsq = pl.tile.full([BATCH, VEC_W], value=0.0, dtype=pl.FP32)
                s3_sumsq = pl.tile.set_validshape(s3_sumsq, [BATCH, 1])
                for kb in pl.range(HIDDEN // MM_N):
                    ...
                    s3_part = pl.tile.row_sum(s3_sq)
                    s3_part = pl.tile.set_validshape(s3_part, [BATCH, 1])
                    s3_sumsq_acc = pl.tile.add(s3_sumsq, s3_part)
                    s3_sumsq = pl.tile.set_validshape(s3_sumsq_acc, [BATCH, 1])

                s3_mean_sq = pl.tile.mul(s3_sumsq, HIDDEN_INV)
                s3_mean_sq = pl.tile.set_validshape(s3_mean_sq, [BATCH, 1])
                s3_variance = pl.tile.add(s3_mean_sq, EPS)
                s3_variance = pl.tile.set_validshape(s3_variance, [BATCH, 1])
                s3_rms = pl.tile.sqrt(s3_variance)
                s3_rms = pl.tile.set_validshape(s3_rms, [BATCH, 1])
                s3_inv_rms = pl.tile.recip(s3_rms)
                s3_inv_rms = pl.tile.set_validshape(s3_inv_rms, [BATCH, 1])
                ...
                for kb in pl.range(HIDDEN // MM_N):
                    ...
                    pn_gamma = pl.tile.reshape(pn_gamma_blk, [1, VEC_W])
                    pn_gamma = pl.tile.set_validshape(pn_gamma, [1, VEC_W])
                    pn_x_scaled = pl.tile.row_expand_mul(pn_x, s3_inv_rms)
                    pn_normed = pl.tile.col_expand_mul(pn_x_scaled, pn_gamma)
                    pn_normed_bf16 = pl.tile.cast(pn_normed, dtype=pl.BF16)
                    pn_normed_bf16 = pl.tile.set_validshape(pn_normed_bf16, [BATCH, VEC_W])
                ...
                for nb in pl.range(INTERMEDIATE // MM_N):
                    ...
                    ml_p0 = pl.tile.reshape(ml_p0_blk, [BATCH, MM_K])
                    ml_p0 = pl.tile.set_validshape(ml_p0, [BATCH, MM_K])
                    ...
                    for kb in pl.range(1, HIDDEN // MM_K):
                        ...
                        ml_p = pl.tile.reshape(ml_p_blk, [BATCH, MM_K])
                        ml_p = pl.tile.set_validshape(ml_p, [BATCH, MM_K])
                    ...
                    ml_mlp_bf16 = pl.tile.cast(ml_mlp_chunk, dtype=pl.BF16)
                    ml_mlp_bf16 = pl.tile.set_validshape(ml_mlp_bf16, [BATCH, MM_N])
                ...
                for nb in pl.range(HIDDEN // MM_N):
                    ...
                    dn_m0 = pl.tile.reshape(dn_m0_blk, [BATCH, MM_K])
                    dn_m0 = pl.tile.set_validshape(dn_m0, [BATCH, MM_K])
                    ...
                    for kb in pl.range(1, INTERMEDIATE // MM_K):
                        ...
                        dn_m = pl.tile.reshape(dn_m_blk, [BATCH, MM_K])
                        dn_m = pl.tile.set_validshape(dn_m, [BATCH, MM_K])
                    ...
                    dn_out_bf16 = pl.tile.cast(dn_out_sum, dtype=pl.BF16)
                    dn_out_bf16 = pl.tile.set_validshape(dn_out_bf16, [BATCH, MM_N])
