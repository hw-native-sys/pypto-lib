...
# -----------------------------------------------------------------------------------------------------------
"""Qwen3-14B PyPTO paged-attention software pipeline.

The kernel uses 24 SPMD core groups. Each group runs a grid-stride task loop,
with five query rows split across the two AIV lanes as 2+3 valid rows.

The software pipeline originates from the tuned B16/S3584 attention benchmark.
Scratch is caller-owned, the launch remains fixed at 24 mixed-core groups, and
the inner task/page bounds follow the active batch and per-request sequence
lengths.
"""
...
def paged_attention_pypto_swpipe(  # noqa: PLR0913 -- fused Phase-0/attention API
    ...
    with pl.spmd(
        ...
        # Phase 0 mirrors the former fused CCE topology exactly: the first 32
        # physical AIV lanes run the original four-item Q/K-norm + RoPE schedule,
        # while the remaining 16 lanes and all AICs proceed directly to the
        # unconditional mixed-core barrier below.  The barrier replaces the
        # former rope-task completion dependency and publishes Q/K/V GM writes
        # before any PA MTE2 read can begin.
        ...
        for task in pl.range(core, num_tasks, ATTN_SPMD_BLOCKS):
            ...
            for aiv_id in pl.split_aiv(2, mode=pl.SplitMode.NONE):
                ...
                # The FIFO evolves (1, 1) -> (1, r0) -> (r0, r1), so each
                # delayed PV result uses the rescale from its own softmax stack.
