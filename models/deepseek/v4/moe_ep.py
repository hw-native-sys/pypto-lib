...
Importing this module has no side effects. Call ``make_demo()`` to apply the
EP demo preset and obtain the compiled entry points.
"""
...
def make_demo():
    """Apply EP demo preset and return (host_orch, moe_ep, golden_fn, build_specs).

    Mutates ``config.FLASH``/``EP_WORLD_SIZE``/``EP_ROUTING_GLOBAL``/``RECV_MAX``
    and imports sub-kernels — only on explicit call, never on ``import moe_ep``.
    """
    ...
    from hc_pre import hc_pre_inline as hc_pre
    from hc_post import hc_post_inline as hc_post
    from gate import gate_inline as gate
    from expert_shared import expert_shared_inline as expert_shared
    from expert_routed import expert_routed_inline as expert_routed
    ...
    return host_orch, moe_ep, golden_moe_ep, build_tensor_specs
...
if __name__ == "__main__":
    ...
    host_orch, _, golden_moe_ep, build_tensor_specs = make_demo()

    device_ids = [int(d) for d in args.device.split(",")]
    if len(device_ids) != 2:
        raise SystemExit(f"need exactly 2 devices, got {device_ids}")
