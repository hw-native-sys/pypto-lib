...
def compressor_step(
    ...
    """Decode one token per batch for the ratio-128 non-overlap compressor."""
    kv_proj = pl.create_tensor([B, OUT_DIM], dtype=pl.FP32)
    score_proj = pl.create_tensor([B, OUT_DIM], dtype=pl.FP32)
    for idx in pl.spmd(B * OUT_DIM // (B_TILE * OUT_TILE), name_hint="kv_score_proj_step"):
        global_row0 = (idx // (OUT_DIM // OUT_TILE)) * B_TILE
        o0 = (idx % (OUT_DIM // OUT_TILE)) * OUT_TILE
        kv_acc = pl.create_tensor([B_TILE, OUT_TILE], dtype=pl.FP32)
        score_acc = pl.create_tensor([B_TILE, OUT_TILE], dtype=pl.FP32)
        for kb in pl.pipeline(0, D // K_TILE, stage=2):
            k0 = kb * K_TILE
            x_tile = x[global_row0 : global_row0 + B_TILE, k0 : k0 + K_TILE]
            wkv_tile = wkv[k0 : k0 + K_TILE, o0 : o0 + OUT_TILE]
            wgate_tile = wgate[k0 : k0 + K_TILE, o0 : o0 + OUT_TILE]
            if k0 == 0:
                kv_acc = pl.matmul(x_tile, wkv_tile, out_dtype=pl.FP32)
        ...
        kv_proj[global_row0 : global_row0 + B_TILE, o0 : o0 + OUT_TILE] = kv_acc
        score_proj[global_row0 : global_row0 + B_TILE, o0 : o0 + OUT_TILE] = score_acc

    compress_state_flat = pl.reshape(compress_state, [COMPRESS_STATE_BLOCK_NUM, COMPRESS_STATE_BLOCK_SIZE * COMPRESS_STATE_DIM])
    with pl.at(level=pl.Level.CORE_GROUP, name_hint="state_scatter_step"):
        for b in pl.range(B):
            pos_b = pl.read(start_pos, [b])
            token_ape_row = pos_b % COMPRESS_RATIO
            state_logical_blk = pos_b // COMPRESS_STATE_BLOCK_SIZE
            state_intra = pos_b % COMPRESS_STATE_BLOCK_SIZE
            state_blk_id = pl.cast(pl.read(compress_state_block_table, [b, state_logical_blk]), target_type=pl.INDEX)
            slot_col0 = state_intra * COMPRESS_STATE_DIM
            ape_row = ape[token_ape_row : token_ape_row + 1, 0 : OUT_DIM]
            kv_row = kv_proj[b : b + 1, 0 : OUT_DIM]
            score_row = pl.add(score_proj[b : b + 1, 0 : OUT_DIM], ape_row)
            compress_state_flat[state_blk_id : state_blk_id + 1, slot_col0 : slot_col0 + OUT_DIM] = kv_row
            compress_state_flat[state_blk_id : state_blk_id + 1, slot_col0 + OUT_DIM : slot_col0 + 2 * OUT_DIM] = score_row

    pooled_kv = pl.create_tensor([B, HEAD_DIM], dtype=pl.FP32)
    for idx in pl.spmd(B * HEAD_DIM // HEAD_TILE, name_hint="softmax_pool_step"):
        global_c_idx = idx // (HEAD_DIM // HEAD_TILE)
        h0 = (idx % (HEAD_DIM // HEAD_TILE)) * HEAD_TILE
        pos_gate = pl.read(start_pos, [global_c_idx])
        if (pos_gate + 1) % COMPRESS_RATIO == 0:
            softmax_score_state = pl.create_tensor([STATE_LEN, HEAD_TILE], dtype=pl.FP32)
            softmax_kv_state = pl.create_tensor([STATE_LEN, HEAD_TILE], dtype=pl.FP32)
            for s in pl.pipeline(STATE_LEN, stage=2):
                state_pos = pos_gate - (COMPRESS_RATIO - 1) + s
                state_logical_blk = state_pos // COMPRESS_STATE_BLOCK_SIZE
                state_intra = state_pos % COMPRESS_STATE_BLOCK_SIZE
                state_blk_id = pl.cast(pl.read(compress_state_block_table, [global_c_idx, state_logical_blk]), target_type=pl.INDEX)
                kv_col0 = state_intra * COMPRESS_STATE_DIM + h0
    ...
    norm_w_2d = pl.reshape(norm_w, [1, HEAD_DIM])
    normed_kv = pl.create_tensor([B, HEAD_DIM], dtype=pl.FP32)
    for batch_base_idx in pl.spmd(B // RMS_TILE, name_hint="rmsnorm_rope_step"):
        batch_base = batch_base_idx * RMS_TILE
        cos_b = cos[batch_base : batch_base + RMS_TILE, 0 : ROPE_HEAD_DIM // 2]
        sin_b = sin[batch_base : batch_base + RMS_TILE, 0 : ROPE_HEAD_DIM // 2]
        partial_sq = pl.full([1, RMS_TILE], dtype=pl.FP32, value=0.0)
    ...
    cmp_kv_cache_flat = pl.reshape(cmp_kv_cache, [CMP_BLOCK_NUM * BLOCK_SIZE, HEAD_DIM])
    for batch_base_idx in pl.spmd(B // RMS_TILE, name_hint="kv_finalize_step"):
        batch_base = batch_base_idx * RMS_TILE
        for inner in pl.range(RMS_TILE):
            global_c_idx = batch_base + inner
            pos_b = pl.read(start_pos, [global_c_idx])
            if (pos_b + 1) % COMPRESS_RATIO == 0:
                kv_row = normed_kv[global_c_idx : global_c_idx + 1, 0 : HEAD_DIM]
                cache_col = pos_b // COMPRESS_RATIO
                cmp_logical_blk = cache_col // BLOCK_SIZE
                cmp_intra = cache_col % BLOCK_SIZE
                cmp_blk_id = pl.cast(pl.read(cmp_block_table, [global_c_idx, cmp_logical_blk]), target_type=pl.INDEX)
                phys_cmp_row = cmp_blk_id * BLOCK_SIZE + cmp_intra
    ...
    compress_state = pl.reshape(compress_state_flat, [COMPRESS_STATE_BLOCK_NUM, COMPRESS_STATE_BLOCK_SIZE, COMPRESS_STATE_DIM])
    cmp_kv_cache = pl.reshape(cmp_kv_cache_flat, [CMP_BLOCK_NUM, BLOCK_SIZE, 1, HEAD_DIM])
    return compress_state, cmp_kv_cache
...
def golden_compressor_step(tensors):
    """Torch reference for one-token decode compressor update."""
    import torch
    ...
    kv = x @ wkv
    score = x @ wgate
    should_compress_rows = torch.zeros(B, dtype=torch.bool, device=x.device)
    pooled = torch.zeros(B, 1, HEAD_DIM, dtype=torch.float32, device=x.device)

    for b in range(B):
        pos = int(start_pos_t[b].item())
        ape_row = pos % ratio
        score_b = score[b] + ape[ape_row]
        write_state_row(b, pos, kv[b], score_b)
        if (pos + 1) % ratio == 0:
            should_compress_rows[b] = True
            kv_rows = []
            score_rows = []
            for state_pos in range(pos - ratio + 1, pos + 1):
                kv_row, score_row = read_state_row(b, state_pos)
                kv_rows.append(kv_row)
                score_rows.append(score_row)
            kv_state = torch.stack(kv_rows, dim=0).unsqueeze(0)
            score_state = torch.stack(score_rows, dim=0).unsqueeze(0)
    ...
    for b in range(B):
        ...
        intra_offset = cache_col % BLOCK_SIZE
        cblk = int(cmp_block_table[b, logical_blk].item())
        cmp_kv_cache[cblk, intra_offset, 0] = kv_b[0, 0].to(torch.bfloat16)
