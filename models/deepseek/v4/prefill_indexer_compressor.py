...
def prefill_indexer_compressor(
    ...
    compress_state_flat = pl.reshape(compress_state, [INNER_STATE_BLOCK_NUM * INNER_STATE_BLOCK_SIZE, COMPRESS_STATE_DIM])
    ...
    for proj_idx in pl.spmd(PACKED_PROJ_BLOCKS, name_hint="prefill_idx_c4_kv_score_proj"):
        ...
        for kb in pl.pipeline(0, D // K_TILE, stage=2):
            ...
            # compressor_ratio4 prefill mode and the decode indexer compressor.
        ...
        if write_slot_raw >= 0:
            ...
            for pool_s in pl.range(COMPRESS_RATIO):
                ...
                if write_pos >= 2 * COMPRESS_RATIO - 1:
                    ...
                    prev_phys_block_raw = pl.read(inner_compress_state_block_table, [0, prev_state_block])
                ...
                cur_phys_block_raw = pl.read(inner_compress_state_block_table, [0, cur_state_block])
    ...
    for update_idx in pl.spmd(T * PACKED_PROJ_BLOCKS, name_hint="prefill_idx_c4_state_update"):
        ...
        if update_t < num_tokens:
            ...
            if state_row_raw >= 0:
                ...
                compress_state_flat[state_row : state_row + 1, OUT_DIM + update_o0 : OUT_DIM + update_o0 + OUT_TILE] = pl.add(
                    pl.add(
    ...
    compress_state = pl.reshape(compress_state_flat, [INNER_STATE_BLOCK_NUM, INNER_STATE_BLOCK_SIZE, COMPRESS_STATE_DIM])
...
def prefill_indexer_compressor_test(
    ...
    compress_state: pl.InOut[pl.Tensor[[INNER_STATE_BLOCK_NUM, INNER_STATE_BLOCK_SIZE, COMPRESS_STATE_DIM], pl.FP32]],
    inner_compress_state_block_table: pl.Tensor[[B, INNER_STATE_MAX_BLOCKS], pl.INT32],
    wkv: pl.Tensor[[OUT_DIM, D], pl.BF16],
    wgate: pl.Tensor[[OUT_DIM, D], pl.BF16],
    ape: pl.Tensor[[COMPRESS_RATIO, OUT_DIM], pl.FP32],
...
def golden_prefill_indexer_compressor(tensors):
    ...
    compress_state_flat = tensors["compress_state"].view(INNER_STATE_BLOCK_NUM * INNER_STATE_BLOCK_SIZE, COMPRESS_STATE_DIM)
...
def build_tensor_specs(start_pos: int = START_POS):
    ...
    def init_inner_compress_state_block_table():
        table = torch.full((B, INNER_STATE_MAX_BLOCKS), -1, dtype=torch.int32)
        for block in range(INNER_STATE_MAX_BLOCKS):
            table[0, block] = (block * 17 + 3) % INNER_STATE_MAX_BLOCKS
        return table
    def state_row(abs_pos):
        if abs_pos < 0 or abs_pos >= MAX_SEQ_LEN:
            return -1
        ...
        block = abs_pos // INNER_STATE_BLOCK_SIZE
        intra = abs_pos % INNER_STATE_BLOCK_SIZE
        return int(table[0, block].item()) * INNER_STATE_BLOCK_SIZE + intra
    def init_x():
    ...
    def idx_row(cmp_slot):
        ...
        phys_block = int(table[0, block].item())
        if phys_block < 0:
            return -1
