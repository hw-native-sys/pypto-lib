...
def decode_fwd(
    ...
    hc_attn_fn_l0 = pl.slice(hc_attn_fn, [MIX_HC, HC_DIM], [0 * MIX_HC, 0])
    hc_attn_scale_l0 = pl.slice(hc_attn_scale, [3], [0 * 3])
    hc_attn_base_l0 = pl.slice(hc_attn_base, [MIX_HC], [0 * MIX_HC])
    attn_norm_w_l0 = pl.slice(attn_norm_w, [D], [0 * D])
    wq_a_l0 = pl.slice(wq_a, [D, Q_LORA], [0 * D, 0])
    wq_b_l0 = pl.slice(wq_b, [Q_LORA, H * HEAD_DIM], [0 * Q_LORA, 0])
    wq_b_scale_l0 = pl.slice(wq_b_scale, [H * HEAD_DIM], [0 * H * HEAD_DIM])
    wkv_l0 = pl.slice(wkv, [D, HEAD_DIM], [0 * D, 0])
    gamma_cq_l0 = pl.slice(gamma_cq, [Q_LORA], [0 * Q_LORA])
    gamma_ckv_l0 = pl.slice(gamma_ckv, [HEAD_DIM], [0 * HEAD_DIM])
    kv_cache_l0 = pl.slice(kv_cache, [B * ORI_MAX_BLOCKS, BLOCK_SIZE, 1, HEAD_DIM], [0 * B * ORI_MAX_BLOCKS, 0, 0, 0])
    attn_sink_l0 = pl.slice(attn_sink, [H], [0 * H])
    wo_a_l0 = pl.slice(wo_a, [O_GROUPS, O_LORA, O_GROUP_IN], [0 * O_GROUPS, 0, 0])
    wo_b_l0 = pl.slice(wo_b, [D, O_GROUPS * O_LORA], [0 * D, 0])
    wo_b_scale_l0 = pl.slice(wo_b_scale, [D], [0 * D])
    cmp_kv_l0 = pl.slice(cmp_kv, [CSA_CMP_BLOCK_NUM, BLOCK_SIZE, 1, HEAD_DIM], [0 * CSA_CMP_BLOCK_NUM, 0, 0, 0])
    x_attn0 = pl.create_tensor([T, HC_MULT, D], dtype=pl.BF16)
    with pl.scope():
        attention_swa(
            x_hc,
    ...
    hc_ffn_fn_l0 = pl.slice(hc_ffn_fn, [MIX_HC, HC_DIM], [0 * MIX_HC, 0])
    hc_ffn_scale_l0 = pl.slice(hc_ffn_scale, [3], [0 * 3])
    hc_ffn_base_l0 = pl.slice(hc_ffn_base, [MIX_HC], [0 * MIX_HC])
    norm_w_l0 = pl.slice(norm_w, [D], [0 * D])
    gate_w_l0 = pl.slice(gate_w, [N_EXPERTS_GLOBAL, D], [0 * N_EXPERTS_GLOBAL, 0])
    gate_bias_l0 = pl.slice(gate_bias, [N_EXPERTS_GLOBAL], [0 * N_EXPERTS_GLOBAL])
    tid2eid_l0 = pl.slice(tid2eid, [VOCAB, TOPK], [0 * VOCAB, 0])
    routed_w1_l0 = pl.slice(routed_w1, [N_LOCAL, MOE_INTER, D], [0 * N_LOCAL, 0, 0])
    routed_w1_scale_l0 = pl.slice(routed_w1_scale, [N_LOCAL, MOE_INTER], [0 * N_LOCAL, 0])
    routed_w3_l0 = pl.slice(routed_w3, [N_LOCAL, MOE_INTER, D], [0 * N_LOCAL, 0, 0])
    routed_w3_scale_l0 = pl.slice(routed_w3_scale, [N_LOCAL, MOE_INTER], [0 * N_LOCAL, 0])
    routed_w2_l0 = pl.slice(routed_w2, [N_LOCAL, D, MOE_INTER], [0 * N_LOCAL, 0, 0])
    routed_w2_scale_l0 = pl.slice(routed_w2_scale, [N_LOCAL, D], [0 * N_LOCAL, 0])
    shared_w1_l0 = pl.slice(shared_w1, [MOE_INTER, D], [0 * MOE_INTER, 0])
    shared_w1_scale_l0 = pl.slice(shared_w1_scale, [MOE_INTER], [0 * MOE_INTER])
    shared_w3_l0 = pl.slice(shared_w3, [MOE_INTER, D], [0 * MOE_INTER, 0])
    shared_w3_scale_l0 = pl.slice(shared_w3_scale, [MOE_INTER], [0 * MOE_INTER])
    shared_w2_l0 = pl.slice(shared_w2, [D, MOE_INTER], [0 * D, 0])
    ...
    hc_attn_fn_l1 = pl.slice(hc_attn_fn, [MIX_HC, HC_DIM], [1 * MIX_HC, 0])
    hc_attn_scale_l1 = pl.slice(hc_attn_scale, [3], [1 * 3])
    hc_attn_base_l1 = pl.slice(hc_attn_base, [MIX_HC], [1 * MIX_HC])
    attn_norm_w_l1 = pl.slice(attn_norm_w, [D], [1 * D])
    wq_a_l1 = pl.slice(wq_a, [D, Q_LORA], [1 * D, 0])
    wq_b_l1 = pl.slice(wq_b, [Q_LORA, H * HEAD_DIM], [1 * Q_LORA, 0])
    wq_b_scale_l1 = pl.slice(wq_b_scale, [H * HEAD_DIM], [1 * H * HEAD_DIM])
    wkv_l1 = pl.slice(wkv, [D, HEAD_DIM], [1 * D, 0])
    gamma_cq_l1 = pl.slice(gamma_cq, [Q_LORA], [1 * Q_LORA])
    gamma_ckv_l1 = pl.slice(gamma_ckv, [HEAD_DIM], [1 * HEAD_DIM])
    kv_cache_l1 = pl.slice(kv_cache, [B * ORI_MAX_BLOCKS, BLOCK_SIZE, 1, HEAD_DIM], [1 * B * ORI_MAX_BLOCKS, 0, 0, 0])
    attn_sink_l1 = pl.slice(attn_sink, [H], [1 * H])
    wo_a_l1 = pl.slice(wo_a, [O_GROUPS, O_LORA, O_GROUP_IN], [1 * O_GROUPS, 0, 0])
    wo_b_l1 = pl.slice(wo_b, [D, O_GROUPS * O_LORA], [1 * D, 0])
    wo_b_scale_l1 = pl.slice(wo_b_scale, [D], [1 * D])
    cmp_kv_l1 = pl.slice(cmp_kv, [CSA_CMP_BLOCK_NUM, BLOCK_SIZE, 1, HEAD_DIM], [1 * CSA_CMP_BLOCK_NUM, 0, 0, 0])
    x_attn1 = pl.create_tensor([T, HC_MULT, D], dtype=pl.BF16)
    with pl.scope():
        attention_swa(
            hidden,
    ...
    hc_ffn_fn_l1 = pl.slice(hc_ffn_fn, [MIX_HC, HC_DIM], [1 * MIX_HC, 0])
    hc_ffn_scale_l1 = pl.slice(hc_ffn_scale, [3], [1 * 3])
    hc_ffn_base_l1 = pl.slice(hc_ffn_base, [MIX_HC], [1 * MIX_HC])
    norm_w_l1 = pl.slice(norm_w, [D], [1 * D])
    gate_w_l1 = pl.slice(gate_w, [N_EXPERTS_GLOBAL, D], [1 * N_EXPERTS_GLOBAL, 0])
    gate_bias_l1 = pl.slice(gate_bias, [N_EXPERTS_GLOBAL], [1 * N_EXPERTS_GLOBAL])
    tid2eid_l1 = pl.slice(tid2eid, [VOCAB, TOPK], [1 * VOCAB, 0])
    routed_w1_l1 = pl.slice(routed_w1, [N_LOCAL, MOE_INTER, D], [1 * N_LOCAL, 0, 0])
    routed_w1_scale_l1 = pl.slice(routed_w1_scale, [N_LOCAL, MOE_INTER], [1 * N_LOCAL, 0])
    routed_w3_l1 = pl.slice(routed_w3, [N_LOCAL, MOE_INTER, D], [1 * N_LOCAL, 0, 0])
    routed_w3_scale_l1 = pl.slice(routed_w3_scale, [N_LOCAL, MOE_INTER], [1 * N_LOCAL, 0])
    routed_w2_l1 = pl.slice(routed_w2, [N_LOCAL, D, MOE_INTER], [1 * N_LOCAL, 0, 0])
    routed_w2_scale_l1 = pl.slice(routed_w2_scale, [N_LOCAL, D], [1 * N_LOCAL, 0])
    shared_w1_l1 = pl.slice(shared_w1, [MOE_INTER, D], [1 * MOE_INTER, 0])
    shared_w1_scale_l1 = pl.slice(shared_w1_scale, [MOE_INTER], [1 * MOE_INTER])
    shared_w3_l1 = pl.slice(shared_w3, [MOE_INTER, D], [1 * MOE_INTER, 0])
    shared_w3_scale_l1 = pl.slice(shared_w3_scale, [MOE_INTER], [1 * MOE_INTER])
    ...
    for loop_i in pl.range(HCA_NUM_LAYERS):
        csa_layer = loop_i * 2 + 2
        hca_layer = loop_i * 2 + 3
        csa_moe_epoch: pl.Scalar[pl.INT32] = loop_i * 2 + 3
        hca_moe_epoch: pl.Scalar[pl.INT32] = loop_i * 2 + 4

        x_attn_csa = pl.create_tensor([T, HC_MULT, D], dtype=pl.BF16)
        hc_attn_fn_csa = pl.slice(hc_attn_fn, [MIX_HC, HC_DIM], [csa_layer * MIX_HC, 0])
        hc_attn_scale_csa = pl.slice(hc_attn_scale, [3], [csa_layer * 3])
        hc_attn_base_csa = pl.slice(hc_attn_base, [MIX_HC], [csa_layer * MIX_HC])
        attn_norm_w_csa = pl.slice(attn_norm_w, [D], [csa_layer * D])
        wq_a_csa = pl.slice(wq_a, [D, Q_LORA], [csa_layer * D, 0])
        wq_b_csa = pl.slice(wq_b, [Q_LORA, H * HEAD_DIM], [csa_layer * Q_LORA, 0])
        wq_b_scale_csa = pl.slice(wq_b_scale, [H * HEAD_DIM], [csa_layer * H * HEAD_DIM])
        wkv_csa = pl.slice(wkv, [D, HEAD_DIM], [csa_layer * D, 0])
        gamma_cq_csa = pl.slice(gamma_cq, [Q_LORA], [csa_layer * Q_LORA])
        gamma_ckv_csa = pl.slice(gamma_ckv, [HEAD_DIM], [csa_layer * HEAD_DIM])
        kv_cache_csa = pl.slice(kv_cache, [B * ORI_MAX_BLOCKS, BLOCK_SIZE, 1, HEAD_DIM], [csa_layer * B * ORI_MAX_BLOCKS, 0, 0, 0])
        attn_sink_csa = pl.slice(attn_sink, [H], [csa_layer * H])
        wo_a_csa = pl.slice(wo_a, [O_GROUPS, O_LORA, O_GROUP_IN], [csa_layer * O_GROUPS, 0, 0])
        wo_b_csa = pl.slice(wo_b, [D, O_GROUPS * O_LORA], [csa_layer * D, 0])
        wo_b_scale_csa = pl.slice(wo_b_scale, [D], [csa_layer * D])
        csa_cmp_wkv_csa = pl.slice(csa_cmp_wkv, [D, CSA_MAIN_OUT_DIM], [loop_i * D, 0])
        csa_cmp_wgate_csa = pl.slice(csa_cmp_wgate, [D, CSA_MAIN_OUT_DIM], [loop_i * D, 0])
        csa_cmp_ape_csa = pl.slice(csa_cmp_ape, [CSA_COMPRESS_RATIO, CSA_MAIN_OUT_DIM], [loop_i * CSA_COMPRESS_RATIO, 0])
        csa_cmp_norm_w_csa = pl.slice(csa_cmp_norm_w, [HEAD_DIM], [loop_i * HEAD_DIM])
        csa_compress_state_csa = pl.slice(csa_compress_state, [CSA_MAIN_STATE_BLOCK_NUM, CSA_MAIN_STATE_BLOCK_SIZE, CSA_MAIN_STATE_DIM], [loop_i * CSA_MAIN_STATE_BLOCK_NUM, 0, 0])
        csa_idx_wq_b_csa = pl.slice(csa_idx_wq_b, [Q_LORA, CSA_IDX_N_HEADS * CSA_IDX_HEAD_DIM], [loop_i * Q_LORA, 0])
        csa_idx_wq_b_scale_csa = pl.slice(csa_idx_wq_b_scale, [CSA_IDX_N_HEADS * CSA_IDX_HEAD_DIM], [loop_i * CSA_IDX_N_HEADS * CSA_IDX_HEAD_DIM])
        csa_weights_proj_csa = pl.slice(csa_weights_proj, [D, CSA_IDX_N_HEADS], [loop_i * D, 0])
        csa_hadamard_idx_csa = pl.slice(csa_hadamard_idx, [CSA_IDX_HEAD_DIM, CSA_IDX_HEAD_DIM], [loop_i * CSA_IDX_HEAD_DIM, 0])
        csa_inner_wkv_csa = pl.slice(csa_inner_wkv, [D, CSA_INNER_OUT_DIM], [loop_i * D, 0])
        csa_inner_wgate_csa = pl.slice(csa_inner_wgate, [D, CSA_INNER_OUT_DIM], [loop_i * D, 0])
        csa_inner_ape_csa = pl.slice(csa_inner_ape, [CSA_COMPRESS_RATIO, CSA_INNER_OUT_DIM], [loop_i * CSA_COMPRESS_RATIO, 0])
        csa_inner_norm_w_csa = pl.slice(csa_inner_norm_w, [CSA_IDX_HEAD_DIM], [loop_i * CSA_IDX_HEAD_DIM])
        csa_inner_compress_state_csa = pl.slice(csa_inner_compress_state, [CSA_INNER_STATE_BLOCK_NUM, CSA_INNER_STATE_BLOCK_SIZE, CSA_INNER_STATE_DIM], [loop_i * CSA_INNER_STATE_BLOCK_NUM, 0, 0])
        cmp_kv_csa = pl.slice(cmp_kv, [CSA_CMP_BLOCK_NUM, BLOCK_SIZE, 1, HEAD_DIM], [csa_layer * CSA_CMP_BLOCK_NUM, 0, 0, 0])
        idx_kv_cache_csa = pl.slice(idx_kv_cache, [CSA_IDX_CACHE_BLOCK_NUM, BLOCK_SIZE, 1, CSA_IDX_HEAD_DIM], [loop_i * CSA_IDX_CACHE_BLOCK_NUM, 0, 0, 0])
        with pl.scope():
            attention_csa(
                hidden,
        ...
        hidden_mid = pl.create_tensor([T, HC_MULT, D], dtype=pl.BF16)
        hc_ffn_fn_csa = pl.slice(hc_ffn_fn, [MIX_HC, HC_DIM], [csa_layer * MIX_HC, 0])
        hc_ffn_scale_csa = pl.slice(hc_ffn_scale, [3], [csa_layer * 3])
        hc_ffn_base_csa = pl.slice(hc_ffn_base, [MIX_HC], [csa_layer * MIX_HC])
        norm_w_csa = pl.slice(norm_w, [D], [csa_layer * D])
        gate_w_csa = pl.slice(gate_w, [N_EXPERTS_GLOBAL, D], [csa_layer * N_EXPERTS_GLOBAL, 0])
        gate_bias_csa = pl.slice(gate_bias, [N_EXPERTS_GLOBAL], [csa_layer * N_EXPERTS_GLOBAL])
        tid2eid_csa = pl.slice(tid2eid, [VOCAB, TOPK], [csa_layer * VOCAB, 0])
        routed_w1_csa = pl.slice(routed_w1, [N_LOCAL, MOE_INTER, D], [csa_layer * N_LOCAL, 0, 0])
        routed_w1_scale_csa = pl.slice(routed_w1_scale, [N_LOCAL, MOE_INTER], [csa_layer * N_LOCAL, 0])
        routed_w3_csa = pl.slice(routed_w3, [N_LOCAL, MOE_INTER, D], [csa_layer * N_LOCAL, 0, 0])
        routed_w3_scale_csa = pl.slice(routed_w3_scale, [N_LOCAL, MOE_INTER], [csa_layer * N_LOCAL, 0])
        routed_w2_csa = pl.slice(routed_w2, [N_LOCAL, D, MOE_INTER], [csa_layer * N_LOCAL, 0, 0])
        routed_w2_scale_csa = pl.slice(routed_w2_scale, [N_LOCAL, D], [csa_layer * N_LOCAL, 0])
        shared_w1_csa = pl.slice(shared_w1, [MOE_INTER, D], [csa_layer * MOE_INTER, 0])
        shared_w1_scale_csa = pl.slice(shared_w1_scale, [MOE_INTER], [csa_layer * MOE_INTER])
        shared_w3_csa = pl.slice(shared_w3, [MOE_INTER, D], [csa_layer * MOE_INTER, 0])
        shared_w3_scale_csa = pl.slice(shared_w3_scale, [MOE_INTER], [csa_layer * MOE_INTER])
        ...
        x_attn_hca = pl.create_tensor([T, HC_MULT, D], dtype=pl.BF16)
        hc_attn_fn_hca = pl.slice(hc_attn_fn, [MIX_HC, HC_DIM], [hca_layer * MIX_HC, 0])
        hc_attn_scale_hca = pl.slice(hc_attn_scale, [3], [hca_layer * 3])
        hc_attn_base_hca = pl.slice(hc_attn_base, [MIX_HC], [hca_layer * MIX_HC])
        attn_norm_w_hca = pl.slice(attn_norm_w, [D], [hca_layer * D])
        wq_a_hca = pl.slice(wq_a, [D, Q_LORA], [hca_layer * D, 0])
        wq_b_hca = pl.slice(wq_b, [Q_LORA, H * HEAD_DIM], [hca_layer * Q_LORA, 0])
        wq_b_scale_hca = pl.slice(wq_b_scale, [H * HEAD_DIM], [hca_layer * H * HEAD_DIM])
        wkv_hca = pl.slice(wkv, [D, HEAD_DIM], [hca_layer * D, 0])
        gamma_cq_hca = pl.slice(gamma_cq, [Q_LORA], [hca_layer * Q_LORA])
        gamma_ckv_hca = pl.slice(gamma_ckv, [HEAD_DIM], [hca_layer * HEAD_DIM])
        kv_cache_hca = pl.slice(kv_cache, [B * ORI_MAX_BLOCKS, BLOCK_SIZE, 1, HEAD_DIM], [hca_layer * B * ORI_MAX_BLOCKS, 0, 0, 0])
        attn_sink_hca = pl.slice(attn_sink, [H], [hca_layer * H])
        wo_a_hca = pl.slice(wo_a, [O_GROUPS, O_LORA, O_GROUP_IN], [hca_layer * O_GROUPS, 0, 0])
        wo_b_hca = pl.slice(wo_b, [D, O_GROUPS * O_LORA], [hca_layer * D, 0])
        wo_b_scale_hca = pl.slice(wo_b_scale, [D], [hca_layer * D])
        hca_cmp_wkv_hca = pl.slice(hca_cmp_wkv, [D, HCA_MAIN_OUT_DIM], [loop_i * D, 0])
        hca_cmp_wgate_hca = pl.slice(hca_cmp_wgate, [D, HCA_MAIN_OUT_DIM], [loop_i * D, 0])
        hca_cmp_ape_hca = pl.slice(hca_cmp_ape, [HCA_COMPRESS_RATIO, HCA_MAIN_OUT_DIM], [loop_i * HCA_COMPRESS_RATIO, 0])
        hca_cmp_norm_w_hca = pl.slice(hca_cmp_norm_w, [HEAD_DIM], [loop_i * HEAD_DIM])
        hca_compress_state_hca = pl.slice(hca_compress_state, [HCA_COMPRESS_STATE_BLOCK_NUM, HCA_COMPRESS_STATE_BLOCK_SIZE, HCA_COMPRESS_STATE_DIM], [loop_i * HCA_COMPRESS_STATE_BLOCK_NUM, 0, 0])
        cmp_kv_hca = pl.slice(cmp_kv, [CSA_CMP_BLOCK_NUM, BLOCK_SIZE, 1, HEAD_DIM], [hca_layer * CSA_CMP_BLOCK_NUM, 0, 0, 0])
        with pl.scope():
            attention_hca(
                hidden_mid,
        ...
        hc_ffn_fn_hca = pl.slice(hc_ffn_fn, [MIX_HC, HC_DIM], [hca_layer * MIX_HC, 0])
        hc_ffn_scale_hca = pl.slice(hc_ffn_scale, [3], [hca_layer * 3])
        hc_ffn_base_hca = pl.slice(hc_ffn_base, [MIX_HC], [hca_layer * MIX_HC])
        norm_w_hca = pl.slice(norm_w, [D], [hca_layer * D])
        gate_w_hca = pl.slice(gate_w, [N_EXPERTS_GLOBAL, D], [hca_layer * N_EXPERTS_GLOBAL, 0])
        gate_bias_hca = pl.slice(gate_bias, [N_EXPERTS_GLOBAL], [hca_layer * N_EXPERTS_GLOBAL])
        tid2eid_hca = pl.slice(tid2eid, [VOCAB, TOPK], [hca_layer * VOCAB, 0])
        routed_w1_hca = pl.slice(routed_w1, [N_LOCAL, MOE_INTER, D], [hca_layer * N_LOCAL, 0, 0])
        routed_w1_scale_hca = pl.slice(routed_w1_scale, [N_LOCAL, MOE_INTER], [hca_layer * N_LOCAL, 0])
        routed_w3_hca = pl.slice(routed_w3, [N_LOCAL, MOE_INTER, D], [hca_layer * N_LOCAL, 0, 0])
        routed_w3_scale_hca = pl.slice(routed_w3_scale, [N_LOCAL, MOE_INTER], [hca_layer * N_LOCAL, 0])
        routed_w2_hca = pl.slice(routed_w2, [N_LOCAL, D, MOE_INTER], [hca_layer * N_LOCAL, 0, 0])
        routed_w2_scale_hca = pl.slice(routed_w2_scale, [N_LOCAL, D], [hca_layer * N_LOCAL, 0])
        shared_w1_hca = pl.slice(shared_w1, [MOE_INTER, D], [hca_layer * MOE_INTER, 0])
        shared_w1_scale_hca = pl.slice(shared_w1_scale, [MOE_INTER], [hca_layer * MOE_INTER])
        shared_w3_hca = pl.slice(shared_w3, [MOE_INTER, D], [hca_layer * MOE_INTER, 0])
        shared_w3_scale_hca = pl.slice(shared_w3_scale, [MOE_INTER], [hca_layer * MOE_INTER])
    ...
    csa_layer_last = pl.const(CSA_LAST_LAYER, pl.INT32)
    last_moe_epoch: pl.Scalar[pl.INT32] = pl.const(2, pl.INT32) * HCA_NUM_LAYERS + 3

    x_attn_last = pl.create_tensor([T, HC_MULT, D], dtype=pl.BF16)
    hc_attn_fn_last = pl.slice(hc_attn_fn, [MIX_HC, HC_DIM], [csa_layer_last * MIX_HC, 0])
    hc_attn_scale_last = pl.slice(hc_attn_scale, [3], [csa_layer_last * 3])
    hc_attn_base_last = pl.slice(hc_attn_base, [MIX_HC], [csa_layer_last * MIX_HC])
    attn_norm_w_last = pl.slice(attn_norm_w, [D], [csa_layer_last * D])
    wq_a_last = pl.slice(wq_a, [D, Q_LORA], [csa_layer_last * D, 0])
    wq_b_last = pl.slice(wq_b, [Q_LORA, H * HEAD_DIM], [csa_layer_last * Q_LORA, 0])
    wq_b_scale_last = pl.slice(wq_b_scale, [H * HEAD_DIM], [csa_layer_last * H * HEAD_DIM])
    wkv_last = pl.slice(wkv, [D, HEAD_DIM], [csa_layer_last * D, 0])
    gamma_cq_last = pl.slice(gamma_cq, [Q_LORA], [csa_layer_last * Q_LORA])
    gamma_ckv_last = pl.slice(gamma_ckv, [HEAD_DIM], [csa_layer_last * HEAD_DIM])
    kv_cache_last = pl.slice(kv_cache, [B * ORI_MAX_BLOCKS, BLOCK_SIZE, 1, HEAD_DIM], [csa_layer_last * B * ORI_MAX_BLOCKS, 0, 0, 0])
    attn_sink_last = pl.slice(attn_sink, [H], [csa_layer_last * H])
    wo_a_last = pl.slice(wo_a, [O_GROUPS, O_LORA, O_GROUP_IN], [csa_layer_last * O_GROUPS, 0, 0])
    wo_b_last = pl.slice(wo_b, [D, O_GROUPS * O_LORA], [csa_layer_last * D, 0])
    wo_b_scale_last = pl.slice(wo_b_scale, [D], [csa_layer_last * D])
    csa_cmp_wkv_last = pl.slice(csa_cmp_wkv, [D, CSA_MAIN_OUT_DIM], [(CSA_NUM_LAYERS - 1) * D, 0])
    csa_cmp_wgate_last = pl.slice(csa_cmp_wgate, [D, CSA_MAIN_OUT_DIM], [(CSA_NUM_LAYERS - 1) * D, 0])
    csa_cmp_ape_last = pl.slice(csa_cmp_ape, [CSA_COMPRESS_RATIO, CSA_MAIN_OUT_DIM], [(CSA_NUM_LAYERS - 1) * CSA_COMPRESS_RATIO, 0])
    csa_cmp_norm_w_last = pl.slice(csa_cmp_norm_w, [HEAD_DIM], [(CSA_NUM_LAYERS - 1) * HEAD_DIM])
    csa_compress_state_last = pl.slice(csa_compress_state, [CSA_MAIN_STATE_BLOCK_NUM, CSA_MAIN_STATE_BLOCK_SIZE, CSA_MAIN_STATE_DIM], [(CSA_NUM_LAYERS - 1) * CSA_MAIN_STATE_BLOCK_NUM, 0, 0])
    csa_idx_wq_b_last = pl.slice(csa_idx_wq_b, [Q_LORA, CSA_IDX_N_HEADS * CSA_IDX_HEAD_DIM], [(CSA_NUM_LAYERS - 1) * Q_LORA, 0])
    csa_idx_wq_b_scale_last = pl.slice(csa_idx_wq_b_scale, [CSA_IDX_N_HEADS * CSA_IDX_HEAD_DIM], [(CSA_NUM_LAYERS - 1) * CSA_IDX_N_HEADS * CSA_IDX_HEAD_DIM])
    csa_weights_proj_last = pl.slice(csa_weights_proj, [D, CSA_IDX_N_HEADS], [(CSA_NUM_LAYERS - 1) * D, 0])
    csa_hadamard_idx_last = pl.slice(csa_hadamard_idx, [CSA_IDX_HEAD_DIM, CSA_IDX_HEAD_DIM], [(CSA_NUM_LAYERS - 1) * CSA_IDX_HEAD_DIM, 0])
    csa_inner_wkv_last = pl.slice(csa_inner_wkv, [D, CSA_INNER_OUT_DIM], [(CSA_NUM_LAYERS - 1) * D, 0])
    csa_inner_wgate_last = pl.slice(csa_inner_wgate, [D, CSA_INNER_OUT_DIM], [(CSA_NUM_LAYERS - 1) * D, 0])
    csa_inner_ape_last = pl.slice(csa_inner_ape, [CSA_COMPRESS_RATIO, CSA_INNER_OUT_DIM], [(CSA_NUM_LAYERS - 1) * CSA_COMPRESS_RATIO, 0])
    csa_inner_norm_w_last = pl.slice(csa_inner_norm_w, [CSA_IDX_HEAD_DIM], [(CSA_NUM_LAYERS - 1) * CSA_IDX_HEAD_DIM])
    csa_inner_compress_state_last = pl.slice(csa_inner_compress_state, [CSA_INNER_STATE_BLOCK_NUM, CSA_INNER_STATE_BLOCK_SIZE, CSA_INNER_STATE_DIM], [(CSA_NUM_LAYERS - 1) * CSA_INNER_STATE_BLOCK_NUM, 0, 0])
    cmp_kv_last = pl.slice(cmp_kv, [CSA_CMP_BLOCK_NUM, BLOCK_SIZE, 1, HEAD_DIM], [csa_layer_last * CSA_CMP_BLOCK_NUM, 0, 0, 0])
    idx_kv_cache_last = pl.slice(idx_kv_cache, [CSA_IDX_CACHE_BLOCK_NUM, BLOCK_SIZE, 1, CSA_IDX_HEAD_DIM], [(CSA_NUM_LAYERS - 1) * CSA_IDX_CACHE_BLOCK_NUM, 0, 0, 0])
    with pl.scope():
        attention_csa(
            hidden,
    ...
    x_next = pl.create_tensor([T, HC_MULT, D], dtype=pl.BF16)
    hc_ffn_fn_last = pl.slice(hc_ffn_fn, [MIX_HC, HC_DIM], [csa_layer_last * MIX_HC, 0])
    hc_ffn_scale_last = pl.slice(hc_ffn_scale, [3], [csa_layer_last * 3])
    hc_ffn_base_last = pl.slice(hc_ffn_base, [MIX_HC], [csa_layer_last * MIX_HC])
    norm_w_last = pl.slice(norm_w, [D], [csa_layer_last * D])
    gate_w_last = pl.slice(gate_w, [N_EXPERTS_GLOBAL, D], [csa_layer_last * N_EXPERTS_GLOBAL, 0])
    gate_bias_last = pl.slice(gate_bias, [N_EXPERTS_GLOBAL], [csa_layer_last * N_EXPERTS_GLOBAL])
    tid2eid_last = pl.slice(tid2eid, [VOCAB, TOPK], [csa_layer_last * VOCAB, 0])
    routed_w1_last = pl.slice(routed_w1, [N_LOCAL, MOE_INTER, D], [csa_layer_last * N_LOCAL, 0, 0])
    routed_w1_scale_last = pl.slice(routed_w1_scale, [N_LOCAL, MOE_INTER], [csa_layer_last * N_LOCAL, 0])
    routed_w3_last = pl.slice(routed_w3, [N_LOCAL, MOE_INTER, D], [csa_layer_last * N_LOCAL, 0, 0])
    routed_w3_scale_last = pl.slice(routed_w3_scale, [N_LOCAL, MOE_INTER], [csa_layer_last * N_LOCAL, 0])
    routed_w2_last = pl.slice(routed_w2, [N_LOCAL, D, MOE_INTER], [csa_layer_last * N_LOCAL, 0, 0])
    routed_w2_scale_last = pl.slice(routed_w2_scale, [N_LOCAL, D], [csa_layer_last * N_LOCAL, 0])
    shared_w1_last = pl.slice(shared_w1, [MOE_INTER, D], [csa_layer_last * MOE_INTER, 0])
    shared_w1_scale_last = pl.slice(shared_w1_scale, [MOE_INTER], [csa_layer_last * MOE_INTER])
    shared_w3_last = pl.slice(shared_w3, [MOE_INTER, D], [csa_layer_last * MOE_INTER, 0])
    shared_w3_scale_last = pl.slice(shared_w3_scale, [MOE_INTER], [csa_layer_last * MOE_INTER])
