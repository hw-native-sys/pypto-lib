...
IDX_CACHE_BLOCKS = CSA_CMP_BLOCK_NUM
...
def prefill_layer_core(
    ...
    kv_cache: pl.Out[pl.Tensor[[PREFILL_ORI_CACHE_BLOCKS_DYN, BLOCK_SIZE, 1, HEAD_DIM], pl.BF16]],
    ori_block_table: pl.Tensor[[PREFILL_ORI_BLOCK_TABLE_DYN], pl.INT32],
    ori_slot_mapping: pl.Tensor[[PREFILL_TOKENS_DYN], pl.INT64],
    cmp_kv: pl.Out[pl.Tensor[[PREFILL_CMP_CACHE_BLOCKS_DYN, BLOCK_SIZE, 1, HEAD_DIM], pl.BF16]],
    ...
    idx_kv_cache: pl.Out[pl.Tensor[[PREFILL_IDX_CACHE_BLOCKS_DYN, BLOCK_SIZE, 1, IDX_HEAD_DIM], pl.BF16]],
    ...
    for request_id in pl.range(user_batch):
        ...
        for tile_id in pl.range(tok_blocks):
            ...
            # Scatter the tile back into the packed output. Chunk lengths are
            # multiples of T (no partial tiles), so the full-T write never
            # overruns the request's packed region.
...
def _iter_request_tiles(seq_lens_v, chunk_lens_v, chunk_offsets_v):
    """Yield (request_id, tile_id, context_len, valid_tok, tile_base) per tile."""
...
def _packed_token_metadata(kind, seq_lens_v, chunk_lens_v, chunk_offsets_v, total_tokens, torch):
    """Assemble the rank-shared packed [total_tokens, ...] metadata tensors."""
    ...
    for _r, _tid, ctx, valid, base in _iter_request_tiles(seq_lens_v, chunk_lens_v, chunk_offsets_v):
        ...
        sparse_idx[base:base + valid] = m["cmp_sparse_indices"][:valid]
        sparse_lens[base:base + valid] = m["cmp_sparse_lens"][:valid]
        if kind == "hca":
...
def _resolve_batch(chunk_lens, start_positions, torch):
    """Normalize batch config into (seq_lens, chunk_lens, chunk_offsets, tile_offsets)."""
    chunk_lens_v = [int(c) for c in chunk_lens]
    for c in chunk_lens_v:
        if c <= 0 or c % T != 0:
            raise ValueError(f"chunk_lens must be positive multiples of T={T}, got {chunk_lens_v}")
...
def build_tensor_specs(layer_id=2, chunk_lens=(T, 2 * T), start_positions=None):
    """Packed batch tensor specs for the chunked prefill layer.

    ``chunk_lens`` lists the current-chunk length per request (multiples of ``T``);
    ``start_positions`` the prior context length per request (default 0 = fresh
    prefill, no cache history). Token tensors are packed to ``sum(chunk_lens)``;
    cache/state/tables are ``user_batch``-concatenated request-local slices.
    """
    import torch
    ...
    def init_x_hc():
        return ((torch.rand(N_RANKS, total_tokens, HC_MULT, D) - 0.5) / 10.0).to(torch.bfloat16)

    def init_input_ids():
        rows = [torch.roll(torch.arange(total_tokens, dtype=torch.int64) % VOCAB, shifts=r)
...
def golden_prefill_layer(tensors):
    ...
    def tile_buffer(packed_per_rank, rank, base, valid, feature_shape, dtype):
        buf = torch.zeros((T, *feature_shape), dtype=dtype)
        buf[:valid] = packed_per_rank[rank, base:base + valid]
        return buf
...
def valid_ratio_reldiff(diff_thd, pct_thd):
    """Relative-diff comparator over all packed token rows (every row is valid)."""
...
if __name__ == "__main__":
    ...
    parser.add_argument("--chunk-lens", type=str, default=f"{T},{2 * T}",
                        help="Comma-separated per-request chunk lengths (multiples of T).")
    ...
    result = run_jit(
        fn=l3_prefill_layer,
        specs=build_tensor_specs(layer_id=args.layer_id, chunk_lens=chunk_lens),
