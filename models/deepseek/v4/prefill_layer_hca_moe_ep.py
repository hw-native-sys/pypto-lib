...
# -----------------------------------------------------------------------------------------------------------
# ci: devices=2
"""DeepSeek-V4 packed prefill HCA single layer with MoE EP2."""
...
# `--hca-case basic128 --layer-id 3` overflows the default RECV_MAX=96;
# run it with `DSV4_MOE_EP_RECV_MAX=128` if that conditional check is needed.
...
from prefill_attention_hca import (
    ...
    SPARSE_TOPK,
    START_POS,
    _prefill_hca_cache_writeback_overlay,
    _resolve_hca_case,
    build_tensor_specs as build_attention_tensor_specs,
    golden_prefill_attention_hca,
)
from prefill_compressor_ratio128 import prefill_compressor_ratio128
...
assert MAX_TOKENS == T, "prefill HCA and MoE EP must agree on token-major T"


@pl.jit
def prefill_layer_hca_moe_ep(
    ...
    _prefill_hca_cache_writeback_overlay(kv, kv_cache, ori_slot_mapping, attn_out, num_tokens)
...
@pl.jit.host
def l3_prefill_layer_hca_moe_ep(
    ...
    cmp_kv_state: pl.Tensor[[N_RANKS, HCA_STATE_BLOCK_NUM, HCA_STATE_BLOCK_SIZE, MAIN_OUT_DIM], pl.FP32],
    cmp_score_state: pl.Tensor[[N_RANKS, HCA_STATE_BLOCK_NUM, HCA_STATE_BLOCK_SIZE, MAIN_OUT_DIM], pl.FP32],
    compress_state_block_table: pl.Tensor[[N_RANKS, MAX_REQS, HCA_STATE_MAX_BLOCKS], pl.INT32],
    kv_cache: pl.Out[pl.Tensor[[N_RANKS, HCA_ORI_BLOCK_NUM, BLOCK_SIZE, 1, HEAD_DIM], pl.BF16]],
    ...
    cmp_kv: pl.Out[pl.Tensor[[N_RANKS, HCA_CMP_BLOCK_NUM, BLOCK_SIZE, 1, HEAD_DIM], pl.BF16]],
    ...
    for rank in pl.range(pld.world_size()):
        ...
        prefill_layer_hca_moe_ep(
            ...
            kv_cache[rank], ori_slot_mapping[rank], ori_block_table[rank],
...
def build_tensor_specs(start_pos=START_POS, num_tokens=MAX_TOKENS, hca_case="custom",
                       hetero_smoke=False, hetero_boundary=False,
                       hetero_mixed_cmp_pos=False, layer_id=0):
    import torch
    ...
    attention_specs = build_attention_tensor_specs(
        ...
        hetero_mixed_cmp_pos=hetero_mixed_cmp_pos,
    )
    moe_specs = build_moe_tensor_specs(layer_id=layer_id)
    specs = build_ranked_layer_specs(
        attention_specs,
        moe_specs,
        N_RANKS,
        [N_RANKS, T, HC_MULT, D],
...
def golden_prefill_layer_hca_moe_ep(tensors):
    import torch
    from golden import TensorSpec
...
if __name__ == "__main__":
    ...
    parser.add_argument("--start-pos", type=int, default=START_POS,
                        help="Fixture-only prefix length for --hca-case=custom.")
    parser.add_argument("--num-tokens", type=int, default=MAX_TOKENS,
                        help="Fixture active token count for --hca-case=custom.")
    parser.add_argument("--hca-case", type=str, default="custom", choices=HCA_CASES)
    ...
    parser.add_argument("--hetero-mixed-cmp-pos", action="store_true", default=False)
    ...
    result = run_jit(
        fn=l3_prefill_layer_hca_moe_ep,
        specs=build_tensor_specs(
            start_pos=args.start_pos,
            num_tokens=args.num_tokens,
            hca_case=args.hca_case,
            hetero_smoke=args.hetero_smoke,
            hetero_boundary=args.hetero_boundary,
            hetero_mixed_cmp_pos=args.hetero_mixed_cmp_pos,
            layer_id=args.layer_id,
        ),
        golden_fn=golden_prefill_layer_hca_moe_ep,
        ...
        compare_fn={
            ...
            "kv_cache": ratio_allclose(atol=3e-3, rtol=1e-2),
