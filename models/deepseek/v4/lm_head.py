...
VOCAB_BLOCKS_FULL = VOCAB // VOCAB_CHUNK
...
def _build_tp_lm_head_program(tp_size: int):
    ...
    class TpLmHead:
        ...
        def gather_step(
            ...
            # logits window. Rank-local and peer-local outputs then share the
            # same [T, VOCAB] layout.
            ...
            for t0 in pl.range(0, T, T_TILE):
                for ob in pl.range(VOCAB_BLOCKS_FULL):
                    o0 = ob * VOCAB_CHUNK
                    tile = pl.load(logits_window, [t0, o0], [T_TILE, VOCAB_CHUNK])
                    pl.store(tile, [t0, o0], logits)
            return logits
