...
def build_tensor_specs(
    ...
    *,
    ...
    base_specs = decode_mtp.build_tensor_specs(
        start_pos=start_pos,
    ...
    ordered_names = [
        ...
        "hidden_out", "next_pre_hc_hidden", "logits",
        "num_tokens", "routing_mode",
    ]
    specs = []
    for name in ordered_names:
        if name in prepared_specs:
            ...
            raise ValueError(f"missing base TensorSpec for MTP-core parameter {name!r}")
    return specs
...
def golden_mtp_decode_core(tensors):
    import torch
    ...
    moe_tensors["layer_id"] = resolve_routing_layer_id(MTP_LAYER_ID, int(tensors["routing_mode"]))
...
def validate_topology(*, ep, tp, device_ids, require_target=False):
    if ep % tp != 0:
        raise ValueError(f"grouped LM head needs ep divisible by tp, got ep={ep}, tp={tp}")
    ...
    if N_EXPERTS_GLOBAL != 128 or N_LOCAL != 16:
        raise ValueError(
            "comparison requires 128 global and 16 local routed experts, "
            f"got {N_EXPERTS_GLOBAL} global and {N_LOCAL} local"
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(description="DeepSeek-V4 strict MTP-core benchmark.")
    ...
    validate_topology(ep=args.ep, tp=args.tp, device_ids=device_ids, require_target=args.finite_only)
    if N_RANKS != args.ep:
