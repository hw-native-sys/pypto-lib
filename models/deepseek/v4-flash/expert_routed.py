...
def expert_routed(
    ...
    # gate (w1) / up (w3) INT32 accumulators, flat expert-major so they survive
    # from the gate/up matmul loop (Loop 1) into the activation+w2 loop (Loop 2).
    ...
    # Loop 1: gate (w1) + up (w3) cube matmuls. Each stage's row-tile loop is
    # pushed inside the spmd so it launches once per expert (not once per tile).
    # The valid row count is read inside each stage so the count scalar lives on
    # the stage's own cores instead of a frame-level task feeding every stage.
    for local_i in pl.parallel(N_LOCAL_EXPERTS):
        # This expert's row slab of the two flat accumulators.
        flat_base = local_i * RECV_MAX
        gate_e = gate_i32[flat_base : flat_base + RECV_MAX]
        up_e = up_i32[flat_base : flat_base + RECV_MAX]

        # gate (w1) cube matmul -> flat INT32 GM accumulator.
        with pl.spmd(MOE_INTER // (MM_GATE_INNER * MM_INTER_TILE), name_hint="exp_gate_mm"):
            ...
            for t in pl.range(n_tiles):
                ...
                for ng in pl.range(MM_GATE_INNER):
                    ...
                    gate_e[t0 : t0 + RECV_TILE, n0 : n0 + MM_INTER_TILE] = pl.reshape(gate_acc, [RECV_TILE, MM_INTER_TILE])

        # up (w3) cube matmul -> flat INT32 GM accumulator.
        with pl.spmd(MOE_INTER // (MM_GATE_INNER * MM_INTER_TILE), name_hint="exp_up_mm"):
            ...
            for t in pl.range(n_tiles):
                ...
                for ng in pl.range(MM_GATE_INNER):
                    ...
                    up_e[t0 : t0 + RECV_TILE, n0 : n0 + MM_INTER_TILE] = pl.reshape(up_acc, [RECV_TILE, MM_INTER_TILE])

    # Loop 2: SwiGLU activation + h requant + w2 matmul + w2 dequant epilogue.
    # h_tile_fp32 / h_tile_i8 / h_tile_scale_dq / y_i32 are produced and consumed
    # within this loop, so they stay per-expert [RECV_MAX, ...].
    for local_i in pl.parallel(N_LOCAL_EXPERTS):
        # This expert's row slab of the two flat accumulators.
        flat_base = local_i * RECV_MAX
        gate_slab = gate_i32[flat_base : flat_base + RECV_MAX]
        up_slab = up_i32[flat_base : flat_base + RECV_MAX]

        # SwiGLU activation -> FP32 GM.
        ...
        with pl.spmd(MOE_INTER // (ACT_GATE_INNER * ACT_INTER_TILE), name_hint="exp_gate_up_act"):
            ...
            for t in pl.range(n_tiles):
                ...
                for ng in pl.pipeline(ACT_GATE_INNER, stage=2):
                    ...
                    gate_2d_i32 = gate_slab[t0 : t0 + RECV_TILE, n0 : n0 + ACT_INTER_TILE]
                    up_2d_i32 = up_slab[t0 : t0 + RECV_TILE, n0 : n0 + ACT_INTER_TILE]
                    ...
                    h_tile_fp32[t0 : t0 + RECV_TILE, n0 : n0 + ACT_INTER_TILE] = gated_masked

        # A8 requant of h before the w2 matmul -> INT8 GM + per-row dequant scale.
        h_tile_i8 = pl.create_tensor([RECV_MAX, MOE_INTER], dtype=pl.INT8)
        h_tile_scale_dq = pl.create_tensor([RECV_MAX, 1], dtype=pl.FP32)
        with pl.at(level=pl.Level.CORE_GROUP, name_hint="exp_h_q"):
            ...
            for t in pl.range(n_tiles):
                ...
                for k0 in pl.pipeline(0, MOE_INTER, QUANT_TILE, stage=2):
                    eh_a_f32 = h_tile_fp32[t0 : t0 + RECV_TILE, k0 : k0 + QUANT_TILE]
                ...
                h_tile_scale_dq[t0 : t0 + RECV_TILE, :] = pl.reshape(pl.recip(eh_sq_row), [RECV_TILE, 1])
                eh_sq_col = pl.reshape(eh_sq_row, [RECV_TILE, 1])
                for k1 in pl.pipeline(0, MOE_INTER, QUANT_TILE, stage=2):
                    eh_q_f32 = h_tile_fp32[t0 : t0 + RECV_TILE, k1 : k1 + QUANT_TILE]
                    ...
                    h_tile_i8[t0 : t0 + RECV_TILE, k1 : k1 + QUANT_TILE] = pl.cast(eh_q_half, target_type=pl.INT8, mode="trunc")

        # w2 cube matmul -> INT32 GM accumulator.
            ...
            for t in pl.range(n_tiles):
                ...
                for dg in pl.range(W2_INNER):
                    ...
                    for k0 in pl.pipeline(0, MOE_INTER, INTER_K, stage=2):
                        h_k = h_tile_i8[t0 : t0 + RECV_TILE, k0 : k0 + INTER_K]
                    ...
                    y_i32[t0 : t0 + RECV_TILE, d0 : d0 + D_OUT_TILE] = pl.reshape(y_acc, [RECV_TILE, D_OUT_TILE])

        # w2 dequant + routing-weight scale -> BF16, assembled into recv_y,
        # folded into the same compute loop. Kept as a frame-level tile loop:
        # recv_y's nz layout in the moe composition forces the writeback through
        # pl.assemble (frame-level only), so this stage's tile loop cannot be
        # pushed inside the spmd like the others; assembling recv_y_tile per tile
        # right after the spmd avoids a full-buffer GM round-trip that would race.
        # Distinct loop vars (wt / w_*) so the frame-level tile loop does not
        # collide with the stages' inner t after inlining.
        w_rows = pl.read(recv_expert_count, [local_i, 0])
        w_tiles = (w_rows + RECV_TILE - 1) // RECV_TILE
        for wt in pl.range(w_tiles):
            wt0 = wt * RECV_TILE
            flat_wt0 = flat_base + wt0
            ...
            with pl.spmd(
                ...
                w_col_blk = pl.reshape(
                    recv_weights[local_i : local_i + 1, wt0 : wt0 + RECV_TILE],
                    [RECV_TILE, 1],
                )
                row_scale_blk = pl.mul(h_tile_scale_dq[wt0 : wt0 + RECV_TILE, :], w_col_blk)
                for dg in pl.pipeline(W2_ACT_INNER, stage=2):
                    act_d0 = act_d_base + dg * D_OUT_TILE_ACT
                    y_2d_i32 = y_i32[wt0 : wt0 + RECV_TILE, act_d0 : act_d0 + D_OUT_TILE_ACT]
            ...
            recv_y_flat = pl.assemble(recv_y_flat, recv_y_tile, [flat_wt0, 0])
