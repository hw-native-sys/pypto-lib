...
"""DeepSeek-V4 Flash host-prepared 43-layer decode through FP32 logits."""
# ruff: noqa: F403,F405
...
# decode_fwd_logits is self-contained: it imports kernels, constants, and per-kind
...
# stage. Their content is smoke-only (decode_fwd_logits has no golden_fn), so we randn a
...
def _make_layer_stacked_spec(name, base_specs, layer_count=FWD_NUM_LAYERS):
    ...
    def init_value():
        if name == "tid2eid":
            ...
            # Each layer reuses hash-layer-zero routing. Across EP8 x T8 x top-k 6,
            # the 384 active routes cover 128 experts exactly three times.
            layer_eids = (token_ids * TOPK + topk_ids) % N_EXPERTS_GLOBAL
            packed = torch.cat(
                [layer_eids for _ in range(layer_count)],
                dim=0,
            )
            return packed.unsqueeze(0).expand(
                N_RANKS,
                -1,
                -1,
            ).contiguous()
...
def _make_balanced_input_ids_spec(base_spec, num_tokens):
    """Offset active token IDs by rank for balanced hash routing."""
    import torch
    from golden import TensorSpec

    def init_value():
        rank_starts = (
            torch.arange(N_RANKS, dtype=torch.int64).unsqueeze(1)
            * num_tokens
        )
...
def build_tensor_specs(
    ...
    for name in ordered_names:
        if name in metadata_specs:
            ...
            raise ValueError(f"unclassified decode_fwd_logits spec: {name}")
...
def main():
    parser = argparse.ArgumentParser(description="DeepSeek-V4 Flash host-prepared decode-to-logits benchmark.")
    ...
    assert args.tp <= args.ep, f"decode_fwd_logits requires --tp <= --ep, got tp={args.tp}, ep={args.ep}"
