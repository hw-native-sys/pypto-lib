...
def build_graph(rank_dir: Path, root: Path, tol: int) -> RankGraph:
    ...
    for core, tid, _seq, st, en, cid in recs:
        t = str(tid)
        ...
        for st, en, t in sl:
            if best_task is not None and best_task != t and best_end <= st + tol and st == start[t]:
                ...
                if cur is None or best_end > cur[1]:
                    core_prev[t] = (best_task, best_end)
            if en > best_end:
                best_end, best_task = en, t

    label = str(rank_dir.relative_to(root)) if rank_dir != root else rank_dir.name
    return RankGraph(freq=freq, start=start, end=end, dur=dur, name=name, nblock=dict(nblock),
                     hb_pred=hb_pred, core_prev=core_prev, t0=min(start.values()),
                     t1=max(end.values()), kept_edges=kept, label=label)
...
def static_cpm(g: RankGraph, order: list[str]) -> tuple[list[str], int]:
    ...
    for v in order:
        ...
        choice[v] = best_p
    sink = max(finish, key=finish.get)
    path: list[str] = []
    v: str | None = sink
    while v is not None:
        path.append(v)
...
def analyze_rank(g: RankGraph, tol: int) -> RankResult:
    ...
    for t, kind in walk:
        ...
        segments.append(Segment(task=t, name=g.name[t], dur=g.dur[t],
                                compute=eff, stall=gap, kind=kind))
    ...
    return RankResult(
        label=g.label, freq=g.freq, makespan=g.t1 - g.t0,
        cpm_len=cpm_len, cpm_ntasks=len(cpm_path),
        cpm_head=[g.name[t] for t in cpm_path[:10]],
        segments=segments, compute_total=compute_total, stall_total=stall_total,
        stall_by_kind=dict(stall_by_kind), ntasks=len(g.start), kept_edges=g.kept_edges)
...
def render_report(results: list[RankResult], top: int) -> str:
    ...
    w("Generated by `tools/critical_path.py`. Times are device wall-clock; "
      "percentages are of each rank's makespan.\n")
    w("- **Static CPM path**: dependency-limited latency floor (unlimited cores).")
    w("- **Observed path**: the as-executed critical path; its per-task compute + "
      "the runtime-scheduling stall before each task tile the makespan exactly.")
    w("- **stall kinds**: `data-wait` = waiting on an upstream producer; "
    ...
    for r in results:
        ...
        w(f"- **static CPM path**: {_us(r.cpm_len, r.freq)/1e3:.3f} ms "
          f"({_pct(r.cpm_len, ms):.1f}% of makespan) over {r.cpm_ntasks} tasks")
        w(f"- **observed critical path**: {len(r.segments)} tasks")
        w(f"  - compute: {_us(r.compute_total, r.freq)/1e3:.3f} ms "
          f"({_pct(r.compute_total, ms):.1f}%)")
        w(f"  - stall (runtime scheduling): {_us(r.stall_total, r.freq)/1e3:.3f} ms "
          f"({_pct(r.stall_total, ms):.1f}%)")
        ...
        w(f"  - tiling check: compute+stall = {check} ticks vs makespan {ms} ticks "
          f"({'exact' if check == ms else 'diff %d' % (check - ms)})")
        ...
        for fam, cval in fam_c.most_common(top):
            w(f"| {fam} | {_us(cval, r.freq)/1e3:.3f} | {_pct(cval, ms):.1f}% | "
        ...
        for i, seg in enumerate(r.segments):
            w(f"| {i} | {seg.name} | {_us(seg.dur, r.freq):.1f} | "
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    p.add_argument("build_dir", type=Path,
                   help="build_output/<case> directory (or any dir containing "
                        "dfx_outputs/rank*/d*/l2_swimlane_records.json)")
    p.add_argument("--report", default="critical_path_report.md",
                   help="report filename, written under build_dir")
    p.add_argument("--top", type=int, default=25,
                   help="rows in the kernel-family table")
    p.add_argument("--tol", type=int, default=2,
                   help="tick tolerance when deciding 'finished before' (clock ticks)")
    p.add_argument("--stdout", action="store_true",
                   help="also print a one-line-per-rank summary to stdout")
...
def main(argv: list[str]) -> int:
    ...
    if not rank_dirs:
        print(f"error: no l2_swimlane_records.json (+deps.json+name_map.json) found "
              f"under {root}. Did the run use --enable-l2-swimlane?", file=sys.stderr)
        return 2
