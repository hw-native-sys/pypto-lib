...
def _run_l3_resident(
    ...
    def _dispatch_resident(dispatch_fn: "Callable[[Any, list[Any]], None]") -> None:
        ...
        with compiled.prepare() as rt:
            ...
            try:
                ...
                dispatch_fn(rt, [_arg(n) for n in ordered_names])
            finally:
    ...
    if not bench:
        _dispatch_resident(lambda rt, ordered: rt(*ordered, config=run_config))
        _validate_once()
        return None
    ...
    def _bench_dispatch(rt: Any, ordered: list[Any]) -> None:
