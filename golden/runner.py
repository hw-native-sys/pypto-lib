    ...
    with _Stage("compute golden"):
        ...
        for spec in specs:
            if isinstance(spec, ScalarSpec):
                ...
                scratch[spec.name] = tensors[spec.name]
                try:
                    readonly_versions[spec.name] = tensors[spec.name]._version
        ...
        mutated_inputs = sorted(
            ...
            if tensors[name]._version != version
        )
        if mutated_inputs:
            raise ValueError(
...
def _run_pipeline(
    ...
    *prologue* runs entry-specific spec validation, may raise ``ValueError``,
    ...
    try:
        ...
        if not compile_only and data_dir is not None and not data_dir.is_dir():
