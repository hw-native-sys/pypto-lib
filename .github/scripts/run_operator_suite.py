...
# -----------------------------------------------------------------------------------------------------------
"""Run and report twenty fixed MTP/DSpark workloads in CI's environment."""
    ...
    return {
        ...
        "cases": [{"case_id": case["case_id"], "report_cases": [row["case_id"] for row in rows],
                   ...
                   "queue_prefix": ["task-submit", "--device",
                                    ",".join(map(str, profile["sequences"][str(case["device_count"])]))],
...
def run_worker(args, manifest, profile):
    ...
    try:
        ...
        command = [sys.executable, str(ROOT / ".github/scripts/run_operator_case.py"),
                   "--case", case["case_id"], "--devices", ",".join(map(str, selected)),
                   "--output", str(output / "raw-result.json")]
...
def queue_command(case, profile, profile_path, output, args, env, remaining):
    # These are provisional safety limits, not measured execution estimates.
    task_limit = args.case_timeout + 90  # Process cleanup and health observations.
    ...
    if wait_limit <= task_limit:
        raise ContractError("suite budget exhausted before another full case could start")
    command = [sys.executable, str(ROOT / ".github/scripts/run_operator_suite.py"),
               "--worker-case", case["case_id"], "--device-profile", str(profile_path),
...
def execute(args, manifest, profile):
    ...
    groups = measurement_groups(manifest)
    suite = {"schema_version": 1, "suite_id": manifest["suite_id"], "run_id": run_id,
             ...
             "execution_count": len(groups), "limits_provisional": True,
             "limits_seconds": {"case": args.case_timeout, "queue": args.queue_timeout,
                                "suite": args.suite_timeout}, "cases": []}
    ...
    try:
        ...
        if os.environ.get("TASKQUEUE_INSIDE") == "1":
            raise ContractError("the suite submits per-case allocations; do not wrap it in task-submit")
    ...
    for case, rows in groups:
        ...
        try:
            ...
            command = queue_command(case, profile, profile_path, case_dir, args, env, remaining)
        ...
        record.update(status="running", source_sha=source_sha, toolchain=toolchain,
                      device_identity={**host, "selected": record["devices"]}, command=command,
                      log=f"{case['case_id']}/run.log")
        checkpoint()
        submitted = time.monotonic()
        outcome = {"status": "queue_failed", "safe_to_continue": False}
        try:
            client_timeout = int(command[command.index("--timeout") + 1]) + 30
            with (case_dir / "queue.log").open("w") as log:
                rc = subprocess.run(command, env=env, stdout=log, stderr=subprocess.STDOUT,
                                    timeout=client_timeout, check=False).returncode
            record["queue_rc"] = rc
            evidence = read_json(case_dir / "measurement.json")
            if evidence.pop("case_id", None) != case["case_id"]:
                raise ContractError("measurement belongs to another case")
            if evidence.get("status") == "running":
                raise ContractError("queue returned without a completed measurement")
            outcome = evidence
            # A transport/reporting failure does not erase a validated measurement.
            if rc != (0 if evidence["status"] == "pass" else 1):
                record["queue_error"] = f"queue exit {rc} disagrees with completed {evidence['status']} measurement"
                outcome["safe_to_continue"] = False
        except (OSError, ValueError, KeyError, TypeError, subprocess.SubprocessError) as error:
            outcome.update(error=f"queue completion could not be verified: {error}")
        record.update(outcome, submission_wall_seconds=time.monotonic() - submitted)
        if not outcome.get("safe_to_continue"):
            stopped = "remaining measurements stopped: " + record.get("queue_error", record.get("error", "unknown queue state"))
        for row in rows:
            target = records[row["case_id"]]
            if target is not record:
                identity = {key: target[key] for key in ("case_id", "case_contract", "shared_measurement")}
                target.update(copy.deepcopy(record), **identity)
            target["previous"] = comparison(target, prior.get(row["case_id"]))
            target["week"] = comparison(target, week.get(row["case_id"]))
        checkpoint()
...
def main(argv=None):
    ...
    parser.add_argument("--suite-timeout", type=int, default=10800)
    parser.add_argument("--worker-case", choices=sorted(CASE_IDS))
    parser.add_argument("--dry-run", action="store_true")
    args = parser.parse_args(argv)
    manifest = load_manifest()
    profile = load_profile(args.device_profile)
    datetime.fromisoformat(args.date)
    if min(args.case_timeout, args.queue_timeout, args.suite_timeout) <= 0:
        parser.error("timeouts must be positive")
    if not args.worker_case and args.queue_timeout <= args.case_timeout + 90:
        parser.error("queue completion timeout must exceed the case timeout plus 90 seconds")
    if args.dry_run:
    ...
    if args.worker_case:
        return run_worker(args, manifest, profile)
