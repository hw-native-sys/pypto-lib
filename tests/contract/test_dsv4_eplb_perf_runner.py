...
_STATS_MOE_COMPARATOR = (
    "rowwise_ratio_reldiff(output=x_next, row_shape=(8, 8), mapping=None, "
    "diff_thd=0.003, pct_thd=0.1, max_diff_hd=inf, "
...
def _numeric_mtp_log(tmp_path: Path) -> Path:
    path = _mtp_log(tmp_path)
    text = path.read_text(encoding="utf-8")
    for comparator in _STATS_MTP_COMPARATORS.values():
        text = text.replace(
            "(finite_tensor_compare)",
            f"({comparator})",
            1,
        )
    text = text.replace(
        f"[RUN] fixture seed={_SEED} python_hash_seed={_SEED}\n",
        f"[RUN] fixture seed={_SEED} python_hash_seed={_SEED}\n"
        "[VALIDATION] mode=numeric_golden reference=golden_eplb_mtp_core\n"
        "[GOLDEN METRIC] output=kv_cache active_rows=64\n"
        "[GOLDEN METRIC] output=hidden_out active_rows=64\n"
        "[GOLDEN METRIC] output=next_pre_hc_hidden active_rows=64\n"
        "[GOLDEN METRIC] output=logits active_rows=64\n",
        1,
    )
...
def _numeric_decode_log(tmp_path: Path) -> Path:
    path = _decode_log(tmp_path)
    validation_dtypes = {
        "idx_kv_cache": "torch.int8",
        "hidden_out": "torch.bfloat16",
    }
    validation_lines = [
        "[VALIDATION] mode=numeric_golden reference=golden_eplb_decode_logits",
        *[
            f"[RUN]   '{name}' PASS  shape=(8,) dtype=torch.float32 "
            f"({comparator})"
            for name, comparator in _STATS_DECODE_COMPARATORS.items()
            if name not in validation_dtypes
        ],
        *[
            f"[RUN]   '{name}' PASS  shape=(8,) dtype={dtype} "
            f"({_STATS_DECODE_COMPARATORS[name]})"
            for name, dtype in validation_dtypes.items()
        ],
        *[
            f"[GOLDEN METRIC] output={name} active_rows={active_rows} "
            "min_cosine=1.000000 max_rel_l2=0.000000"
            for name, active_rows in _STATS_DECODE_METRIC_ROWS.items()
        ],
    ]
    text = path.read_text(encoding="utf-8")
    text = text.replace(
        ...
        "[RUN] PASS (1.00s, validation skipped: no golden_fn or golden_data)",
        "[RUN] PASS (1.00s)",
    )
    path.write_text(text, encoding="utf-8")
    return path
...
def test_stats_profile_requires_the_declared_moe_golden(tmp_path: Path) -> None:
    path = _moe_log(tmp_path)
    text = path.read_text(encoding="utf-8").replace(
        "(ratio_reldiff)",
        f"({_STATS_MOE_COMPARATOR})",
        1,
    ).replace(
        f"[RUN] fixture seed={_SEED} python_hash_seed={_SEED}\n",
        f"[RUN] fixture seed={_SEED} python_hash_seed={_SEED}\n"
    ...
    result = parse_perf_log(
        ...
        warmup=_WARMUP,
        device_set=_DEVICE_SET,
        validation_profile="stats-placement-numeric",
    )

    assert result.metric_contract_version == "dsv4-stats-placement-moe-ep8-v3"
    assert result.validation_mode == "numeric_golden"
    ...
    with pytest.raises(MetricParseError, match="validation comparator mismatch"):
        parse_perf_log(
            "moe-ep8",
            path,
            rounds=_ROUNDS,
...
def test_stats_profile_requires_moe_row_coverage(tmp_path: Path) -> None:
    path = _moe_log(tmp_path)
    text = path.read_text(encoding="utf-8").replace(
        "(ratio_reldiff)",
        f"({_STATS_MOE_COMPARATOR})",
        ...
        f"[RUN] fixture seed={_SEED} python_hash_seed={_SEED}\n"
        "[VALIDATION] mode=numeric_golden reference=golden_moe\n"
        "[GOLDEN METRIC] output=x_next active_rows=63\n",
        1,
    )
    path.write_text(text, encoding="utf-8")

    with pytest.raises(MetricParseError, match="Golden metric coverage mismatch"):
        parse_perf_log(
            "moe-ep8",
            path,
            rounds=_ROUNDS,
...
def test_stats_profile_requires_full_numeric_decode_validation(tmp_path: Path) -> None:
    log_path = _numeric_decode_log(tmp_path)
    result = parse_perf_log(
        "decode-logits",
        log_path,
        rounds=_ROUNDS,
    ...
    assert result.metric_contract_version == "dsv4-stats-placement-numeric-v4"
    assert result.validation_mode == "numeric_golden"
    assert len(result.validation_outputs) == 11
    assert "[GOLDEN METRIC] output=idx_kv_cache" not in log_path.read_text(
        encoding="utf-8"
    )
...
@pytest.mark.parametrize(
    "mutated_comparator",
    [
        _STATS_DECODE_COMPARATORS["kv_cache"].replace(
            "min_cosine=0.99", "min_cosine=0.98", 1
        ),
        _STATS_DECODE_COMPARATORS["kv_cache"].replace(
            "(kv_cache,", "(hca_cmp_kv,", 1
        ),
    ],
    ids=("wrong-threshold", "wrong-pool"),
)
def test_stats_profile_rejects_nonexact_decode_comparator(
    tmp_path: Path,
    mutated_comparator: str,
) -> None:
    path = _numeric_decode_log(tmp_path)
    expected = _STATS_DECODE_COMPARATORS["kv_cache"]
    text = path.read_text(encoding="utf-8").replace(
        f"({expected})",
        f"({mutated_comparator})",
        1,
    )
    path.write_text(text, encoding="utf-8")
...
@pytest.mark.parametrize(
    ...
    [
        "",
        (
            "[GOLDEN METRIC] output=kv_cache active_rows=2751 "
            "min_cosine=1.000000 max_rel_l2=0.000000"
        ),
    ],
    ids=("missing-metric", "wrong-active-rows"),
)
def test_stats_profile_rejects_incomplete_decode_golden_coverage(
    tmp_path: Path,
    replacement: str,
) -> None:
    path = _numeric_decode_log(tmp_path)
    expected_line = (
        "[GOLDEN METRIC] output=kv_cache active_rows=2752 "
        "min_cosine=1.000000 max_rel_l2=0.000000"
    )
    text = path.read_text(encoding="utf-8").replace(expected_line, replacement, 1)
    path.write_text(text, encoding="utf-8")
...
def test_stats_profile_accepts_numeric_mtp_and_rejects_finite_only(
    tmp_path: Path,
) -> None:
    result = parse_perf_log(
        "mtp-core",
        _numeric_mtp_log(tmp_path),
        rounds=_ROUNDS,
        warmup=_WARMUP,
        device_set=_DEVICE_SET,
        validation_profile="stats-placement-numeric",
    ...
    finite_path = _mtp_log(tmp_path)
    finite_text = finite_path.read_text(encoding="utf-8").replace(
        f"[RUN] fixture seed={_SEED} python_hash_seed={_SEED}\n",
        f"[RUN] fixture seed={_SEED} python_hash_seed={_SEED}\n"
        "[VALIDATION] mode=numeric_golden reference=golden_eplb_mtp_core\n",
        1,
    )
    finite_path.write_text(finite_text, encoding="utf-8")
    with pytest.raises(MetricParseError, match="validation comparator mismatch"):
        parse_perf_log(
            "mtp-core",
            finite_path,
...
def test_stats_profile_requires_exact_numeric_mtp_thresholds(tmp_path: Path) -> None:
    path = _numeric_mtp_log(tmp_path)
    expected = _STATS_MTP_COMPARATORS["kv_cache"]
    mutated = expected.replace("diff_thd=0.01", "diff_thd=0.02", 1)
    path.write_text(
        path.read_text(encoding="utf-8").replace(
            f"({expected})",
            f"({mutated})",
...
def test_stats_profile_requires_mtp_row_coverage(
    tmp_path: Path,
    output_name: str,
) -> None:
    path = _numeric_mtp_log(tmp_path)
    path.write_text(
        path.read_text(encoding="utf-8").replace(
            f"[GOLDEN METRIC] output={output_name} active_rows=64",
            f"[GOLDEN METRIC] output={output_name} active_rows=0",
        ),
        encoding="utf-8",
    )
...
def test_stats_profile_rejects_missing_mtp_row_metric(tmp_path: Path) -> None:
    path = _numeric_mtp_log(tmp_path)
    path.write_text(
        path.read_text(encoding="utf-8").replace(
            "[GOLDEN METRIC] output=logits active_rows=64\n",
...
def test_legacy_mtp_profile_still_requires_finite_only(tmp_path: Path) -> None:
    with pytest.raises(MetricParseError, match="validation comparator mismatch"):
        parse_perf_log(
            "mtp-core",
            _numeric_mtp_log(tmp_path),
...
def test_legacy_profile_keeps_prefix_comparator_compatibility(tmp_path: Path) -> None:
    path = _moe_log(tmp_path)
    path.write_text(
        path.read_text(encoding="utf-8").replace(
            "(ratio_reldiff)",
...
def test_parser_rejects_unsupported_devices(tmp_path: Path) -> None:
...
@pytest.mark.parametrize(
    ("case", "log_factory", "expected_device"),
    [
        ("decode-logits", _decode_log, 1),
        ("mtp-core", _mtp_log, 15),
    ],
)
def test_parser_preserves_the_allocator_device_mapping(
    tmp_path: Path,
    case: str,
    log_factory: Callable[[Path], Path],
    expected_device: int,
) -> None:
    result = parse_perf_log(
        case,
        log_factory(tmp_path),
        rounds=_ROUNDS,
        warmup=_WARMUP,
        device_set=_ALLOCATOR_DEVICE_SET,
    )

    assert result.selected_device == expected_device
    assert [metric.device_id for metric in result.rank_metrics if metric.slot == 0] == [
        1,
        3,
        5,
        7,
